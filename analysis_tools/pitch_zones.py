...
def identify_zone(
    single_event, zone_type="jdp_custom", get_centers=False, source="WhoScored"
):
    """Identify pitch zone in which a WhoScored-style event started and finished.
    ...
    if source == "Statsbomb":
        ...
        box_y_ratio = 0.225
        x_startpos = (
            single_event["location"][0]
            if single_event["location"] == single_event["location"]
            else single_event["location"]
        )
        y_startpos = (
            single_event["location"][1]
            if single_event["location"] == single_event["location"]
            else single_event["location"]
        )
        if single_event["type"] == "Pass":
            x_endpos = (
                single_event["pass_end_location"][0]
                if single_event["pass_end_location"]
                == single_event["pass_end_location"]
                else single_event["pass_end_location"]
            )
            y_endpos = (
                single_event["pass_end_location"][1]
                if single_event["pass_end_location"]
                == single_event["pass_end_location"]
                else single_event["pass_end_location"]
            )
            ...
            x_endpos = (
                single_event["carry_end_location"][0]
                if single_event["carry_end_location"]
                == single_event["carry_end_location"]
                else single_event["carry_end_location"]
            )
            y_endpos = (
                single_event["carry_end_location"][1]
                if single_event["carry_end_location"]
                == single_event["carry_end_location"]
                else single_event["carry_end_location"]
            )
    ...
    if zone_type == "jdp_custom":
        for idx, [x_pos, y_pos] in enumerate(
            [[x_startpos, y_startpos], [x_endpos, y_endpos]]
        ):
            if (x_pos == 0 and y_pos == 0) or x_pos != x_pos or y_pos != y_pos:
                ...
                if y_pos < box_y_ratio * pitch_width_y:
                    zone[idx] = int(2)
                    zone_center[idx] = (
                        box_x_ratio * pitch_length_x / 2,
                        box_y_ratio * pitch_width_y / 2,
                    )
                elif (
                    box_y_ratio * pitch_width_y
                    <= y_pos
                    <= (1 - box_y_ratio) * pitch_width_y
                ):
                    zone[idx] = int(1)
                    zone_center[idx] = (
                        box_x_ratio * pitch_length_x / 2,
                        ((1 - box_y_ratio) + box_y_ratio) * pitch_width_y / 2,
                    )
                elif y_pos > (1 - box_y_ratio) * pitch_width_y:
                    zone[idx] = int(0)
                    zone_center[idx] = (
                        box_x_ratio * pitch_length_x / 2,
                        (1 + (1 - box_y_ratio)) * pitch_width_y / 2,
                    )
            elif box_x_ratio * pitch_length_x < x_pos <= 0.5 * pitch_length_x:
                if y_pos < box_y_ratio * pitch_width_y:
                    zone[idx] = int(7)
                    zone_center[idx] = (
                        (box_x_ratio + 0.5) * pitch_length_x / 2,
                        box_y_ratio * pitch_width_y / 2,
                    )
                    ...
                    zone_center[idx] = (
                        (box_x_ratio + 0.5) * pitch_length_x / 2,
                        (0.3675 + box_y_ratio) * pitch_width_y / 2,
                    )
                elif 0.3675 * pitch_width_y <= y_pos <= 0.6325 * pitch_width_y:
                    zone[idx] = int(5)
                    zone_center[idx] = (
                        (box_x_ratio + 0.5) * pitch_length_x / 2,
                        (0.3675 + 0.6325) * pitch_width_y / 2,
                    )
                elif (
                    0.6325 * pitch_width_y < y_pos <= (1 - box_y_ratio) * pitch_width_y
                ):
                    zone[idx] = int(4)
                    zone_center[idx] = (
                        (box_x_ratio + 0.5) * pitch_length_x / 2,
                        ((1 - box_y_ratio) + 0.6325) * pitch_width_y / 2,
                    )
                elif y_pos > (1 - box_y_ratio) * pitch_width_y:
                    zone[idx] = int(3)
                    zone_center[idx] = (
                        (box_x_ratio + 0.5) * pitch_length_x / 2,
                        ((1 - box_y_ratio) + 1) * pitch_width_y / 2,
                    )
            elif 0.5 * pitch_length_x < x_pos < (2 / 3) * pitch_length_x:
                if y_pos < box_y_ratio * pitch_width_y:
                    zone[idx] = int(10)
                    zone_center[idx] = (
                        ((2 / 3) + 0.5) * pitch_length_x / 2,
                        box_y_ratio * pitch_width_y / 2,
                    )
                elif box_y_ratio * pitch_width_y <= y_pos < 0.3675 * pitch_width_y:
                    zone[idx] = int(12)
                    zone_center[idx] = (
                        ((1 - box_x_ratio) + 0.5) * pitch_length_x / 2,
                        (0.3675 + box_y_ratio) * pitch_width_y / 2,
                    )
                elif 0.3675 * pitch_width_y <= y_pos <= 0.6325 * pitch_width_y:
                    zone[idx] = int(9)
                    ...
                    zone_center[idx] = (
                        ((1 - box_x_ratio) + 0.5) * pitch_length_x / 2,
                        (0.6325 + (1 - box_y_ratio)) * pitch_width_y / 2,
                    )
                elif y_pos > (1 - box_y_ratio) * pitch_width_y:
                    zone[idx] = int(8)
                    zone_center[idx] = (
                ...
                if y_pos < box_y_ratio * pitch_width_y:
                    zone[idx] = int(15)
                    zone_center[idx] = (
                        ((2 / 3) + (1 - box_x_ratio)) * pitch_length_x / 2,
                        box_y_ratio * pitch_width_y / 2,
                    )
                elif box_y_ratio * pitch_width_y <= y_pos < 0.3675 * pitch_width_y:
                    zone[idx] = int(12)
                ...
                if y_pos < box_y_ratio * pitch_width_y:
                    zone[idx] = int(18)
                    zone_center[idx] = (
                        (1 + (1 - box_x_ratio)) * pitch_length_x / 2,
                        box_y_ratio * pitch_width_y / 2,
                    )
                elif (
                    box_y_ratio * pitch_width_y
                    ...
                    zone_center[idx] = (
                        (1 + (1 - box_x_ratio)) * pitch_length_x / 2,
                        (box_y_ratio + (1 - box_y_ratio)) * pitch_width_y / 2,
                    )
                elif y_pos > (1 - box_y_ratio) * pitch_width_y:
                    zone[idx] = int(16)
                    zone_center[idx] = (
        ...
        for idx, [x_pos, y_pos] in enumerate(
            ...
            if (x_pos == 0 and y_pos == 0) or x_pos != x_pos or y_pos != y_pos:
                ...
                if y_pos < box_y_ratio * pitch_width_y:
                    zone[idx] = int(7)
                    zone_center[idx] = (
                        (box_x_ratio + (1 / 3)) * pitch_length_x / 2,
                        box_y_ratio * pitch_width_y / 2,
                    )
                elif box_y_ratio * pitch_width_y <= y_pos < 0.3675 * pitch_width_y:
                    zone[idx] = int(6)
...
def add_pitch_zones(
    ...
    if pitch_orientation == "vertical":
        ...
        if zone_type == "jdp_dense":
            pitch.plot(
                [box_y_ratio * pitch_width_y, box_y_ratio * pitch_width_y],
                [(box_x_ratio + 0.001) * pitch_length_x, 0.499 * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            )
            pitch.plot(
                [box_y_ratio * pitch_width_y, box_y_ratio * pitch_width_y],
                [0.501 * pitch_length_x, (1 - box_x_ratio - 0.001) * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            )
            pitch.plot(
                [(1 - box_y_ratio) * pitch_width_y, (1 - box_y_ratio) * pitch_width_y],
                [(box_x_ratio + 0.001) * pitch_length_x, 0.499 * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            ...
            pitch.plot(
                [0.3675 * pitch_width_y, 0.3675 * pitch_width_y],
                [(box_x_ratio + 0.001) * pitch_length_x, 0.499 * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            ...
            pitch.plot(
                [0.6325 * pitch_width_y, 0.6325 * pitch_width_y],
                [(box_x_ratio + 0.001) * pitch_length_x, 0.499 * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            ...
            pitch.plot(
                [0.6325 * pitch_width_y, 0.6235 * pitch_width_y],
                [0.501 * pitch_length_x, (1 - box_x_ratio - 0.001) * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            ...
            pitch.plot(
                [0.001 * pitch_width_y, (box_y_ratio - 0.001) * pitch_width_y],
                [box_x_ratio * pitch_length_x, box_x_ratio * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            )
            pitch.plot(
                [(1 - box_y_ratio - 0.001) * pitch_width_y, 0.999 * pitch_width_y],
                [box_x_ratio * pitch_length_x, box_x_ratio * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            ...
            pitch.plot(
                [0.001 * pitch_width_y, 0.999 * pitch_width_y],
                [(1 / 3) * pitch_length_x, (1 / 3) * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            )
            pitch.plot(
                [0.001 * pitch_width_y, 0.999 * pitch_width_y],
                [(2 / 3) * pitch_length_x, (2 / 3) * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
            )
        ...
        if zone_type == "jdp_sparse":
            ...
            pitch.plot(
                [0.999 * pitch_width_y, (1 - box_y_ratio - 0.001) * pitch_width_y],
                [(1 / 3) * pitch_length_x, (1 / 3) * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
        ...
        if zone_type == "jdp_custom":
            ...
            pitch.plot(
                [0.3675 * pitch_width_y, 0.6235 * pitch_width_y],
                [(2 / 3) * pitch_length_x, (2 / 3) * pitch_length_x],
                c=line_colour,
                linestyle=ls,
                linewidth=lw,
