...
def add_cumulative_mins(events):
    ...
    for match_id in events["match_id"].unique():
        ...
        for period in np.arange(1, match_events["period"].max() + 1, 1):
            if period > 1:
                t_delta = (
                    match_events[match_events["period"] == period - 1][
                        "cumulative_mins"
                    ].max()
                    - match_events[match_events["period"] == period][
                        "cumulative_mins"
                    ].min()
                )
                ...
                t_delta = 0
            match_events.loc[match_events["period"] == period, "cumulative_mins"] += (
                t_delta
            )
...
def process_lineups(lineups, events, tactics):
    ...
    for match_id in events["match_id"].unique():
        ...
        starting_xi_players = match_tactics[
            match_tactics["id"].isin(starting_xis["id"])
        ]
        match_start_lineups = match_start_lineups.merge(
            starting_xi_players[["player_id", "position_name"]],
            how="left",
            on="player_id",
        )
        starting_formations = starting_xis[["team_id", "tactics_formation"]]
        match_start_lineups = match_start_lineups.merge(
            starting_formations, how="left", on="team_id"
        )
        ...
        match_start_lineups.loc[
            match_start_lineups["position_name"]
            == match_start_lineups["position_name"],
            "time_on",
        ] = 0
        match_start_lineups.loc[
        ...
        for team_id in events[events["match_id"] == match_id]["team_id"].unique():
            ...
            team_lineups = match_start_lineups[
                match_start_lineups["team_id"] == team_id
            ].copy()
            ...
            if len(match_events[match_events["type_name"] == "Substitution"]) != 0:
                team_sub_events = match_events[
                    ...
                    & (match_events["type_name"] == "Substitution")
                ]
            else:
            ...
            if len(match_events[match_events["type_name"] == "Tactical Shift"]) != 0:
                team_tactical_shift_events = match_events[
                    (match_events["team_id"] == team_id)
                    & (match_events["type_name"] == "Tactical Shift")
                ]
            else:
                team_tactical_shift_events = pd.DataFrame()

            if "bad_behaviour_card_name" in match_events.columns:
                team_bad_behaviour_events = match_events[
                    (match_events["team_id"] == team_id)
                    & (
                        match_events["bad_behaviour_card_name"].isin(
            ...
            if "foul_committed_card_name" in match_events.columns:
                team_foulcard_events = match_events[
                    (match_events["team_id"] == team_id)
                    & (
                        match_events["foul_committed_card_name"].isin(
                            ["Red Card", "Second Yellow"]
                        )
                    )
                ]
            ...
            team_tactical_events = pd.concat(
                [
                    team_sub_events,
                    team_tactical_shift_events,
                    team_bad_behaviour_events,
                    team_foulcard_events,
                ],
                ignore_index=False,
            ).sort_index()
            ...
            if len(team_tactical_events) >= 1:
                team_lineups.loc[
                    (team_lineups["position_name"] != "Substitute"), "time_off"
                ] = team_tactical_events["cumulative_mins"].min()
            else:
                team_lineups.loc[
                    (team_lineups["position_name"] != "Substitute"), "time_off"
                ] = match_duration
            ...
            for _, team_tactical_event in team_tactical_events.iterrows():
                ...
                if team_tactical_event["type_name"] == "Tactical Shift":
                    ...
                    # Get xi
                    current_xi = match_tactics[
                        match_tactics["id"] == team_tactical_event["id"]
                    ].sort_values("player_id")
                    current_xi = current_xi[
                        current_xi["player_id"].isin(prev_lineup["player_id"])
                    ]

                    # Copy prev lineup players based on new xi, and store new positions, formation and sub times
                    lineup_entry = (
                        prev_lineup[
                            prev_lineup["player_id"].isin(current_xi["player_id"])
                        ]
                        .copy()
                        .sort_values("player_id")
                    )
                    lineup_entry["tactics_formation"] = team_tactical_event[
                        "tactics_formation"
                    ]
                    lineup_entry["position_name"] = current_xi["position_name"].tolist()
                    lineup_entry["time_on"] = team_tactical_event["cumulative_mins"]
                    lineup_entry["time_off"] = (
                        match_duration
                        if idx == len(team_tactical_events) - 1
                        else team_tactical_events.iloc[idx + 1, :]["cumulative_mins"]
                    )
                ...
                # Subs
                    ...
                    subbed_in = team_lineups[
                        team_lineups["player_id"]
                        == team_tactical_event["substitution_replacement_id"]
                    ].tail(1)
                    subbed_out = lineup_entry[
                        lineup_entry["player_id"] == team_tactical_event["player_id"]
                    ]
                    subbed_in["position_name"] = subbed_out["position_name"].values[0]
                    subbed_in["tactics_formation"] = subbed_out[
                        "tactics_formation"
                    ].values[0]
                    lineup_entry = pd.concat(
                        [lineup_entry, subbed_in], ignore_index=True
                    )
                    lineup_entry["time_on"] = team_tactical_event["cumulative_mins"]
                    lineup_entry["time_off"] = (
                        match_duration
                ...
                elif team_tactical_event["type_name"] in [
                    "Bad Behaviour",
                    "Foul Committed",
                ]:
                    # Iterate tactical change and player change id
                ...
                team_lineups = pd.concat(
                    [team_lineups, lineup_entry], ignore_index=True
                )
                idx += 1

            # Add time played
            team_lineups["time_played"] = (
                team_lineups["time_off"] - team_lineups["time_on"]
            )

            # Tag players in starting xi
            team_lineups["starting_xi"] = team_lineups["player_id"].apply(
                lambda x: 1
                if x in list(team_lineups[team_lineups["time_on"] == 0]["player_id"])
                else np.nan
            )

            # Tag longest tactical setup
            temp_team_lineups = team_lineups[
                team_lineups["position_name"] != "Substitute"
            ]
            grouped_tactical_lineups = temp_team_lineups[
                ~temp_team_lineups.duplicated(
                    keep="first", subset=["player_setup_id", "tactical_setup_id"]
                )
            ]
            longest_tactical_setup_ids = grouped_tactical_lineups[
                grouped_tactical_lineups["time_played"]
                == np.max(grouped_tactical_lineups["time_played"])
            ][["player_setup_id", "tactical_setup_id"]]

            team_lineups["longest_tactic"] = team_lineups[
                ["player_setup_id", "tactical_setup_id", "time_played"]
            ].apply(
                lambda x: 1
                if x[0] == longest_tactical_setup_ids["player_setup_id"].values[0]
                and x[1] == longest_tactical_setup_ids["tactical_setup_id"].values[0]
                and x[2] == x[2]
                else np.nan,
                axis=1,
            )
            ...
            # Tag longest xi
            grouped_player_lineups = (
                temp_team_lineups[
                    ~temp_team_lineups.duplicated(
                        keep="first", subset=["player_setup_id", "tactical_setup_id"]
                    )
                ]
                .groupby("player_setup_id", as_index=False)
                .sum(numeric_only=True)
            )
            longest_player_setup_id = grouped_player_lineups[
                grouped_player_lineups["time_played"]
                == np.max(grouped_player_lineups["time_played"])
            ]["player_setup_id"]
            team_lineups["longest_xi"] = team_lineups[
                ["player_setup_id", "time_played"]
            ].apply(
                lambda x: 1
                if x[0] == longest_player_setup_id.values[0] and x[1] == x[1]
                else np.nan,
                axis=1,
            )
        ...
        match_lineups = match_lineups.sort_values(
            ["match_id", "team_id", "player_id", "time_on"]
        ).reset_index(drop=True)
        match_lineups = pd.concat(
            [
                match_lineups.groupby(
                    [
                        "player_id",
                        "player_name",
                        "player_nickname",
                ...
                .min()
                .drop(columns=["time_off", "time_played"]),
                match_lineups.groupby(
                    [
                        "player_id",
                        "player_name",
        ...
        match_lineups["time_played"] = (
            match_lineups["time_off"] - match_lineups["time_on"]
        )

        # Further condense to revert to standard lineups dataframe (using players most common position)
        match_lineups_condensed = pd.concat(
            [
                match_lineups.groupby(
                    [
                        "player_id",
        ...
        match_lineups_condensed["time_played"] = (
            match_lineups_condensed["time_off"] - match_lineups_condensed["time_on"]
        )

        # Add longest positions per player to condensed version
        player_pos_grouped = (
            match_lineups.groupby(["player_id", "position_name"], as_index=False)
            .sum(numeric_only=True)
            .sort_values(["player_id", "time_played"], ascending=False)
        )
        player_pos_grouped = player_pos_grouped[
            ~player_pos_grouped.duplicated(keep="first", subset=["player_id"])
        ][["player_id", "position_name"]]
        match_lineups_condensed["position_name"] = match_lineups_condensed.merge(
            player_pos_grouped, how="left", on="player_id"
        )["position_name"]

        # Add longest formation to condensed version
        team_form_grouped = (
            match_lineups.groupby(
                ["player_id", "team_id", "tactics_formation"], as_index=False
            )
            .sum(numeric_only=True)
            .sort_values(["team_id", "time_played"], ascending=False)
        )
        team_form_grouped = team_form_grouped[
            ~team_form_grouped.duplicated(keep="first", subset=["team_id"])
        ][["team_id", "tactics_formation"]]
        match_lineups_condensed["tactics_formation"] = match_lineups_condensed.merge(
            team_form_grouped, how="left", on="team_id"
        )["tactics_formation"]
        ...
        (
            match_lineups_condensed["position_group"],
            match_lineups_condensed["position_category"],
        ) = zip(*match_lineups_condensed["position_name"].apply(group_positions))

        # Sort and order dataframes
        match_lineups.sort_values(["match_id", "team_name", "time_on"], inplace=True)
        match_lineups_condensed.sort_values(
            ["match_id", "team_name", "time_on"], inplace=True
        )
        match_lineups = match_lineups[
            list(match_lineups.columns[0:16])
            + [
                "position_group",
                "position_category",
                "tactics_formation",
        ...
        match_lineups_condensed = match_lineups_condensed[
            list(match_lineups_condensed.columns[0:15])
            + [
                "position_name",
                "position_group",
                "position_category",
        ...
        lineups_out = pd.concat(
            [lineups_out, match_lineups_condensed], ignore_index=True
        )
        lineups_dense_out = pd.concat(
            [lineups_dense_out, match_lineups], ignore_index=True
        )
        ...
        for team in set(match_events["team_name"]):
            ...
            if event_team == "own":
                if event_name == "Touch":
                    ...
                    team_events = match_events[
                        match_events["possession_team_name"] == team
                    ]
                else:
                    team_events = match_events[
                        (match_events["team_name"] == team)
            ...
            for idx, player in team_lineup.iterrows():
                if event_name == "Possession":
                    event_count = len(
                        set(
                            team_events[
                                (team_events["cumulative_mins"] >= player["time_on"])
...
def add_player_nickname(events, lineups):
    ...
    lineups_out["player_nickname"] = lineups_out.apply(
        lambda row: (
            row["player_name"]
            if row["player_nickname"] is None
            else row["player_nickname"]
        ),
        axis=1,
    )
    ...
    events_out = events_out.merge(
        names_df, left_on="player", right_on="player_name", how="left"
...
def create_player_list(
    ...
    if additional_cols is None:
        included_cols = lineups.groupby(
            ...
            axis=0,
        ).sum(numeric_only=True)["time_played"]
    else:
        included_cols = lineups.groupby(
            [
                "player_id",
            ...
            axis=0,
        ).sum(numeric_only=True)[["time_played"] + additional_cols]

    playerinfo_df = playerinfo_df.merge(
        included_cols,
        left_on=[
            "player_id",
            "position_name",
    ...
    if group_position:
        ...
        if additional_cols is None:
            playerinfo_df["time_played"] = playerinfo_df.groupby(
                ["player_id", "team_name", "competition", "season"]
            )["time_played"].transform("sum")
        else:
            playerinfo_df[["time_played"] + additional_cols] = playerinfo_df.groupby(
                ["player_id", "team_name", "competition", "season"]
            )[["time_played"] + additional_cols].transform("sum")
        playerinfo_df.drop_duplicates(
            subset=["player_id", "team_name", "competition", "season"],
            keep="first",
            inplace=True,
        )
    ...
    if group_team:
        ...
        playerinfo_df.drop_duplicates(
            subset=["player_id", "competition", "season"], keep="first", inplace=True
        )

    # Group players minutes over different competitions (if group_comp is True).
    if group_comp:
        if additional_cols is None:
            playerinfo_df["time_played"] = playerinfo_df.groupby(["player_id"])[
                "time_played"
            ].transform("sum")
        else:
            playerinfo_df[["time_played"] + additional_cols] = playerinfo_df.groupby(
...
def group_player_events(
    events,
    player_data,
    group_type="count",
    agg_columns=None,
    primary_event_name="Column Name",
):
    """Aggregate event types per player, and add to player information dataframe
    ...
    player_data_out = player_data_out.merge(
        selected_events, left_on="player_id", right_index=True, how="left"
    )
    if agg_columns != list() and group_type in ["sum", "mean"]:
        player_data_out = player_data_out.rename(
            columns={agg_columns: primary_event_name}
        )
...
def create_team_list(lineups):
    ...
    included_cols_max = lineups.groupby(["team_name", "match_id"], axis=0).max()[
        "time_played"
    ]

    teaminfo_df = teaminfo_df.merge(
        included_cols_max, left_on=["team_name", "match_id"], right_index=True
    )
    teaminfo_df["matches_played"] = 1

    teaminfo_df = teaminfo_df.groupby(["team_name"]).sum()[
...
def group_team_events(
    ...
    # Merge into player information dataframe
    team_info_out = team_info_out.merge(
        selected_events, left_on="team_name", right_index=True, how="left"
    )
    if agg_columns != list() and group_type in ["sum", "mean"]:
...
def create_league_table(matches, xmetrics=False):
    ...
    leaguetable["team"] = sorted(
        list(
            set(
                matches_out["home_team"].values.tolist()
    ...
    matches_out["home_points"] = matches_out[["home_score", "away_score"]].apply(
        lambda x: 3 if x[0] > x[1] else 1 if x[0] == x[1] else 0, axis=1
    )
    matches_out["away_points"] = matches_out[["home_score", "away_score"]].apply(
        lambda x: 3 if x[0] < x[1] else 1 if x[0] == x[1] else 0, axis=1
    )

    # Add information to league table dataframe
    leaguetable["matches_played"] = (
        matches_out.groupby("home_team").count()["match_id"]
        + matches_out.groupby("away_team").count()["match_id"]
    ).values
    leaguetable["points"] = (
        matches_out.groupby("home_team").sum(numeric_only=True)["home_points"]
        + matches_out.groupby("away_team").sum(numeric_only=True)["away_points"]
    ).values
    leaguetable["goals_for"] = (
        matches_out.groupby("home_team").sum(numeric_only=True)["home_score"]
        + matches_out.groupby("away_team").sum(numeric_only=True)["away_score"]
    ).values
    leaguetable["goals_against"] = (
        matches_out.groupby("home_team").sum(numeric_only=True)["away_score"]
        + matches_out.groupby("away_team").sum(numeric_only=True)["home_score"]
    ).values
    leaguetable["goal_difference"] = (
        leaguetable["goals_for"] - leaguetable["goals_against"]
    )
    leaguetable["position"] = (
        leaguetable[["points", "goal_difference", "goals_for"]]
        .apply(tuple, axis=1)
    ...
    if xmetrics:
        leaguetable["xg_for"] = (
            matches_out.groupby("home_team").sum(numeric_only=True)["home_xg"]
            + matches_out.groupby("away_team").sum(numeric_only=True)["away_xg"]
        ).values
        leaguetable["xg_against"] = (
            matches_out.groupby("home_team").sum(numeric_only=True)["away_xg"]
            + matches_out.groupby("away_team").sum(numeric_only=True)["home_xg"]
        ).values
        leaguetable["xg_difference"] = leaguetable["xg_for"] - leaguetable["xg_against"]
        leaguetable["expected_points"] = (
            matches_out.groupby("home_team").sum(numeric_only=True)["home_xpoints"]
            + matches_out.groupby("away_team").sum(numeric_only=True)["away_xpoints"]
        ).values
        leaguetable["expected_position"] = (
            leaguetable[["expected_points", "xg_difference", "xg_for"]]
            .apply(tuple, axis=1)
            .rank(method="min", ascending=False)
            .astype(int)
        )
