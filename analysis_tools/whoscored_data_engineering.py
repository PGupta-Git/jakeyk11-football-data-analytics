...
def add_team_name(events_df, players_df):
    ...
    for match_id in events_df["match_id"].unique():
        team_name_and_id = players_df[players_df["match_id"] == match_id][
            ["teamId", "team"]
        ].values
        ...
        match_events["team_name"] = match_events["teamId"].apply(
            lambda x: team_dict[x] if x == x else np.nan
        )
        match_events["opp_team_name"] = match_events["teamId"].apply(
            lambda x: "".join(
                dict((k, v) for k, v in team_dict.items() if k != x).values()
            )
            if x == x
            else np.nan
        )
...
def cumulative_match_mins(events_df):
    ...
    for match_id in events_df["match_id"].unique():
        match_events = events_df[events_df["match_id"] == match_id].copy()
        match_events["cumulative_mins"] = (
            match_events["minute"] + (1 / 60) * match_events["second"]
        )
...
def minutes_played(players_df, events_df=None):
    ...
    for match_id in players_df["match_id"].unique():
        ...
        players[["time_on", "time_off", "mins_played"]] = players.apply(
            time_played, axis=1, result_type="expand", total_mins=match_minutes
        )
    ...
    for match_id in players_df["match_id"].unique():
        ...
        for idx, team in enumerate(players["teamId"].unique()):
            ...
            for _, player in team_player_df.iterrows():
                if (
                    player["time_off"] != match_end
                    and player["time_off"] == player["time_off"]
                ):
            ...
            for idxx in np.arange(1, len(sub_mins[idx])):
                ...
                if min_diff > max_min_diff:
                    ...
                    same_team_idxx = idxx
            same_team_mins = [
                sub_mins[idx][same_team_idxx - 1],
                sub_mins[idx][same_team_idxx],
            ]
            ...
            for playerid, player in team_player_df.iterrows():
                if (
                    player["time_on"] <= same_team_mins[0]
                    and player["time_off"] >= same_team_mins[1]
                ):
    ...
    for match_id in events_df["match_id"].unique():
        ...
        for team in set(match_events["teamId"]):
            ...
            for idx, player in team_players.iterrows():
                event_count = len(
                    team_events[
                        (team_events["expandedMinute"] > player["time_on"])
...
def create_player_list(lineups, additional_cols=None, pass_extra=None):
    ...
    if pass_extra is None:
        ...
        playerinfo_df = lineups[
            ["name", "position", "team"] + pass_extra
        ].drop_duplicates()

    # Calculate total playing minutes for each player and add to dataframe
    if additional_cols is None:
        included_cols = lineups.groupby(["name", "position", "team"], axis=0).sum(
            numeric_only=True
        )["mins_played"]
    else:
        included_cols = lineups.groupby(["name", "position", "team"], axis=0).sum(
            numeric_only=True
        )[["mins_played"] + additional_cols]

    playerinfo_df = playerinfo_df.merge(
        included_cols, left_on=["name", "position", "team"], right_index=True
    )

    # Sum minutes played in each position
    playerinfo_df["tot_mins_played"] = playerinfo_df.groupby(["name", "team"])[
        "mins_played"
    ].transform("sum")
    if additional_cols is not None:
        for col in additional_cols:
            playerinfo_df["tot_" + col] = playerinfo_df.groupby(["name", "team"])[
                col
            ].transform("sum")
    ...
    playerinfo_df = playerinfo_df[
        ~playerinfo_df[["name", "team"]].duplicated(keep="first")
    ]
    ...
    # Add position type
    playerinfo_df["pos_type"] = playerinfo_df["position"].apply(
        lambda x: "DEF"
        if x in ["DC", "DL", "DR", "DMR", "DML"]
        else (
            "MID"
            if x in ["AML", "AMR", "AMC", "DM", "DMC", "MC", "ML", "MR"]
            else (
                "FWD" if x in ["FW", "FWL", "FWR"] else "GK" if x in ["GK"] else "SUB"
            )
        )
    )
...
def group_player_events(
    events, player_data, group_type="count", agg_columns=None, col_names="Column Name"
):
    ...
    player_data_out = player_data_out.merge(
        selected_events, left_on="playerId", right_index=True, how="left"
    )
    ...
    if agg_columns != list() and group_type in ["sum", "mean"]:
        for idx in np.arange(len(agg_columns)):
            player_data_out = player_data_out.rename(
                columns={agg_columns[idx]: col_names[idx]}
            )
