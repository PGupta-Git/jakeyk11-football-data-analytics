...
def pre_assist(events_df):
    ...
    for idx, assist_event in events_out[
        events_out["satisfiedEventsTypes"].apply(lambda x: 92 in x if x == x else False)
    ].iterrows():
        ...
        while loop:
            ...
            if events_out.iloc[scan_idx]["teamId"] != assist_team:
                loop = False
            if (
                events_out.iloc[scan_idx]["period"] == period_number
...
def progressive_action(single_event, inplay=True, successful_only=True):
    ...
    if single_event["eventType"] in ["Carry", "Pass"]:
        ...
        if inplay:
            check_inplay = not any(
                item in single_event["satisfiedEventsTypes"]
                for item in [48, 50, 51, 42, 44, 45, 31, 34, 212]
            )
        else:
            check_inplay = True
        ...
        delta_goal_dist = np.sqrt(
            (120 - x_startpos) ** 2 + (40 - y_startpos) ** 2
        ...
        if (
            (check_success and check_inplay)
            and (x_startpos < 60 and x_endpos < 60)
            and delta_goal_dist >= 32.8
        ):
            return True
...
def box_entry(single_event, inplay=True, successful_only=True):
    ...
    if single_event["eventType"] in ["Pass", "Carry"]:
        ...
        if (
            (check_success and check_inplay)
            and (x_position_end >= 83)
            and (21.1 <= y_position_end <= 78.9)
            and ((x_position < 83) or ((y_position < 21.1) or (y_position > 78.9)))
        ):
            return True
        else:
            return float("nan")
...
def create_convex_hull(
    events_df, name="default", min_events=3, include_events="1std", pitch_area=10000
):
    """Create a dataframe of convex hull information from statsbomb-style event data.
    ...
    if len(events_df) >= min_events:
        ...
        hull_df.at[name, "hull_area_%"] = (
            100 * hull_df.loc[name, "hull_area"] / pitch_area
        )
            ...
            if pass_event["outcomeType"] == "Successful":
                ...
                if xt_info:
                    hull_df["suc_pass_into_hull"].append(
                        [pass_start_loc_flip, pass_end_loc_flip, pass_event["xThreat"]]
                    )
                    suc_into_hull_xt_net = np.nansum(
                        [suc_into_hull_xt_net, pass_event["xThreat"]]
                    )
                    suc_into_hull_xt_gen = np.nansum(
                        [
                            suc_into_hull_xt_gen,
                ...
                if xt_info:
                    ...
                    unsuc_into_hull_xt_net = np.nansum(
                        [unsuc_into_hull_xt_net, pass_event["xThreat"]]
                    )
                    unsuc_into_hull_xt_gen = np.nansum(
                        [
                            unsuc_into_hull_xt_gen,
                            0 if pass_event["xThreat"] < 0 else pass_event["xThreat"],
                        ]
                    )
                else:
...
def insert_ball_carries(
    ...
    for match_id in events_df["match_id"].unique():
            ...
            if idx < len(match_events) - 1:
                ...
                while incorrect_next_evt:
                    ...
                    if (
                        ...
                        (
                            next_evt["eventType"] == "TakeOn"
                            and next_evt["outcomeType"] == "Unsuccessful"
                        )
                        or (
                            next_evt["teamId"] != prev_evt_team
                            and next_evt["eventType"] == "Challenge"
                            and next_evt["outcomeType"] == "Unsuccessful"
                        )
                ...
                if valid_carry:
                    ...
                    carry["minute"] = np.floor(
                        (
                            (init_next_evt["minute"] * 60 + init_next_evt["second"])
                    ...
                    carry["expandedMinute"] = np.floor(
                        (
                            (
                                init_next_evt["expandedMinute"] * 60
                                + init_next_evt["second"]
                            )
                    ...
                    carry["type"] = carry.apply(
                        lambda x: {"value": 99, "displayName": "Carry"}, axis=1
                    )
                    carry["outcomeType"] = "Successful"
                    carry["qualifiers"] = carry.apply(
                        lambda x: {
                            "type": {"value": 999, "displayName": "takeOns"},
                            "value": str(take_ons),
                        },
                        axis=1,
                    )
                    ...
                    carry["cumulative_mins"] = (
                        prev["cumulative_mins"] + init_next_evt["cumulative_mins"]
                    ) / 2
        ...
        match_events_and_carries = match_events_and_carries.sort_values(
            ["match_id", "period", "cumulative_mins"]
...
def get_xthreat(events_df, interpolate=True, pitch_length=100, pitch_width=100):
    ...
    def get_cell_indexes(
        x_series, y_series, cell_cnt_l, cell_cnt_w, field_length, field_width
    ):
    ...
    move_actions = events_df[
        (events_df["eventType"].isin(["Carry", "Pass"]))
    ...
    if interpolate:
        ...
        interpolator = interp2d(
            x=x, y=y, z=xt_grid.values, kind="linear", bounds_error=False
        )
    ...
    for match_id in move_actions["match_id"].unique():
        ...
        startxc, startyc = get_cell_indexes(
            match_move_actions["x"],
            match_move_actions["y"],
            cell_count_l,
            cell_count_w,
            pitch_length,
            pitch_width,
        )
        endxc, endyc = get_cell_indexes(
            match_move_actions["endX"],
            match_move_actions["endY"],
            cell_count_l,
            cell_count_w,
            pitch_length,
            pitch_width,
        ...
        ratings = pd.DataFrame(
            data=xt_end - xt_start, index=match_move_actions.index, columns=["xThreat"]
        )

        # Merge ratings dataframe to all match events
        match_events_and_ratings = pd.merge(
            left=events_df[events_df["match_id"] == match_id],
            right=ratings,
            how="left",
            left_index=True,
            right_index=True,
        ...
        events_out = pd.concat(
            [events_out, match_events_and_ratings], ignore_index=True, sort=False
        )
        events_out["xThreat_gen"] = events_out["xThreat"].apply(
...
def find_offensive_actions(events_df):
    ...
    offensive_aerials = aerials[
        aerials["qualifiers"].apply(
            lambda x: {"type": {"value": 286, "displayName": "Offensive"}} in x
        )
    ]
    ...
    offensive_action_df = events_df[
        (events_df["eventType"].isin(offensive_actions))
        & (
            events_df["satisfiedEventsTypes"].apply(
                lambda x: not (5 in x or 6 in x or 31 in x or 34 in x or 212 in x)
            )
        )
    ]
    offensive_action_df = pd.concat(
        [offensive_action_df, offensive_aerials], axis=0
    ).sort_values(["match_id", "cumulative_mins"])
...
def find_defensive_actions(events_df):
    ...
    # Defensive Aerials
    aerials = events_df[events_df["eventType"] == "Aerial"]
    defensive_aerials = aerials[
        aerials["qualifiers"].apply(
            lambda x: {"type": {"value": 285, "displayName": "Defensive"}} in x
        )
    ]
    ...
    defensive_action_df = pd.concat(
        [defensive_action_df, defensive_aerials], axis=0
    ).sort_values(["match_id", "cumulative_mins"])
...
def get_pass_outcome(pass_events, contextual_events, t=5):
    ...
    for idx, pass_evt in pass_events_out.iterrows():
        ...
        elif ("Goal" in team_next_evts["eventType"].tolist()) or (
            92 in pass_evt["satisfiedEventsTypes"]
        ):
            pass_events_out.loc[idx, "pass_outcome"] = "Goal"

        # Look for shots in next t seconds
        elif (
            ("SavedShot" in team_next_evts["eventType"].tolist())
            or ("ShotOnPost" in team_next_evts["eventType"].tolist())
            or ("MissedShots" in team_next_evts["eventType"].tolist())
        ):
        ...
        # Look for key pass events in next t seconds
            ...
            set(
                [
                    item
                    for sublist in next_evts["satisfiedEventsTypes"].tolist()
                    for item in sublist
                ]
            )
            & set(np.arange(39, 47))
...
def get_possession_chains(events_df, chain_check=5, suc_evts_in_chain=3):
    ...
    for match_id in events_df["match_id"].unique():
        ...
        match_pos_events_df = match_events_df[
            ~match_events_df["eventType"].isin(
                [
                    "OffsideGiven",
                    "CornerAwarded",
                    "Start",
        ...
        match_pos_events_df["outcomeBinary"] = match_pos_events_df["outcomeType"].apply(
            lambda x: 1 if x == "Successful" else 0
        )
        match_pos_events_df["teamBinary"] = match_pos_events_df["teamId"].apply(
            lambda x: 1 if x == min(match_pos_events_df["teamId"]) else 0
        )
        match_pos_events_df["goalBinary"] = (
            (match_pos_events_df["eventType"] == "Goal")
            .astype(int)
            .diff(periods=1)
        ...
        for n in np.arange(1, chain_check):
            ...
            pos_chain_df[f"evt_{n}_same_team"] = pos_chain_df[
                f"evt_{n}_same_team"
        ...
        pos_chain_df["enough_evt_same_team"] = pos_chain_df[
            "enough_evt_same_team"
        ].diff(periods=1)
        ...
        for ko in match_pos_events_df[
            (match_pos_events_df["goalBinary"] == 1)
            | (match_pos_events_df["period"].diff(periods=1))
        ].index.values:
        ...
        pos_chain_df.loc[
            pos_chain_df["kick_off_period_change"] == 1, "valid_pos_start"
        ] = 1
        ...
        pos_chain_df.loc[pos_chain_df.head(1).index, "possession_team"] = (
            pos_chain_df.loc[pos_chain_df.head(1).index, "teamId"]
        )
        ...
        for idx in np.arange(1, len(valid_pos_start_id)):
            ...
            if (
                ...
                & (pos_chain_df.loc[valid_pos_start_id[idx], "kick_off_goal"] != 1)
                & (
                    pos_chain_df.loc[valid_pos_start_id[idx], "kick_off_period_change"]
                    != 1
                )
            ):
                pos_chain_df.loc[valid_pos_start_id[idx], "possession_id"] = np.nan
            else:
                pos_chain_df.loc[valid_pos_start_id[idx], "possession_id"] = (
                    possession_id
                )
                pos_chain_df.loc[valid_pos_start_id[idx], "possession_team"] = (
                    pos_chain_df.loc[valid_pos_start_id[idx], "teamId"]
                )
                possession_id += 1

        # Assign possession id and team back to events dataframe
        match_events_df = pd.merge(
            match_events_df,
        ...
        match_events_df[["possession_id", "possession_team"]] = match_events_df[
