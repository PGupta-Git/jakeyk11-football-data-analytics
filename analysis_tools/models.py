...
def get_pass_clusters(events, data_mode="whoscored"):
    ...
    os.chdir(
        current_dir.split("football-data-analytics")[0]
    ...
    passes_out["pass_cluster_mean_x"] = passes_out["pass_cluster_id"].apply(
        lambda x: cluster_centers[x, 0]
    )
    passes_out["pass_cluster_mean_y"] = passes_out["pass_cluster_id"].apply(
        lambda x: cluster_centers[x, 1]
    )
    passes_out["pass_cluster_mean_end_x"] = passes_out["pass_cluster_id"].apply(
        lambda x: cluster_centers[x, 2]
    )
    passes_out["pass_cluster_mean_end_y"] = passes_out["pass_cluster_id"].apply(
        lambda x: cluster_centers[x, 3]
    )

    # Return data to standard state based on data_mode
    if data_mode == "whoscored":
        passes_out["pass_cluster_mean_x"] = (
            100 * passes_out["pass_cluster_mean_x"] / 120
        )
        passes_out["pass_cluster_mean_y"] = 100 * passes_out["pass_cluster_mean_y"] / 80
        passes_out["pass_cluster_mean_end_x"] = (
            100 * passes_out["pass_cluster_mean_end_x"] / 120
        )
        passes_out["pass_cluster_mean_end_y"] = (
            100 * passes_out["pass_cluster_mean_end_y"] / 80
        )
...
def simulate_match_outcome(events, matches, match_id, sim_count=10000):
    ...
    match_xg_events = events[
        (events["match_id"] == match_id)
        & (events["shot_statsbomb_xg"] == events["shot_statsbomb_xg"])
    ]
    home_xg_list = match_xg_events[
        match_xg_events["team_name"] == match_simulate["home_team"].values[0]
    ]["shot_statsbomb_xg"].values
    away_xg_list = match_xg_events[
        match_xg_events["team_name"] == match_simulate["away_team"].values[0]
    ]["shot_statsbomb_xg"].values
    ...
    match_simulation_results = pd.DataFrame(
        zip(home_goal_list, away_goal_list, outcome_list),
    ...
    result_dict["home_xpoints"] = (
        result_dict["home_win_probability"] * 3 + result_dict["draw_probability"] * 1
    )
    result_dict["away_xpoints"] = (
        result_dict["away_win_probability"] * 3 + result_dict["draw_probability"] * 1
    )
    ...
    if "home_xpoints" in matches.columns:
        ...
        matches_out.loc[
            matches["match_id"] == match_id, list(result_dict.keys())[1:]
        ] = list(result_dict.values())[1:]
    else:
        join_df = pd.DataFrame(result_dict, index=[0])
        matches_out = pd.merge(
            matches, join_df, left_on="match_id", right_on="match_id", how="left"
        )
