...
def tag_in_play(events):
    ...
    events_out.loc[
        (events_out["type_name"] == "Goal Keeper")
        & (
            ~events_out["sub_type_name"].isin(
                ["Penalty Conceded", "Penalty Saved", "Penalty Saved To Post"]
            )
        ),
        "in_play_event",
    ] = 1
    events_out.loc[
        (events_out["type_name"] == "Pass")
        & (
            ~events_out["sub_type_name"].isin(
    ...
    events_out.loc[
        (events_out["type_name"] == "Shot")
        & (events_out["sub_type_name"] == "Open Play"),
        "in_play_event",
    ] = 1
...
def pre_assist(events):
    ...
    for idx, assist_event in events_out[
        events_out["pass_goal_assist"] == True
    ].iterrows():
        ...
        while loop:
            if events_out.loc[scan_idx, "possession"] != possession_number:
                loop = False
            if (
                events_out.loc[scan_idx, "possession"] == possession_number
...
def xg_assisted(events):
    ...
    for idx, assist_event in events_out[
        events_out["pass_shot_assist"] == True
    ].iterrows():
        events_out.loc[idx, "xg_assisted"] = events_out[
            events_out["id"] == assist_event["pass_assisted_shot_id"]
...
def istouch(single_event):
    ...
    if single_event["type_name"] == "50/50":
        ...
        if single_event["outcome_name"] in [
            "Won",
            "Success To Team",
    ...
    elif single_event["type_name"] == "Ball Receipt" and (
        single_event["outcome_name"] != single_event["outcome_name"]
    ):
        touch_type = "Offensive"
        touch_success = 1
    ...
    # Successful recovery
        ...
        if "ball_recovery_offensive" in single_event.index.tolist():
            if (
                single_event["ball_recovery_offensive"]
                == single_event["ball_recovery_offensive"]
            ):
                touch_type = "Offensive"
        else:
        ...
        if "ball_recovery_recovery_failure" in single_event.index.tolist():
            if (
                single_event["ball_recovery_recovery_failure"]
                != single_event["ball_recovery_recovery_failure"]
            ):
                touch_success = 1
        else:
    ...
    # Interception
        ...
        single_event["type_name"] == "Interception"
        and single_event["outcome_name"] != "Lost"
    ):
        touch_type = "Defensive"
        if single_event["outcome_name"] in [
            "Won",
    ...
    # Pass
        ...
        single_event["type_name"] == "Pass"
        and single_event["body_part_name"] != "No Touch"
    ):
        touch_type = "Offensive"
...
def box_entry(events, inplay=True, successful_only=True):
    ...
    box_entries = events[
        (events["type_name"].isin(["Pass", "Carry"]))
    ...
    if successful_only:
        box_entries = box_entries[
            box_entries["outcome_name"] != box_entries["outcome_name"]
        ]
...
def progressive_action(events, inplay=True, successful_only=True):
    ...
    prog_action["delta_goal_dist"] = np.sqrt(
        (120 - prog_action["x"]) ** 2 + (40 - prog_action["y"]) ** 2
    ...
    prog_action = prog_action[
        (
            (prog_action["x"] < 60)
            & (prog_action["end_x"] < 60)
            & (prog_action["delta_goal_dist"] >= 32.8)
        )
        | (
            (prog_action["x"] < 60)
            & (prog_action["end_x"] >= 60)
            & (prog_action["delta_goal_dist"] >= 16.4)
        )
        | (
            (prog_action["x"] >= 60)
            & (prog_action["end_x"] >= 60)
            & (prog_action["delta_goal_dist"] >= 10.94)
        )
    ]

    if successful_only:
        prog_action = prog_action[
            prog_action["outcome_name"] != prog_action["outcome_name"]
        ]
...
def half_space_actions(events, inplay=True, successful_only=True):
    ...
    if successful_only:
        hs_s_action = hs_s_action[
            hs_s_action["outcome_name"] != hs_s_action["outcome_name"]
        ]
        hs_e_action = hs_e_action[
            hs_e_action["outcome_name"] != hs_e_action["outcome_name"]
        ]
    if inplay:
...
def pre_shot_evts(events, t=5):
    ...
    for idx, shot in all_shots.iterrows():
        ...
        previous_passes_carries = previous_events[
            (
                (previous_events["type"] == "Pass")
...
def create_convex_hull(
    events, name="default", include_events="1std", min_events=3, pitch_area=9600
):
    """Create a dataframe of convex hull information from statsbomb-style event data.
    ...
    if len(events) >= min_events:
        ...
        hull_data["x_from_mean"] = (
            hull_data["x_position"] - hull_data["x_position"].mean()
        )
        hull_data["y_from_mean"] = (
            hull_data["y_position"] - hull_data["y_position"].mean()
        )
        hull_data["dist_from_mean"] = np.sqrt(
            hull_data["x_from_mean"] ** 2 + hull_data["y_from_mean"] ** 2
        )
        ...
        if "std" in str(include_events):
            ...
            sqrt_variance = np.sqrt(
                sum(hull_data["dist_from_mean"] ** 2)
                / (len(hull_data["dist_from_mean"]) - 1)
            )
            reduced_hull_data = hull_data[
                hull_data["dist_from_mean"] <= sqrt_variance * num_stds
            ]
        else:
            reduced_hull_data = hull_data.head(
                int(np.ceil(hull_data.shape[0] * include_events / 100))
            )

        # Build list of hull points and a convex hull dataframe
        hull_pts = list(
            zip(reduced_hull_data["x_position"], reduced_hull_data["y_position"])
        )
        hull_df.at[name, "hull_x"] = list(hull_data["x_position"].values)
        hull_df.at[name, "hull_reduced_x"] = list(
            reduced_hull_data["x_position"].values
        )
        hull_df.at[name, "hull_y"] = list(hull_data["y_position"].values)
        hull_df.at[name, "hull_reduced_y"] = list(
            reduced_hull_data["y_position"].values
        )

        # Calculate and store convex hull centre, area and perimeter
        hull_df.at[name, "hull_centre"] = (
            reduced_hull_data["x_position"].mean(),
        ...
        hull_df.at[name, "hull_area_%"] = round(
            100 * hull_df.loc[name, "hull_area"] / pitch_area, 2
        )
...
def passes_into_hull(hull_info, events, opp_passes=True, obv_info=False):
    ...
    for _, pass_event in events_to_check.iterrows():
        ...
        if opp_passes is True:
            ...
            pass_end_loc_flip = list(
                np.subtract([120, 80], pass_event["pass_end_location"])
            )
        else:
        ...
        if in_hull(pass_end_loc_flip, hull_pts):
            ...
            if pass_event["pass_outcome"] != pass_event["pass_outcome"]:
                ...
                if obv_info:
                    hull_df["suc_pass_into_hull"].append(
                        [
                            pass_start_loc_flip,
                            pass_end_loc_flip,
                            pass_event["obv_against_net"],
                            pass_event["obv_for_net"],
                        ]
                    )
                    suc_into_hull_obvfor = np.nansum(
                        [suc_into_hull_obvfor, pass_event["obv_for_net"]]
                    )
                    suc_into_hull_obvagainst = np.nansum(
                        [suc_into_hull_obvagainst, pass_event["obv_against_net"]]
                    )
                    suc_into_hull_obvtot = np.nansum(
                        [suc_into_hull_obvtot, pass_event["obv_total_net"]]
                    )
                else:
                    hull_df["suc_pass_into_hull"].append(
                        [pass_start_loc_flip, pass_end_loc_flip]
                    )

            else:
                unsuc_into_hull_count += 1
                if obv_info:
                    hull_df["unsuc_pass_into_hull"].append(
                        [
                            pass_start_loc_flip,
                            pass_end_loc_flip,
                            pass_event["obv_against_net"],
                    ...
                    unsuc_into_hull_obvfor = np.nansum(
                        [unsuc_into_hull_obvfor, pass_event["obv_for_net"]]
                    )
                    unsuc_into_hull_obvagainst = np.nansum(
                        [unsuc_into_hull_obvagainst, pass_event["obv_against_net"]]
                    )
                    unsuc_into_hull_obvtot = np.nansum(
                        [unsuc_into_hull_obvtot, pass_event["obv_total_net"]]
                    )
                else:
                    hull_df["unsuc_pass_into_hull"].append(
                        [pass_start_loc_flip, pass_end_loc_flip]
    ...
    hull_df["pct_tot_pass_into_hull"] = round(
        100 * (suc_into_hull_count + unsuc_into_hull_count) / len(events_to_check), 2
    )
    hull_df["hull_pass_prevented_%"] = round(
        100 * unsuc_into_hull_count / (suc_into_hull_count + unsuc_into_hull_count), 2
    )
    if obv_info:
...
def defensive_line_positions(events, team, include_events="1std"):
    ...
    for match_id in events["match_id"].unique():
        ...
        cb_actions = defensive_actions_df[
            (defensive_actions_df["team"] == team)
            & (
                defensive_actions_df["position"].isin(
                    ["Center Back", "Left Center Back", "Right Center Back"]
                )
            )
        ]
        left_def_actions = defensive_actions_df[
            (defensive_actions_df["team"] == team)
            & (
                defensive_actions_df["position"].isin(
                    ["Left Back", "Left Midfield", "Left Wing Back", "Left Wing"]
                )
            )
        ]
        right_def_actions = defensive_actions_df[
            (defensive_actions_df["team"] == team)
            & (
                defensive_actions_df["position"].isin(
                    ["Right Back", "Right Midfield", "Right Wing Back", "Right Wing"]
                )
            )
        ]
        pressures = match_events[
            (match_events["team"] == team) & (match_events["type"] == "Pressure")
        ]

        # Calculate offsides from opposition team
        general_offsides = match_events[
            (match_events["team"] != team) & (match_events["type"] == "Offside")
        ]
        pass_offsides = match_events[
            (match_events["team"] != team)
            & (match_events["pass_outcome"] == "Pass Offside")
        ]

        # Create list of heights of defensive line actions for match
        general_offside_heights = [
            120 - x[0] for x in general_offsides["location"].values
        ]
        pass_offside_heights = [
            120 - x[0] for x in pass_offsides["pass_end_location"].values
        ]
        cb_action_heights = [x[0] for x in cb_actions["location"].values]
        def_line_event_height = (
            general_offside_heights + pass_offside_heights + cb_action_heights
        )
    ...
    if "std" in str(include_events):
        num_stds = float(include_events.split("std")[0])
        sqrt_variance_dh = np.sqrt(
            sum((def_line_event_heights - np.mean(def_line_event_heights)) ** 2)
            / (len(def_line_event_heights) - 1)
        )
        def_line_event_heights = np.array(def_line_event_heights)[
            abs(def_line_event_heights - np.mean(def_line_event_heights))
            <= sqrt_variance_dh * num_stds
        ]
        sqrt_variance_p = np.sqrt(
            sum((pressure_heights - np.mean(pressure_heights)) ** 2)
            / (len(pressure_heights) - 1)
        )
        pressure_heights = np.array(pressure_heights)[
            abs(pressure_heights - np.mean(pressure_heights))
            <= sqrt_variance_p * num_stds
        ]
        sqrt_variance_lw = np.sqrt(
            sum((left_def_widths - np.mean(left_def_widths)) ** 2)
            / (len(left_def_widths) - 1)
        )
        left_def_widths = np.array(left_def_widths)[
            abs(left_def_widths - np.mean(left_def_widths))
            <= sqrt_variance_lw * num_stds
        ]
        sqrt_variance_rw = np.sqrt(
            sum((right_def_widths - np.mean(right_def_widths)) ** 2)
            / (len(right_def_widths) - 1)
        )
        right_def_widths = np.array(right_def_widths)[
            abs(right_def_widths - np.mean(right_def_widths))
            <= sqrt_variance_rw * num_stds
        ]

    else:
        def_line_event_heights = np.sort(def_line_event_heights)[
            0 : int((include_events / 100) * len(def_line_event_heights))
        ]
        pressure_heights = np.sort(pressure_heights)[
            0 : int((include_events / 100) * len(pressure_heights))
        ]
        left_def_widths = np.sort(left_def_widths)[
            0 : int((include_events / 100) * len(left_def_widths))
        ]
        right_def_widths = np.sort(right_def_widths)[
            0 : int((include_events / 100) * len(right_def_widths))
        ]

    return (
        np.median(def_line_event_heights),
        np.median(pressure_heights),
        np.median(left_def_widths),
        np.median(right_def_widths),
    )
...
def long_ball_retention(events, player_name, player_team):
    ...
    long_ball_to_player = to_player[
        (
            (to_player["pass_length"] > 21.87)
            & (to_player["pass_height"].isin(["Low Pass", "High Pass"]))
        )
        | (
            (to_player["pass_length"] > 32.8)
            & (to_player["pass_height"] == "Ground Pass")
        )
    ]
    ...
    long_ball_received = pd.DataFrame(
        columns=[
            "match_id",
            "match_period",
            "long_ball_matchtime",
    ...
    for idx, long_ball_evt in long_ball_to_player.iterrows():
        ...
        evt_match, evt_period, evt_time, evt_index = long_ball_evt[
            ["match_id", "period", "cumulative_mins", "index"]
        ]

        # Obtain the following 20s worth of events
        following_evts = events_out[
            (events_out["match_id"] == evt_match)
            & (events_out["period"] == evt_period)
            & (events_out["cumulative_mins"] >= evt_time)
            & (events_out["cumulative_mins"] <= evt_time + 1 / 3)
            & (events_out["index"] >= evt_index)
        ].sort_values("index")

        # Get player ball receipt (first instance)
        ball_receipt = following_evts[
            (following_evts["type"] == "Ball Receipt*")
            & (following_evts["player"] == player_name)
        ].head(1)
        ...
        if len(ball_receipt) == 1:
            ...
            if receipt_outcome != receipt_outcome:
                ...
                player_immed_evt = following_evts[
                    (following_evts["player"] == player_name)
                    & (following_evts["cumulative_mins"] == receipt_time)
                    & (following_evts["type"] != "Ball Receipt*")
                ].head(1)
                player_next_evt = following_evts[
                    (following_evts["player"] == player_name)
                    & (following_evts["cumulative_mins"] > receipt_time)
                ].head(1)
                ...
                if len(player_immed_evt) == 1:
                    immed_header = (
                        True
                        if (
                            (player_immed_evt["pass_body_part"].values[0] == "Head")
                            and (long_ball_evt["pass_height"] == "High")
                        )
                        else False
                    )
                    ...
                    if player_immed_evt["type"].values[0] == "Pass":
                        ...
                        player_next_evt_success = (
                            True
                            if (
                                player_immed_evt["pass_outcome"].values[0]
                                != player_immed_evt["pass_outcome"].values[0]
                            )
                            else False
                        )
                        [player_next_evt_endx, player_next_evt_endy] = player_immed_evt[
                            "pass_end_location"
                        ].values[0]
                        player_next_evt_time = player_immed_evt[
                            "cumulative_mins"
                        ].values[0]
                        ...
                        player_next_evt_success = (
                            True
                            if (
                                player_immed_evt["shot_outcome"].values[0]
                                in ["Saved", "Goal", "Saved To Pos"]
                            )
                            else False
                        )
                        [player_next_evt_endx, player_next_evt_endy] = player_immed_evt[
                        ...
                        if len(player_next_evt) == 1:
                            ...
                            if player_next_evt["type"].values[0] == "Pass":
                                ...
                                player_next_evt_success = (
                                    True
                                    if (
                                        player_next_evt["pass_outcome"].values[0]
                                        != player_next_evt["pass_outcome"].values[0]
                                    )
                                    else False
                                )
                                ...
                                player_next_evt_time = player_next_evt[
                                    "cumulative_mins"
                                ].values[0]
                                ...
                                player_next_evt_success = (
                                    True
                                    if (
                                        player_next_evt["shot_outcome"].values[0]
                                        in ["Saved", "Goal", "Saved To Pos"]
                                    )
                                    else False
                                )
                            ...
                            # Free kick win
                                ...
                                [player_next_evt_endx, player_next_evt_endy] = (
                                    player_next_evt["location"].values[0]
                                )
                                player_next_evt_time = player_next_evt[
                                    "cumulative_mins"
                                ].values[0]
                            ...
                            # Dribble
                                ...
                                player_next_evt_success = (
                                    True
                                    if player_next_evt["dribble_outcome"].values[0]
                                    == "Complete"
                                    else False
                                )
                                [player_next_evt_endx, player_next_evt_endy] = (
                ...
                # Account for occasions where there is no interim carry and a pass/shot is made after ball receipt
                    ...
                    if player_next_evt["type"].values[0] == "Pass":
                        ...
                        [player_next_evt_endx, player_next_evt_endy] = player_next_evt[
                            "pass_end_location"
                        ].values[0]
                        player_next_evt_time = player_next_evt[
                            "cumulative_mins"
                ...
                possession_team = (
                    following_evts[
                        (following_evts["cumulative_mins"] <= receipt_time + (1 / 6))
                    ]
                    .iloc[-1:]["possession_team"]
                    .values[0]
                )
                ...
                if not immed_header:
                    ...
                    long_ball_received.loc[idx, "long_ball_matchtime"] = long_ball_evt[
                        "timestamp"
                    ]
                    long_ball_received.loc[idx, ["pass_x", "pass_y"]] = long_ball_evt[
                        "location"
                    ]
                    long_ball_received.loc[idx, ["receipt_x", "receipt_y"]] = (
                        long_ball_evt["pass_end_location"]
                    )
                    long_ball_received.loc[idx, "receipt_under_pressure"] = (
                        ball_receipt["under_pressure"].values[0]
                    )
                    ...
                    if len(player_initial_carry) == 1:
                        ...
                        long_ball_received.loc[idx, "carry_under_pressure"] = (
                            True
                            if (
                                player_initial_carry["under_pressure"].values[0] == True
                            )
                            else np.nan
                        )
                        long_ball_received.loc[
                            idx, ["init_carry_endx", "init_carry_endy"]
                        ] = player_initial_carry["carry_end_location"].values[0]
                    long_ball_received.loc[idx, "next_action"] = player_next_evt_type
                    long_ball_received.loc[idx, "next_action_success"] = (
                        player_next_evt_success
                    )
                    long_ball_received.loc[
                        idx, ["next_action_endx", "next_action_endy"]
                    ] = [player_next_evt_endx, player_next_evt_endy]
                    long_ball_received.loc[idx, "t_next_action"] = 60 * (
                        player_next_evt_time - receipt_time
                    )
                    long_ball_received.loc[idx, "long_ball_success"] = (
                        True
                        if (possession_team == player_team and miscontrol != miscontrol)
                        else np.nan
                    )
...
def analyse_ball_receipts(analysis_events, contextual_events, player_id=np.nan):
    ...
    if player_id == player_id:
        ball_to_player = analysis_events[
            analysis_events["pass_recipient_id"] == player_id
        ].copy()
    else:
    ...
    contextual_events = contextual_events[
        (
            contextual_events["match_id"].isin(
                ball_to_player["match_id"].unique().tolist()
            )
        )
    ]
    ...
    for idx, ball_evt in ball_to_player.iterrows():
        ...
        # Obtain the following 20s worth of events
        following_evts = contextual_events[
            (contextual_events["match_id"] == evt_match)
            ...
            & (contextual_events["index"] >= evt_index)
        ].sort_values("index")

        # Get player ball receipt (first instance)
        ball_receipt = following_evts[
            (following_evts["type_name"] == "Ball Receipt")
            & (following_evts["player_id"] == evt_player)
        ].head(1)
            ...
            if receipt_outcome is None:
                ...
                if len(player_immed_evt) == 1:
                    ...
                    if player_immed_evt["type_name"].values[0] in ["Pass", "Shot"]:
                        ...
                        player_next_evt_body_part = player_immed_evt[
                            "body_part_name"
                        ].values[0]
                        player_next_evt_success = (
                            1
                            if (
                                (player_immed_evt["outcome_name"].values[0] is None)
                                or (
                                    player_immed_evt["outcome_name"].values[0]
                                    in ["Saved", "Goal", "Saved To Post"]
                                )
                            )
                            else np.nan
                        )
                        ...
                        player_next_evt_obv_for_net = player_immed_evt[
                            "obv_for_net"
                        ].values[0]
                        player_next_evt_obv_for_net_z = player_immed_evt[
                            "obv_for_net_z"
                        ].values[0]
                    ...
                    # A carry event is an interim event that should be accounted for. We then look for following events
                        ...
                        if len(player_next_evt) == 1:
                            # Pass
                            if player_next_evt["type_name"].values[0] in [
                                "Pass",
                                "Shot",
                                "Dribble",
                            ]:
                                player_next_evt_type = player_next_evt[
                                    "type_name"
                                ].values[0]
                                player_next_evt_body_part = player_next_evt[
                                    "body_part_name"
                                ].values[0]
                                player_next_evt_success = (
                                    1
                                    if (
                                        (
                                            player_next_evt["outcome_name"].values[0]
                                            is None
                                        )
                                        or (
                                            player_next_evt["outcome_name"].values[0]
                                ...
                                if player_next_evt_type == "Dribble":
                                    player_next_evt_endx = player_next_evt["x"].values[
                                        0
                                    ]
                                    player_next_evt_endy = player_next_evt["y"].values[
                                        0
                                    ]
                                else:
                                    player_next_evt_endx = player_next_evt[
                                        "end_x"
                                    ].values[0]
                                    player_next_evt_endy = player_next_evt[
                                        "end_y"
                                    ].values[0]
                                player_next_evt_time = player_next_evt[
                                    "cumulative_mins"
                                ].values[0]
                                player_next_evt_obv_for_net = np.nansum(
                                    [
                                        player_next_evt["obv_for_net"].values[0],
                                        player_initial_carry["obv_for_net"].values[0],
                                    ]
                                )
                                player_next_evt_obv_for_net_z = (
                                    0
                                    if player_next_evt_obv_for_net < 0
                            ...
                            # Dispossession
                                ...
                                player_next_evt["type_name"].values[0] == "Dispossessed"
                            ):
                                player_next_evt_type = player_next_evt[
                                    "type_name"
                                ].values[0]
                ...
                # Account for occasions where there is no interim carry and a pass/shot is made after ball receipt
                    ...
                    if player_next_evt["type_name"].values[0] in ["Pass", "Shot"]:
                        ...
                        player_next_evt_success = (
                            1
                            if (
                                (player_next_evt["outcome_name"].values[0] is None)
                                or (
                                    player_next_evt["outcome_name"].values[0]
                                    in ["Complete", "Saved", "Goal", "Saved To Post"]
                                )
                            )
                            else np.nan
                        )
                        ...
                        player_next_evt_obv_for_net = player_next_evt[
                            "obv_for_net"
                        ].values[0]
                        player_next_evt_obv_for_net_z = player_next_evt[
                            "obv_for_net_z"
                        ].values[0]
                ...
                goal = (
                    True
                    if True
                    in (following_evts["outcome_name"] == "Goal")
                    & (following_evts["team_id"] == evt_team)
                    else False
                )
                ...
                ball_received.loc[idx, "receipt_under_pressure"] = ball_receipt[
                    "under_pressure"
                ].values[0]
                ball_received.loc[idx, "receipt_miscontrol"] = miscontrol
                if len(player_initial_carry) == 1:
                    ball_received.loc[idx, "initial_carry"] = 1
                    ball_received.loc[idx, "carry_under_pressure"] = (
                        player_initial_carry["under_pressure"].values[0]
                    )
                    ball_received.loc[idx, "init_carry_endx"] = player_initial_carry[
                        "end_x"
                    ].values[0]
                    ball_received.loc[idx, "init_carry_endy"] = player_initial_carry[
                        "end_y"
                    ].values[0]
                ball_received.loc[idx, "next_action"] = player_next_evt_type
                ball_received.loc[idx, "next_action_body_part"] = (
                    player_next_evt_body_part
                )
                ...
                ball_received.loc[idx, "next_actions_obv_for_net"] = (
                    player_next_evt_obv_for_net
                )
                ball_received.loc[idx, "next_actions_obv_for_net_z"] = (
                    player_next_evt_obv_for_net_z
                )
                ball_received.loc[idx, "t_next_action"] = 60 * (
                    player_next_evt_time - receipt_time
                )
                ball_received.loc[idx, "team_retention"] = (
                    1
                    if (possession_team == evt_team and miscontrol != miscontrol)
                    or goal
                    else np.nan
                )
...
def find_offensive_actions(events, in_play=False):
    ...
    offensive_action_df = events[
        events["type_name"].isin(offensive_action_names)
    ].reset_index(drop=True)

    # Remove defensive foul won
    offensive_action_df = offensive_action_df.drop(
        offensive_action_df[
            (offensive_action_df["type_name"] == "Foul Won")
            & (offensive_action_df["foul_won_defensive"] == True)
        ].index
    )

    # Remove set piece information if 'in-play' = True
    if in_play:
        offensive_action_df = offensive_action_df[
            offensive_action_df["in_play_event"] == 1
        ]
...
def find_defensive_actions(events):
    ...
    defensive_action_df = events[
        events["type_name"].isin(defensive_action_names)
    ].reset_index(drop=True)

    # Remove offensive team block
    defensive_action_df = defensive_action_df.drop(
        defensive_action_df[
            (defensive_action_df["type_name"] == "Block")
            & (defensive_action_df["block_offensive"] == True)
        ].index
    )
    ...
    defensive_action_df = defensive_action_df.drop(
        defensive_action_df[
            (defensive_action_df["type_name"] == "Foul Won")
            & (defensive_action_df["foul_won_defensive"] != True)
        ].index
    )
...
def get_counterpressure_events(events_in, t=5):
    ...
    all_ball_loss = events[
        (events["type_name"] == "Dispossessed")
        | (
            (events["type_name"] == "Dribble")
            & (events["outcome_name"] == "Incomplete")
        )
        | (
            (events["type_name"] == "Pass")
            & (events["outcome_name"].isin(["Out", "Incomplete"]))
        )
    ]
    ...
    for idx, ball_loss in all_ball_loss.iterrows():
        next_evts = events[
            (events["match_id"] == ball_loss["match_id"])
            & (events["period"] == ball_loss["period"])
        ...
        while flag and chk_idx < len(next_evts):
            ...
            # Check for passes out of play by the team that did not just lose the ball
                ...
                (check_evt["team_name"] != ball_loss["team_name"])
                and (check_evt["type_name"] == "Pass")
                and (check_evt["outcome_name"] == "Out")
            ):
                all_ball_loss.loc[idx, "recovery_action"] = (
                    "Opposition " + check_evt["type_name"] + " Out"
                )
                all_ball_loss.loc[idx, "recovery_action_t"] = 60 * (
                    check_evt["cumulative_mins"] - ball_loss["cumulative_mins"]
                )
            ...
            # Check for backwards passes by the team that did not just lose the ball
                ...
                (check_evt["team_name"] != ball_loss["team_name"])
                and (check_evt["type_name"] == "Pass")
                and (abs(check_evt["pass_angle"]) > (3 / 4) * np.pi)
            ):
                all_ball_loss.loc[idx, "recovery_action"] = (
                    "Opposition " + check_evt["type_name"] + " Backward"
                )
                all_ball_loss.loc[idx, "recovery_action_t"] = 60 * (
                    check_evt["cumulative_mins"] - ball_loss["cumulative_mins"]
                )
...
def get_counterattack_events(events_in, t=10):
    ...
    ground_duels = def_events[
        (
            (def_events["type_name"] == "Duel")
            & (def_events["sub_type_name"] == "Tackle")
            & (def_events["outcome_name"].isin(["Won", "Success", "Success In Play"]))
        )
        | (def_events["type_name"] == "50/50")
    ]

    interceptions = def_events[
        (def_events["type_name"] == "Interception")
        & (
            def_events["outcome_name"].isin(
                ["Won", "Success", "Success In Play", "Success Out"]
            )
        )
    ]

    ball_recoveries = def_events[
        (def_events["type_name"] == "Ball Recovery")
    ...
    for idx, ball_win in ball_wins.iterrows():
        next_evts = events[
            (events["match_id"] == ball_win["match_id"])
        ...
        while flag and chk_idx < len(next_evts):
            ...
            next_evts = next_evts[
                ~((next_evts["type_name"] == "Carry") & (next_evts["duration"] < 3))
            ]
            check_evt = next_evts.iloc[chk_idx, :]

            # Check for passes, shots or carries completed by team that won the ball back, and add required information
            if (check_evt["team_name"] == ball_win["team_name"]) and (
                check_evt["type_name"] in ["Carry", "Pass", "Shot"]
            ):
                ...
                # Box entries tagged as successful
                    ...
                    (
                        (check_evt["x"] < 102)
                        or ((check_evt["y"] < 18) or (check_evt["y"] > 62))
                    )
                    and (check_evt["end_x"] >= 102)
                    and (check_evt["end_y"] >= 18)
                    and (check_evt["end_y"] <= 62)
                ):
...
def get_pass_outcome(pass_events, contextual_events, t=5):
    ...
    for idx, pass_evt in pass_events_out.iterrows():
        ...
        next_evts = contextual_events[
            (
                (contextual_events["match_id"] == pass_evt["match_id"])
                & (contextual_events["period"] == pass_evt["period"])
                & (contextual_events["cumulative_mins"] > pass_evt["cumulative_mins"])
                & (
                    contextual_events["cumulative_mins"]
                    <= pass_evt["cumulative_mins"] + (t / 60)
                )
            )
        ]
