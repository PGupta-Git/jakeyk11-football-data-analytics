...
for match_id in matches_df["match_id"].values:
    matches_df, match_simulation_df = mod.simulate_match_outcome(
        events_df, matches_df, match_id, sim_count=200000
    )
...
ax.plot(
    [0, 1], [1 - header_height, 1 - header_height], color="darkgrey", zorder=3, lw=0.5
)
ax.add_patch(
    Rectangle([0, 1 - header_height], 1, header_height - 0.002, color="#262625")
)

# Header titles
ax.text(
    0.04,
    0.998 - header_height / 2,
    "Team Name",
    ha="left",
    va="center_baseline",
    fontweight="bold",
...
for idx, table_team in leaguetable_df.iterrows():
    ...
    ab = AnnotationBbox(
        OffsetImage(team_logo, zoom=1 / len(leaguetable_df) + 0.01, resample=True),
        (0.075, row_centre),
        frameon=False,
    )
    ax.add_artist(ab)
    ...
    ax.text(
        0.535,
        row_centre,
        f"{table_team['expected_points']:.2f}",
        ha="center",
        va="top",
        color="w",
    )
    ...
    xga_delta_str = (
        "+" if table_team["xg_against"] > table_team["goals_against"] else ""
    )
    xga_col = "green" if xga_delta_str == "" else "indianred"
    xgd_delta_str = (
        "+" if table_team["xg_difference"] > table_team["goal_difference"] else ""
    )
    ...
    xpos_delta_str = (
        "+" if table_team["position"] > table_team["expected_position"] else ""
    )
    xpos_col = (
        "green"
        if table_team["expected_position"] < table_team["position"]
        else "w"
        if table_team["expected_position"] == table_team["position"]
        else "indianred"
    )
    ...
    ax.text(
        0.04,
        row_centre,
        f"{xpos_delta_str}{table_team['position'] - table_team['expected_position']}",
        ha="center",
        va="bottom",
        fontweight="bold",
        color=xpos_col,
    ...
    ax.text(
        0.575,
        row_centre,
        f"{xp_delta_str}{(table_team['expected_points'] - table_team['points']):.2f}",
        ha="center",
        va="bottom",
        color=xp_col,
        fontsize=7,
    ...
    ax.text(
        0.765,
        row_centre,
        f"{xg_delta_str}{(table_team['xg_for'] - table_team['goals_for']):.2f}",
        ha="center",
        va="bottom",
        color=xg_col,
        fontsize=7,
    ...
    ax.text(
        0.96,
        row_centre,
        f"{xga_delta_str}{(table_team['xg_against'] - table_team['goals_against']):.2f}",
        ha="center",
        va="bottom",
        color=xga_col,
        fontsize=7,
...
fig.text(
    0.5,
    0.045,
    f"Monte Carlo method implemented to model the probability of individual match outcomes based on shot events, with {sim_count} repetitions completed per match. Expected\n"
...
# Save fig
fig.savefig(
    f"justice_league/{data_grab[0][1].replace(' ', '-').lower()}-{data_grab[0][2]}-justice-league-{run_date}.png",
    dpi=300,
)
