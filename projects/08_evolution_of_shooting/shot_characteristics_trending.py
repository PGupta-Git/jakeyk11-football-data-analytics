...
all_shots = events_df[
    events_df["eventType"].isin(["Goal", "MissedShots", "SavedShot", "ShotOnPost"])
]
all_shots["x_distance"] = PITCH_LENGTH_X * (100 - all_shots["x"]) / 100
all_shots["y_distance"] = (
    PITCH_WIDTH_Y * (50 - all_shots["y"]) / 100
)  # -ve is to the left of the goal, +ve is to the right of the goal
all_shots["distance"] = np.sqrt(
    all_shots["x_distance"] ** 2 + all_shots["y_distance"] ** 2
)
all_shots["angle"] = (
    np.arctan2(all_shots["y_distance"], all_shots["x_distance"]) * 180 / np.pi
)

# %% Get ground inplay shots and headed shots

all_ip_shots = all_shots[
    ~all_shots["satisfiedEventsTypes"].apply(lambda x: 5 in x or 6 in x)
]
all_ip_ground_shots = all_ip_shots[
    ~all_ip_shots["satisfiedEventsTypes"].apply(lambda x: 14 in x)
]
...
for idx, league_entry in shot_characteristics_df.iterrows():
    ...
    season_headers = season_shots[
        season_shots["satisfiedEventsTypes"].apply(lambda x: 14 in x)
    ]
    season_foot_shots = season_shots[
        season_shots["satisfiedEventsTypes"].apply(lambda x: 12 in x or 13 in x)
    ]
    season_rfoot_shots = season_foot_shots[
        season_foot_shots["satisfiedEventsTypes"].apply(lambda x: 12 in x)
    ]
    season_lfoot_shots = season_foot_shots[
        season_foot_shots["satisfiedEventsTypes"].apply(lambda x: 13 in x)
    ]
    season_out_box_shots = season_foot_shots[
        season_foot_shots[["x", "y"]].apply(
            lambda x: x.x < 83 or x.y < 21.1 or x.y > 78.9, axis=1
        )
    ]

    # Average shot and goal count per match
    shot_characteristics_df.loc[idx, "total_ip_shots_match"] = len(season_shots) / len(
        match_ids
    )
    shot_characteristics_df.loc[idx, "total_ip_goals_match"] = len(
        season_shots[season_shots["eventType"] == "Goal"]
    ) / len(match_ids)

    # Average shot distance and angle
    shot_characteristics_df.loc[idx, "mean_ip_foot_shot_dist"] = season_foot_shots[
        "distance"
    ].mean()
    shot_characteristics_df.loc[idx, "mean_ip_foot_shot_angle"] = season_foot_shots[
        "angle"
    ].mean()

    # Proportion of shots by shot type
    shot_characteristics_df.loc[idx, "proportion_ip_oob_shots"] = (
        100 * len(season_out_box_shots) / len(season_foot_shots)
    )
    shot_characteristics_df.loc[idx, "proportion_ip_headers"] = (
        100 * len(season_headers) / len(season_shots)
    )
    shot_characteristics_df.loc[idx, "proportion_ip_foot_shots_left"] = (
        100 * len(season_lfoot_shots) / len(season_foot_shots)
    )

    # Goal rates
    shot_characteristics_df.loc[idx, "ip_foot_shot_goal_conv"] = (
        100
        * len(season_foot_shots[season_foot_shots["eventType"] == "Goal"])
        / len(season_foot_shots)
    )
    shot_characteristics_df.loc[idx, "ip_shot_goal_conv"] = (
        100 * len(season_shots[season_shots["eventType"] == "Goal"]) / len(season_shots)
    )
    ...
    for team in shooting_teams:
        ...
        shot_characteristics_df.loc[idx, f"{team}_team_gcr"] = (
            100
            * len(team_foot_shots[team_foot_shots["eventType"] == "Goal"])
            / len(team_foot_shots)
        )
        shot_characteristics_df.loc[idx, f"{team}_team_dist"] = team_foot_shots[
            "distance"
        ].mean()
        shot_characteristics_df.loc[idx, f"{team}_team_angle"] = team_foot_shots[
            "angle"
        ].mean()
        shot_characteristics_df.loc[idx, f"{team}_team_lf_shot"] = (
            100 * len(team_lfoot_shots) / len(team_foot_shots)
        )
...
CustomCmap = mpl.colors.LinearSegmentedColormap.from_list(
    "", ["#313332", "#47516B", "#848178", "#B2A66F", "#FDE636"]
)
...
for _, league_entry in shot_characteristics_df.head(12).iterrows():
    ...
    ax["pitch"][idx].set_title(
        f"{year}/{str(year + 1).replace('20', '', 1)}",
        loc="center",
        color="w",
        fontsize=10,
        pad=-2,
    ...
    # Add text
    ax["pitch"][idx].text(
        96, 57.5, "Mean Dist:", fontsize=8, fontweight="regular", color="w", zorder=3
    )
    ax["pitch"][idx].text(
        96,
        53,
        f"{round(league_entry['mean_ip_foot_shot_dist'], 1)} yds",
        fontsize=8,
        fontweight="bold",
        color="w",
        zorder=3,
...
# Add line
sns.lineplot(
    shot_characteristics_df, x="year", y="mean_ip_foot_shot_dist", color="coral", ax=ax
)
sns.regplot(
    shot_characteristics_df,
    x="year",
    y="mean_ip_foot_shot_dist",
    scatter=True,
    line_kws={"color": "mistyrose", "lw": 1, "ls": "--"},
    scatter_kws={"color": "coral", "s": 50},
    ax=ax,
)
sns.lineplot(
    shot_characteristics_df,
    x="year",
    y="ip_foot_shot_goal_conv",
    color="turquoise",
    ax=twin_ax,
)
sns.regplot(
    ...
    y="ip_foot_shot_goal_conv",
    scatter=True,
    line_kws={"color": "lightblue", "lw": 1, "ls": "--"},
    scatter_kws={"color": "turquoise", "s": 50},
    ax=twin_ax,
)
...
# Labels and ticks
twin_ax.set_yticks(
    np.linspace(twin_ax.get_yticks()[0], twin_ax.get_yticks()[-1], len(ax.get_yticks()))
)
ax.set_yticks(
    np.linspace(ax.get_yticks()[0], ax.get_yticks()[-1], len(twin_ax.get_yticks()))
)
ax.xaxis.set_ticks([year for year in shot_characteristics_df["year"]])
ax.set_xticklabels(
    [
        f"{str(year).replace('20', '', 1)}/{str(year + 1).replace('20', '', 1)}"
        for year in shot_characteristics_df["year"]
    ],
    rotation=270,
)
ax.set_xlabel("Season", fontsize=12, fontweight="bold", labelpad=10)
ax.set_ylabel(
    "Mean Shot Distance (yards)",
    fontsize=12,
    color="coral",
    fontweight="bold",
    labelpad=10,
...
# Add line
sns.lineplot(
    shot_characteristics_df, x="year", y="total_ip_shots_match", color="coral", ax=ax
)
sns.regplot(
    shot_characteristics_df,
    x="year",
    y="total_ip_shots_match",
    scatter=True,
    line_kws={"color": "mistyrose", "lw": 1, "ls": "--"},
    scatter_kws={"color": "coral", "s": 50},
    ax=ax,
...
sns.regplot(
    ...
    y="total_ip_goals_match",
    scatter=True,
    line_kws={"color": "paleturquoise", "lw": 1, "ls": "--"},
    scatter_kws={"color": "mediumaquamarine", "s": 50},
    ax=twin_ax,
)
...
ax.set_xlabel("Season", fontsize=12, fontweight="bold", labelpad=10)
ax.set_ylabel(
    "Mean Shots per Game", fontsize=12, color="coral", fontweight="bold", labelpad=10
)
twin_ax.set_ylabel(
    "Mean Goals per Game",
    fontsize=12,
...
sns.lineplot(
    shot_characteristics_df,
    x="year",
    y="proportion_ip_foot_shots_left",
    color="coral",
    ax=ax,
)
sns.regplot(
...
ax.set_xlabel("Season", fontsize=12, fontweight="bold", labelpad=10)
ax.set_ylabel(
    "% of Ground Shots with Left Foot",
    fontsize=12,
    color="coral",
    fontweight="bold",
    labelpad=10,
...
ax.set_xlabel("Season", fontsize=12, fontweight="bold", labelpad=10)
ax.set_ylabel(
    "Proportion of Shots Outside Box (%)",
    fontsize=12,
    color="mediumaquamarine",
    fontweight="bold",
    labelpad=10,
...
columns = list(
    filter(
        ...
        [
            col if stat_name in col or "year" in col else None
            for col in shot_characteristics_df.columns
        ],
    )
)
...
for team_col in df_to_plot.columns[1:]:
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(
        team_data["year"][mask], team_data[f"{team}_{stat_name}"][mask]
    )
...
ax = ax.reshape(-1)
plt.subplots_adjust(
    left=0.09, bottom=0.06, right=0.94, top=0.86, hspace=0.5, wspace=0.3
)
...
for team in pd.DataFrame.from_dict(trend_dict, orient="index").sort_values(0).index:
    ...
    team_data = pd.concat(
        [df_to_plot["year"], df_to_plot[f"{team}_{stat_name}"]], axis=1
    )
    ...
    # Plot team data
    sns.lineplot(
        df_to_plot, x="year", y=f"{team}_{stat_name}", color=team_color, ax=ax[idx]
    )
    sns.regplot(
        df_to_plot,
        x="year",
        y=f"{team}_{stat_name}",
        scatter=True,
        line_kws={"color": team_color, "alpha": 0.5, "lw": 1, "ls": "--"},
        scatter_kws={"color": team_color, "s": 25},
        ax=ax[idx],
    )
    ax[idx].text(
        2019,
        22.75,
        f"Mean dist. {'drops' if trend_dict[team] < 0 else 'rises'}\nby {abs(round(trend_dict[team], 2))} yds/yr",
        ha="center",
        color="w",
        fontsize=5.5,
    )
    ...
    ax[idx].xaxis.set_ticks(
        [year for year in shot_characteristics_df["year"] if year % 3 == 0]
    )
    ...
    # Plot labeling
    ax[idx].set_title(
        f"{idx + 1}: {team}", loc="left", color="w", fontweight="bold", fontsize=9
    )
...
# Iterate through each team, starting with highest slope of trend
for team in (
    pd.DataFrame.from_dict(trend_dict, orient="index")
    ...
    ax[idx].text(
        2015,
        17.3,
        f"Shot conversion rate\n{'drops' if trend_dict[team] < 0 else 'rises'} by mean {abs(round(trend_dict[team], 1))} %/yr",
        ha="center",
        color="w",
        fontsize=5.5,
    )
...
for team in (
    ...
    ax[idx].text(
        2018.5,
        22,
        f"Pct. left foot {'drops' if trend_dict[team] < 0 else 'rises'}\nby mean {abs(round(trend_dict[team], 1))} %/yr",
        ha="center",
        color="w",
        fontsize=5.5,
    )
...
fig.savefig(
    f"shot_characteristics_trending/{league_name}-team-left-foot-shots", dpi=300
)
