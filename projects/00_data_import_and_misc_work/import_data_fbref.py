...
for idx, statistic_group in enumerate(list(fbref_dict.keys())):
    ...
    if idx != 0:
        teaminfo_for_df = teaminfo_for_df.merge(
            temp_team_stat_for_df,
            left_on="Team ID",
            right_on="Team ID",
            suffixes=("", "_duplicate"),
            how="outer",
    ...
    if idx != 0:
        teaminfo_against_df = teaminfo_against_df.merge(
    ...
    if idx != 0:
        playerinfo_df = playerinfo_df.merge(
            temp_player_stat_df,
            left_on=["Player", "Player ID", "Squad"],
            right_on=["Player", "Player ID", "Squad"],
            suffixes=("", "_duplicate"),
            how="outer",
        )
    else:
...
teaminfo_for_df = teaminfo_for_df.loc[
    :, [False if "_duplicate" in x else True for x in teaminfo_for_df.columns]
]
teaminfo_against_df = teaminfo_against_df.loc[
    :, [False if "_duplicate" in x else True for x in teaminfo_against_df.columns]
]
playerinfo_df = playerinfo_df.loc[
    :, [False if "_duplicate" in x else True for x in playerinfo_df.columns]
]
