...
(
    ...
    competition_data,
    team_data,
) = wde.format_wyscout_data(leagues)
...
for _, shot in event_data[event_data["eventName"] == "Shot"].iterrows():
    ...
    if shot["playerId"] != 0:
        shots_model_df.loc[i, "player_name"] = (
            player_data.loc[shot["playerId"]]["shortName"]
            .encode("ascii", "strict")
            .decode("unicode-escape")
        )
    else:
        shots_model_df.loc[i, "player_name"] = np.nan
    shots_model_df.loc[i, "team_name"] = (
        team_data.loc[shot["teamId"]]["name"]
        .encode("ascii", "strict")
        .decode("unicode-escape")
    )
    shots_model_df.loc[i, "competition_name"] = competition_data.loc[
        match_data.loc[shot["matchId"]]["competitionId"]
    ]["name"]

    # Position and distance info
    shots_model_df.loc[i, "x_yards"] = (PITCH_LENGTH_X / 100) * (
        100 - shot["positions"][0]["x"]
    )
    shots_model_df.loc[i, "c_yards"] = (PITCH_WIDTH_Y / 100) * (
        shot["positions"][0]["y"] - 50
    )
    shots_model_df.loc[i, "distance_yards"] = np.sqrt(
        shots_model_df.loc[i, "x_yards"] ** 2 + shots_model_df.loc[i, "c_yards"] ** 2
    )

    # Angle info
    angle_denominator = (
        shots_model_df.loc[i, "x_yards"] ** 2
        + shots_model_df.loc[i, "c_yards"] ** 2
        - (GOAL_WIDTH_Y / 2) ** 2
    )
    if angle_denominator == 0:
        ...
        angle = np.arctan(
            (2 * (GOAL_WIDTH_Y / 2) * shots_model_df.loc[i, "x_yards"])
            / angle_denominator
        )
    if angle < 0:
        angle = np.pi + angle
...
X = shots_model_df.drop(
    ["player_name", "team_name", "competition_name", "goal"], axis=1
)
...
X_test_grnd = pd.DataFrame(
    columns=["xpos", "cpos", "distance", "angle", "header_tag"], dtype="float64"
)
X_test_head = pd.DataFrame(
    columns=["xpos", "cpos", "distance", "angle", "header_tag"], dtype="float64"
)
i = 0
...
prob_goal_grnd = neural_net.predict([X_test_grnd])[:, 1].reshape(
    int(1 + PITCH_LENGTH_X / 2), int(1 + PITCH_WIDTH_Y)
)
prob_goal_head = neural_net.predict([X_test_head])[:, 1].reshape(
    int(1 + PITCH_LENGTH_X / 2), int(1 + PITCH_WIDTH_Y)
)
...
# Plot pitches
pitch = VerticalPitch(
    half=True, pitch_color="#313332", line_color="white", linewidth=1, stripe=False
)
...
fig.text(
    0.045,
    0.85,
    "Trained on all 40,000+ shots during the 2017/18 season across Europe's 'big five' Leagues",
    fontsize=14,
    color="white",
    fontweight="regular",
)
...
# Colourbar
cbar = fig.colorbar(
    pos2, ax=ax["pitch"][1], location="bottom", fraction=0.04, pad=0.0335
)
cbar.ax.set_ylabel(
    "xG", loc="bottom", color="white", fontweight="bold", rotation=0, labelpad=20
)
...
fig.text(
    0.255,
    0.09,
    "Created by Jake Kolliari. Data provided by Wyscout.com",
    fontstyle="italic",
    ha="center",
    fontsize=9,
    color="white",
