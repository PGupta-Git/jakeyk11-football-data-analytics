...
shots_model_df = bz2.BZ2File(
    "../../data_directory/misc_data/log_regression_xg_data.pbz2", "rb"
)
...
if not shots_model_df[shots_model_df["competition_name"] == player_team_or_comp].empty:
    selected_shots = shots_model_df[
        shots_model_df["competition_name"] == player_team_or_comp
    ]
    comp_selected = 1
    ...
    selected_shots = shots_model_df[
        shots_model_df["player_name"] == player_team_or_comp
    ]
    comp_selected = 0
...
highest_xg_miss = (
    selected_shots[selected_shots["goal"] == 0]
...
# Plot ground shots
ax["pitch"].scatter(
    PITCH_WIDTH_Y / 2 + selected_ground_shots["c_yards"],
    PITCH_LENGTH_X - selected_ground_shots["x_yards"],
    marker="h",
    s=200,
    alpha=0.2,
    c=selected_ground_shots["xG"],
    edgecolors="w",
    vmin=-0.04,
    vmax=0.4,
    cmap=plt.cm.inferno,
...
p1 = ax["pitch"].scatter(
    PITCH_WIDTH_Y / 2 + selected_ground_goals["c_yards"],
    PITCH_LENGTH_X - selected_ground_goals["x_yards"],
    marker="h",
    s=200,
    c=selected_ground_goals["xG"],
    edgecolors="w",
    lw=2,
    vmin=-0.04,
    vmax=0.4,
...
# Plot headers
ax["pitch"].scatter(
    PITCH_WIDTH_Y / 2 + selected_headers["c_yards"],
    PITCH_LENGTH_X - selected_headers["x_yards"],
    marker="o",
    s=200,
    alpha=0.2,
    c=selected_headers["xG"],
    edgecolors="w",
    vmin=-0.04,
    vmax=0.4,
    cmap=plt.cm.inferno,
...
ax["pitch"].scatter(
    PITCH_WIDTH_Y / 2 + selected_headed_goals["c_yards"],
    PITCH_LENGTH_X - selected_headed_goals["x_yards"],
    marker="o",
    s=200,
    c=selected_headed_goals["xG"],
    edgecolors="w",
    lw=2,
    vmin=-0.04,
    vmax=0.4,
...
ax["pitch"].scatter(
    PITCH_WIDTH_Y / 2 + highest_xg_miss["c_yards"],
    PITCH_LENGTH_X - highest_xg_miss["x_yards"],
    marker=highxg_marker,
    s=200,
    c="r",
    edgecolors="grey",
    lw=2.5,
...
ax["pitch"].scatter(
    PITCH_WIDTH_Y / 2 + lowest_xg_goal["c_yards"],
    PITCH_LENGTH_X - lowest_xg_goal["x_yards"],
    marker=lowxg_marker,
    s=200,
    c="g",
    edgecolors="w",
...
fig.text(
    0.73,
    0.9,
    f"{round(selected_shots.sum()['xG'], 1)}",
    fontweight="regular",
    fontsize=10,
    color="w",
)
...
fig.text(
    0.73,
    0.85,
    f"{sign}{int(round(100 * (selected_shots['goal'].sum() - selected_shots.sum()['xG']) / selected_shots.sum()['xG'], 0))}%",
    fontweight="regular",
    fontsize=10,
    color="w",
)
...
fig.text(
    0.89,
    0.925,
    f"{round(selected_shots.sum()['xG'] / selected_shots.count()[0], 2)}",
    fontweight="regular",
    fontsize=10,
    color="w",
)
fig.text(
    0.89,
    0.9,
    f"{round(selected_shots['goal'].sum() / selected_shots.count()[0], 2)}",
    fontweight="regular",
    fontsize=10,
    color="w",
)
fig.text(
    0.89,
    0.875,
    f"{round(lowest_xg_goal['xG'].values[0], 2)}",
    fontweight="regular",
    fontsize=10,
    color="w",
)
fig.text(
    0.89,
    0.85,
    f"{round(highest_xg_miss['xG'].values[0], 2)}",
    fontweight="regular",
    fontsize=10,
    color="w",
)
