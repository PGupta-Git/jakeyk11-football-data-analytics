...
with bz2.BZ2File(
    "../../data_directory/misc_data/log_regression_xg_data.pbz2", "wb"
) as f:
...
for x_pos in range(0, int(PITCH_LENGTH_X / 2 + 1)):
    for y_pos in range(0, int(PITCH_WIDTH_Y + 1)):
        ...
        prob_goal_grnd[x_pos, y_pos] = log_model.predict_proba(
            [[x_pos, c_pos, distance, angle, 0]]
        )[:, 1]
        prob_goal_head[x_pos, y_pos] = log_model.predict_proba(
            [[x_pos, c_pos, distance, angle, 1]]
        )[:, 1]
...
fig.text(
    0.045,
    0.9,
    "Expected Goals - Logistic Regression Model",
    fontsize=16,
    color="white",
    fontweight="bold",
)
...
fig.text(
    0.255,
    0.09,
    "Created by Jake Kolliari (@_JKDS_). Data provided by Wyscout.",
    fontstyle="italic",
    ha="center",
    fontsize=9,
    color="white",
