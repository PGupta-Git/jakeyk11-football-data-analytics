...
for file in files:
    if "-eventdata-" in file:
        if (file.split("-")[3] in all_teams) or (
            file.split("-")[4].replace(".pbz2", "") in all_teams
        ):
            match_events = bz2.BZ2File(f"{file_path}/{file}", "rb")
            match_events = pickle.load(match_events)
    ...
    for file in files:
        if "-eventdata-" in file:
            ...
            if (file.split("-")[3] in all_teams) or (
                ...
                match_players = bz2.BZ2File(f"{file_path}/{file}", "rb")
                match_players = pickle.load(match_players)
                players_prev_above_df = pd.concat(
                    [players_prev_above_df, match_players]
                )
        else:
            pass
...
def process_event_data(events_in):
    ...
    threat_creating_events = threat_creating_events[
        ~threat_creating_events["satisfiedEventsTypes"].apply(
...
threat_creating_events_prev_below_df, events_prev_below_df = process_event_data(
    events_prev_below_df
)
if league_above != None:
    threat_creating_events_prev_above_df, events_prev_above_df = process_event_data(
        events_prev_above_df
    )

# %% Get teams and order on total threat created

# Set up dictionary to store xt per 90 per team
team_xt_df = pd.DataFrame(
    columns=["team", "mins_played", "xT_90", "mins_played_prev", "xT_90_prev"]
)
...
for idx, team in enumerate(all_teams):
    ...
    team_threat_creating_events = threat_creating_events_df[
        threat_creating_events_df["teamId"] == team_id
    ]
    ...
    if team in set(players_prev_df["team"]):
        team_threat_creating_events_prev = threat_creating_events_prev_df[
            threat_creating_events_prev_df["teamId"] == team_id
        ]
        all_events_prev = events_prev_df.copy()
    elif team in set(players_prev_below_df["team"]):
        team_threat_creating_events_prev = threat_creating_events_prev_below_df[
            threat_creating_events_prev_below_df["teamId"] == team_id
        ]
        all_events_prev = events_prev_below_df.copy()
    elif team in set(players_prev_above_df["team"]):
        team_threat_creating_events_prev = threat_creating_events_prev_above_df[
            threat_creating_events_prev_above_df["teamId"] == team_id
        ]
    ...
    for match in team_matches_prev:
        team_mins_prev += all_events_prev[all_events_prev["match_id"] == match][
            "cumulative_mins"
        ].max()
    ...
    team_xt_df.loc[idx, "xT_90"] = 90 * (
        team_threat_creating_events["xThreat_gen"].sum() / team_mins
    )
    team_xt_df.loc[idx, "xT_90_prev"] = 90 * (
        team_threat_creating_events_prev["xThreat_gen"].sum() / team_mins_prev
    )
...
rgb_left = (
    int(255 * selected_cmap(0)[0]),
    int(255 * selected_cmap(0)[1]),
    int(255 * selected_cmap(0)[2]),
)
rgb_right = (
    int(255 * selected_cmap(255)[0]),
    int(255 * selected_cmap(255)[1]),
    int(255 * selected_cmap(255)[2]),
)
...
# Path effects
path_eff = [
    path_effects.Stroke(linewidth=4, foreground="#313332"),
    path_effects.Normal(),
]
...
for _, team_row in team_xt_df.iterrows():
    ...
    bin_statistic = pitch.bin_statistic(
        team_threat_creating_events["x"],
        team_threat_creating_events["y"],
        statistic="sum",
        bins=(6, 5),
        normalize=False,
        values=team_threat_creating_events["xThreat_gen"],
    )
    bin_statistic["statistic"] = (
        90 * bin_statistic["statistic"] / team_row["mins_played"]
    ).round(3)

    bin_statistic_prev = pitch.bin_statistic(
        team_threat_creating_events_prev["x"],
        team_threat_creating_events_prev["y"],
        statistic="sum",
        bins=(6, 5),
        normalize=False,
        values=team_threat_creating_events_prev["xThreat_gen"],
    )
    bin_statistic_prev["statistic"] = (
        90 * bin_statistic_prev["statistic"] / team_row["mins_played_prev"]
    ).round(3)

    bin_statistic_change = bin_statistic.copy()
    bin_statistic_change["statistic"] = (
        bin_statistic["statistic"] - bin_statistic_prev["statistic"]
    )

    # Draw heatmap
    pcm = pitch.heatmap(
        bin_statistic_change,
        ax["pitch"][idx],
        cmap=CustomCmap,
        edgecolor="w",
        lw=0.5,
    ...
    ax["pitch"][idx].text(
        2,
        12,
        f"xT/90 {int(year.replace('20', '', 1))}/{int(year.replace('20', '', 1)) + 1}:",
        fontsize=9,
        fontweight="bold",
        color="w",
        zorder=3,
    ...
    ax["pitch"][idx].text(
        2,
        2,
        f"xT/90 {int(year.replace('20', '', 1)) - 1}/{int(year.replace('20', '', 1))}:",
        fontsize=9,
        fontweight="bold",
        color="w",
        zorder=3,
    ...
    ax["pitch"][idx].set_title(
        f"  {idx + 1}:  {team}", pad=2, loc="left", color="w", fontsize=16
    )
...
title_text = (
    f"{leagues[league]} {year}/{int(year) + 1} − Improvement in Threat Creation by Team"
)
subtitle_text = (
    "Zones of <Improved> and <Reduced> Threat Creation compared to Last Season"
)
...
htext.fig_text(
    ...
    color="w",
    highlight_textprops=[
        {"color": "palegreen", "fontweight": "bold"},
