...
for team in teams:
    ...
    for match_id in match_ids:
        ...
        team_match_players = players_df[
            (players_df["teamId"] == team_id) & (players_df["match_id"] == match_id)
        ]
        starting_rb_id = team_match_players[
            team_match_players["position"].isin(["DMR", "DR"])
        ].index.values.tolist()
        starting_lb_id = team_match_players[
            team_match_players["position"].isin(["DML", "DL"])
        ].index.values.tolist()

        # Get all events completed by team within match
        team_match_events = events_df[
            (events_df["teamId"] == team_id) & (events_df["match_id"] == match_id)
        ]
        ...
        if (len(starting_rb_id) > 0) & (len(starting_lb_id) > 0):
            ...
            fb_pass_combos_match = team_match_events[
                (team_match_events["eventType"] == "Pass")
                & (team_match_events["outcomeType"] == "Successful")
                & (
                    ~team_match_events["satisfiedEventsTypes"].apply(
                        lambda x: 31 in x or 32 in x or 33 in x or 34 in x or 212 in x
                    )
                )
                & (
                    (
                        (team_match_events["playerId"] == starting_rb_id[0])
                        & (team_match_events["pass_recipient"] == starting_lb_id[0])
                    )
                    | (
                        (team_match_events["playerId"] == starting_lb_id[0])
                        & (team_match_events["pass_recipient"] == starting_rb_id[0])
                    )
                )
            ]
            ...
            for idx, fb_pass in fb_pass_combos_match.iterrows():
                following_evts = team_match_events[
                    (team_match_events["match_id"] == fb_pass["match_id"])
                    & (team_match_events["period"] == fb_pass["period"])
                    & (
                        team_match_events["cumulative_mins"]
                        > fb_pass["cumulative_mins"]
                    )
                    & (
                        team_match_events["cumulative_mins"]
                        <= fb_pass["cumulative_mins"] + min_delta
                    )
                ]
                fb_pass_combos_match.loc[idx, "leads_to_shot"] = (
                    True if True in following_evts["isShot"].tolist() else False
                )
...
for team in team_order_xt_90:
    ...
    # Add xT text to plot
    ax["pitch"][idx].text(
        2, 3, "xT / match", fontsize=8, fontweight="bold", color="w", zorder=3
    )
    ax["pitch"][idx].text(
        28, 3, round(team_fb_combo_xt[team], 3), fontsize=8, color="w", zorder=3
    )
    ...
    ax["pitch"][idx].set_title(
        f"  {idx + 1}:  {team}", loc="left", color="w", fontsize=14
    )
...
fig.savefig(
    f"team_fullback_combinations/{league}-{year}-team_fullback_combinations", dpi=300
)
