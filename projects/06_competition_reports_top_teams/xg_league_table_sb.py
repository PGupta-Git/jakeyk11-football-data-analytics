...
# Statsbomb
data_dict = gfd.load_statsbomb_sql(
    data_grab, events=True, matches=True, lineups=True, player_stats=True
)
...
comp_logo = lab.get_competition_logo(
    data_grab[0][1], data_grab[0][2], logo_brighten=True
)
...
for idx, error in all_errors.iterrows():
    error_evts = events_df[
        (events_df["match_id"] == error["match_id"])
...
ip_goals_for = pd.concat(
    [
        ip_shots_for[ip_shots_for["outcome_name"] == "Goal"],
        events_df[events_df["type_name"] == "Own Goal For"],
    ]
)
teaminfo_df = sde.group_team_events(
    shots_for,
    teaminfo_df,
    group_type="sum",
    agg_columns="shot_statsbomb_xg",
    primary_event_name="xg_for",
)
teaminfo_df = sde.group_team_events(
    ip_shots_for,
    teaminfo_df,
    group_type="sum",
    agg_columns="shot_statsbomb_xg",
    primary_event_name="ip_xg_for",
)
teaminfo_df = sde.group_team_events(
    ip_goals_for, teaminfo_df, group_type="count", primary_event_name="ip_goals_for"
)

# In play offensive OBV
ip_obv_events = events_df[
    (events_df["type_name"].isin(["Pass", "Carry", "Dribble"]))
...
teaminfo_df = sde.group_team_events(
    ...
    teaminfo_df,
    group_type="sum",
    agg_columns="obv_for_net_z",
    primary_event_name="ip_xt_for",
)
...
for team_name, _ in teaminfo_df.iterrows():
    ...
    ip_goals_against = pd.concat(
        [
            ip_shots_against[ip_shots_against["outcome_name"] == "Goal"],
            evts_against[evts_against["type_name"] == "Own Goal For"],
        ]
    )
    ip_obv_events_against = evts_against[
        (evts_against["type_name"].isin(["Pass", "Carry", "Dribble"]))
        & (evts_against["in_play_event"] == 1)
    ]
    teaminfo_df.loc[team_name, "xg_against"] = shots_against["shot_statsbomb_xg"].sum(
        numeric_only=True
    )
    teaminfo_df.loc[team_name, "ip_xg_against"] = ip_shots_against[
        "shot_statsbomb_xg"
    ].sum(numeric_only=True)
    teaminfo_df.loc[team_name, "ip_goals_against"] = len(ip_goals_against)
    teaminfo_df.loc[team_name, "ip_xt_against"] = ip_obv_events_against[
        "obv_for_net_z"
    ].sum(numeric_only=True)

    team_post_error_evts_against = events_following_error[
        (events_following_error["match_id"].isin(match_ids))
        & (events_following_error["team_name"] != team_name)
    ]
    post_error_shots_against = team_post_error_evts_against[
        (team_post_error_evts_against["type_name"] == "Shot")
        & (team_post_error_evts_against["in_play_event"] == 1)
    ]
    teaminfo_df.loc[team_name, "xg_against_following_error"] = post_error_shots_against[
        "shot_statsbomb_xg"
    ].sum(numeric_only=True)

teaminfo_df["non-error_ip_xg_against"] = (
    teaminfo_df["ip_xg_against"] - teaminfo_df["xg_against_following_error"]
)
teaminfo_df["xg_difference"] = teaminfo_df["xg_for"] - teaminfo_df["xg_against"]
teaminfo_df["ip_xg_difference"] = (
    teaminfo_df["ip_xg_for"] - teaminfo_df["ip_xg_against"]
)
teaminfo_df["ip_xg_xt_ratio"] = teaminfo_df["ip_xg_for"] / teaminfo_df["ip_xt_for"]
teaminfo_df["ip_xg_xt_against_ratio"] = (
    teaminfo_df["ip_xg_against"] / teaminfo_df["ip_xt_against"]
)
teaminfo_df["ip_goal_xg_ratio"] = teaminfo_df["ip_goals_for"] / teaminfo_df["ip_xg_for"]
teaminfo_df["ip_goal_xg_against_ratio"] = (
    teaminfo_df["ip_goals_against"] / teaminfo_df["ip_xg_against"]
)
...
for column in teaminfo_df.columns:
    if ("xg" in column or "xt" in column) and ("ratio" not in column):
        teaminfo_df[column + "_90"] = (
            90 * teaminfo_df[column] / teaminfo_df["time_played"]
        )
...
# Label axes
ax.set_ylabel(
    "Ratio between In-Play Goals and Expected Goals (Goals/xG)",
    labelpad=10,
    fontweight="bold",
    fontsize=12,
    color="w",
...
ax.set_xlabel(
    "Ratio between In-Play Expected Goals and Expected Threat (xG/xT)",
    labelpad=10,
    fontweight="bold",
    fontsize=12,
    color="w",
...
for team, team_metrics in teaminfo_df.iterrows():
    ...
    # Plot logo
    ab = AnnotationBbox(
        OffsetImage(team_logo, zoom=0.07, resample=True),
        (team_metrics[plot_x], team_metrics[plot_y]),
        frameon=False,
    )
    ax.add_artist(ab)
...
for team, team_metrics in teaminfo_df.iterrows():
    ...
    team_logo, _ = lab.get_team_badge_and_colour(team)
    ab = AnnotationBbox(
        OffsetImage(team_logo, zoom=0.06, resample=True),
        (0.08, 0.9 * (1 - idx / 19) + 0.003),
        frameon=False,
    )
    ax.add_artist(ab)
    ...
    ax.text(
        0.5875,
        0.9 * (1 - idx / 19),
        round(team_metrics["ip_xg_xt_ratio"], 2),
        va="center",
        ha="center",
        color="w",
    )
    ax.text(
        0.8625,
        0.9 * (1 - idx / 19),
        round(team_metrics["ip_goal_xg_ratio"], 2),
        va="center",
        ha="center",
        color="w",
    )
    ...
    ax.plot(
        [0, 1],
        [0.9 * (1 - idx / 19) - 0.02, 0.9 * (1 - idx / 19) - 0.02],
        color="grey",
        lw=0.5,
        zorder=1,
    )
...
fig.savefig(
    f"team_xg_metrics/{data_grab[0][1].replace(' ', '-').lower()}-{data_grab[0][2]}-xg-xt-table",
    dpi=300,
)
