...
all_fks_and_corners = events_df[
    events_df["satisfiedEventsTypes"].apply(
        lambda x: True if (31 in x or 34 in x or 6 in x or 5 in x) else False
    )
]

fks_and_corners = all_fks_and_corners[
    (all_fks_and_corners["eventType"] == "Pass")
    | (all_fks_and_corners["eventType"] == "SavedShot")
    | (
        (all_fks_and_corners["eventType"] == "MissedShots")
        & (all_fks_and_corners["blockedX"] == "blockedX")
    )
]
...
for idx, set_piece in fks_and_corners.iterrows():
    next_evts = events_df[
        (events_df["match_id"] == set_piece["match_id"])
        & (events_df["period"] == set_piece["period"])
        & (events_df["eventId"] != set_piece["eventId"])
        & (events_df["cumulative_mins"] >= set_piece["cumulative_mins"])
        & (events_df["cumulative_mins"] <= set_piece["cumulative_mins"] + (5 / 60))
    ]
    team_next_evts = next_evts[next_evts["teamId"] == set_piece["teamId"]]
    opp_next_evts = next_evts[next_evts["teamId"] != set_piece["teamId"]]
    if (True in team_next_evts["isGoal"].tolist()) or (
        True in opp_next_evts["isOwnGoal"].tolist()
    ):
        fks_and_corners.loc[idx, "set_piece_outcome"] = "Goal"
    elif (
        (True in team_next_evts["isShot"].tolist())
        or ("ChanceMissed" in team_next_evts["eventType"].tolist())
        or ("Foul" in opp_next_evts["isOwnGoal"].tolist())
    ):
...
team_sp_concede_df = pd.DataFrame(
    columns=[
        "team",
        "matches_played",
        "sp_concede",
...
for idx, team in enumerate(teams):
    ...
    team_sp_chance_concede = team_sp_concede[
        team_sp_concede["set_piece_outcome"].isin(["Goal", "Chance"])
    ]
    team_sp_goal_concede = team_sp_concede[
        team_sp_concede["set_piece_outcome"].isin(["Goal", "Direct Goal"])
    ]

    # Add information to df
    team_sp_concede_df.loc[idx, "team"] = team
    team_sp_concede_df.loc[idx, "matches_played"] = len(
        set(events_df[events_df["team_name"] == team]["match_id"])
    )
...
team_sp_concede_df["sp_chance_concede_per_match"] = (
    team_sp_concede_df["sp_chance_concede"] / team_sp_concede_df["matches_played"]
)
team_sp_concede_df["sp_goal_concede_per_match"] = (
    team_sp_concede_df["sp_goal_concede"] / team_sp_concede_df["matches_played"]
)
team_sp_concede_df["sp_chance_concede_pct"] = (
    100 * team_sp_concede_df["sp_chance_concede"] / team_sp_concede_df["sp_concede"]
)
team_sp_concede_df["sp_goal_concede_pct"] = (
    100 * team_sp_concede_df["sp_goal_concede"] / team_sp_concede_df["sp_concede"]
)
...
color_scale = (
    ...
    * (
        team_sp_concede_df["sp_chance_concede_pct"]
        - team_sp_concede_df["sp_chance_concede_pct"].min()
    )
    / (
        team_sp_concede_df["sp_chance_concede_pct"].max()
        - team_sp_concede_df["sp_chance_concede_pct"].min()
    )
)
...
team_rank = [
    str(i) + ": " for i in team_sp_concede_df.index.values + 1
] + team_sp_concede_df["team"]

# Plot data
ax.barh(
    team_rank, team_sp_concede_df["sp_chance_concede_pct"], color=my_cmap[color_scale]
)
ax.barh(
    team_rank,
    team_sp_concede_df["sp_goal_concede_pct"],
...
# Define axes
ax.set_xlabel(
    "Percentage of Opposition Set-pieces Resulting in Chance",
    fontweight="bold",
    fontsize=12,
    color="w",
)
...
for idx, plot_pos in enumerate(np.arange(0.2, len(team_sp_concede_df))):
    goal_text = (
        " goal" if team_sp_concede_df.loc[idx, "sp_goal_concede"] == 1 else " goals"
    )
    chance_text = (
        " chance"
        if team_sp_concede_df.loc[idx, "sp_chance_concede"] == 1
        else " chances"
    )
    ax.text(
        team_sp_concede_df.loc[idx, "sp_goal_concede_pct"],
        plot_pos,
        " " + str(team_sp_concede_df.loc[idx, "sp_goal_concede"]) + goal_text,
        fontsize=8,
        color="w",
        path_effects=path_eff,
    )
    ax.text(
        team_sp_concede_df.loc[idx, "sp_chance_concede_pct"],
        plot_pos,
        " " + str(team_sp_concede_df.loc[idx, "sp_chance_concede"]) + chance_text,
        fontsize=8,
        color="w",
        path_effects=path_eff,
    )
...
# Save image to file
fig.savefig(
    f"team_setpiece_chance_concession/{league}-{year}-team_setpiece_chance_concession_pct",
    dpi=300,
)
...
# Order dataframe and reindex
team_sp_concede_df.sort_values(
    "sp_chance_concede_per_match", ascending=True, inplace=True
)
team_sp_concede_df.reset_index(drop=True, inplace=True)
...
color_scale = (
    ...
    * (
        team_sp_concede_df["sp_chance_concede_per_match"]
        - team_sp_concede_df["sp_chance_concede_per_match"].min()
    )
    / (
        team_sp_concede_df["sp_chance_concede_per_match"].max()
        - team_sp_concede_df["sp_chance_concede_per_match"].min()
    )
)
color_scale = [int(color) for color in color_scale.values.tolist()]
...
title_text = f'{leagues[league]} {year}/{str(int(year) + 1).replace("20", "", 1)} − Defending "Indirect" Set Pieces'
subtitle_text = (
    "Set Pieces Against Resulting in Opp. Chance or Goal (within 5s) per match"
)
subsubtitle_text = f'Correct as of {run_date}. "Indirect" covers corners or FKs where ball remains in-play after initial action.'
fig.text(0.135, 0.94, title_text, fontweight="bold", fontsize=15, color="w")
fig.text(0.135, 0.912, subtitle_text, fontweight="bold", fontsize=12, color="w")
...
# Define axes
ax.set_xlabel(
    "Number of Opposition Set-pieces Resulting in Chance per Match",
    fontweight="bold",
    fontsize=12,
    color="w",
)
...
for idx, plot_pos in enumerate(np.arange(0.2, len(team_sp_concede_df))):
    ...
    ax.text(
        team_sp_concede_df.loc[idx, "sp_goal_concede_per_match"],
        plot_pos,
        " " + str(team_sp_concede_df.loc[idx, "sp_goal_concede"]) + goal_text,
        fontsize=8,
        color="w",
    ...
    ax.text(
        team_sp_concede_df.loc[idx, "sp_chance_concede_per_match"],
        plot_pos,
        " " + str(team_sp_concede_df.loc[idx, "sp_chance_concede"]) + chance_text,
        fontsize=8,
        color="w",
...
# Save image to file
fig.savefig(
    f"team_setpiece_chance_concession/{league}-{year}-team_setpiece_chance_concession_per_match",
    dpi=300,
)
