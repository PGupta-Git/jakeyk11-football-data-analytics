...
all_crosses = events_df[
    (events_df["eventType"] == "Pass")
    & (abs(events_df["endY"] - events_df["y"]) >= 10)
    & (
...
open_play_crosses = all_crosses[
    ~all_crosses["satisfiedEventsTypes"].apply(
...
for idx, cross in open_play_crosses.iterrows():
    next_evts = events_df[
        (events_df["match_id"] == cross["match_id"])
        & (events_df["period"] == cross["period"])
        & (events_df["cumulative_mins"] > cross["cumulative_mins"])
        & (events_df["cumulative_mins"] <= cross["cumulative_mins"] + (5 / 60))
    ]
    ...
    if (cross["endX"] == 100) | (cross["endY"] == 100) | (cross["endY"] == 0):
        ...
        if ("Goal" in next_evts["eventType"].tolist()) or (
            92 in cross["satisfiedEventsTypes"]
        ):
...
for team in teams:
    ...
    team_effective_crosses = team_crosses[
        team_crosses["cross_outcome"].isin(["Goal", "Shot", "Key Pass"])
    ]
...
# Sort cross count
team_effective_cross = sorted(
    team_effective_cross.items(), key=lambda x: x[1], reverse=True
)
...
for team in team_effective_cross:
    ...
    team_key_crosses = team_crosses[
        team_crosses["cross_outcome"].isin(["Shot", "Key Pass"])
    ]
    ...
    pitch.lines(
        team_unsuc_crosses["x"],
        team_unsuc_crosses["y"],
        team_unsuc_crosses["endX"],
        team_unsuc_crosses["endY"],
        color=normal_col,
        alpha=0.3,
        zorder=1,
        lw=1,
    ...
    pitch.lines(
        team_suc_crosses["x"],
        team_suc_crosses["y"],
        team_suc_crosses["endX"],
        team_suc_crosses["endY"],
        color=normal_col,
        alpha=0.3,
        zorder=2,
        lw=1,
    ...
    for x in np.arange(x_min, x_max - x_range + 1, 2.5):
        ...
        for y in np.arange(y_min, y_max - y_range + 1, 2.5):
            ...
            for _, cross in pd.concat(
                [team_goal_crosses, team_key_crosses], axis=0
            ).iterrows():
                p = [cross["x"], cross["y"]]
    ...
    ax["pitch"][idx].fill(
        [zone_st[1], zone_st[3], zone_st[3], zone_st[1]],
        [zone_st[0], zone_st[0], zone_st[2], zone_st[2]],
        edgecolor="w",
        facecolor="#313332",
        hatch="////\\\\\\\\",
        alpha=0.7,
    ...
    # Add axis text
    ax["pitch"][idx].text(
        98, 56, "Crosses:", color="w", fontsize=6, fontweight="bold", va="center"
    )
    ax["pitch"][idx].text(68, 56, len(team_crosses), color="w", fontsize=6, va="center")
    ax["pitch"][idx].text(
        98, 52, "Effective:", color="w", fontsize=6, fontweight="bold", va="center"
    )
    ax["pitch"][idx].text(
        68,
        52,
        len(team_key_crosses) + len(team_goal_crosses),
        color="w",
        fontsize=6,
        va="center",
    )
    ...
    ax["pitch"][idx].set_title(
        f"  {idx + 1}: {team_name}", loc="left", color="w", fontsize=11, pad=2
    )
...
subtitle_text = (
    "In-Play Crosses Shown: <Cross Assists> and <Effective Crosses> highlighted"
)
...
htext.fig_text(
    ...
    color="w",
    highlight_textprops=[
        {"color": assist_col, "fontweight": "bold"},
        {"color": key_col, "fontweight": "bold"},
    ],
)
...
ax.text(
    1.2,
    0.4,
    "12.5m x 12.5m area from which largest volume\nof effective crosses are made. # represents number\nof crosses from the area",
    color="w",
    fontsize=7,
    va="center",
    ha="left",
