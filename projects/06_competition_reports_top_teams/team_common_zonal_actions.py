...
# Passes
ip_passes = events_df[
    (events_df["eventType"].isin(["Pass", "OffsidePass"]))
    & (
...
ip_shots = events_df[
    ...
    & (
        ~events_df["satisfiedEventsTypes"].apply(
            lambda x: True if (5 in x or 6 in x or 22 in x or 135 in x) else False
        )
    )
].reset_index(drop=True)
...
cross_longballs = ip_passes[
    ip_passes["satisfiedEventsTypes"].apply(
        lambda x: 59 in x or 125 in x or 126 in x or 127 in x or 128 in x
    )
]
...
standardpasses[["angle", "direction"]] = standardpasses.apply(
    pass_direction, side_angle=side_angle, result_type="expand", axis=1
)
...
for team in league_table["team"].tolist():
    ...
    if mode == "threat":
        bin_shots = pitch.bin_statistic(
            team_shots["x"],
            team_shots["y"],
            statistic="sum",
            bins=(6, 5),
            values=team_shots["xThreat_gen"],
            normalize=False,
        )
        bin_carries = pitch.bin_statistic(
            team_carries["x"],
            team_carries["y"],
            statistic="sum",
            bins=(6, 5),
            values=team_carries["xThreat_gen"],
            normalize=False,
        )
        bin_crosses_longballs = pitch.bin_statistic(
            team_cross_longballs["x"],
            team_cross_longballs["y"],
            statistic="sum",
            bins=(6, 5),
            values=team_cross_longballs["xThreat_gen"],
            normalize=False,
        )
        bin_backpasses = pitch.bin_statistic(
            team_backpasses["x"],
            team_backpasses["y"],
            statistic="sum",
            bins=(6, 5),
            values=team_backpasses["xThreat_gen"],
            normalize=False,
        )
        bin_forwardpasses = pitch.bin_statistic(
            team_forwardpasses["x"],
            team_forwardpasses["y"],
            statistic="sum",
            bins=(6, 5),
            values=team_forwardpasses["xThreat_gen"],
            normalize=False,
        )
        bin_sidepasses = pitch.bin_statistic(
            team_sidepasses["x"],
            team_sidepasses["y"],
            statistic="sum",
            bins=(6, 5),
            values=team_sidepasses["xThreat_gen"],
            normalize=False,
        )
    else:
        bin_shots = pitch.bin_statistic(
    ...
    bin_max = np.maximum.reduce(
        [
            bin_shots["statistic"],
            bin_carries["statistic"],
            bin_crosses_longballs["statistic"],
            bin_backpasses["statistic"],
            bin_forwardpasses["statistic"],
            bin_sidepasses["statistic"],
        ]
    )

    common_action_bin = pitch.bin_statistic(
        team_shots["x"],
        team_shots["y"],
        statistic="count",
        bins=(6, 5),
...
CustomCmap = mpl.colors.LinearSegmentedColormap.from_list(
    "",
    [shot_col, cross_longball_col, carry_col, fwdpass_col, backpass_col, sidepass_col],
)
...
for idx, team in enumerate(league_table["team"].tolist()):
    ...
    ax["pitch"][idx].set_title(
        f"  {idx + 1}:  {team_name}", loc="left", color="w", fontsize=16
    )
...
htext.fig_text(
    ...
    color="w",
    highlight_textprops=[
        {"color": "khaki", "fontweight": "bold"},
        {"color": "lightskyblue", "fontweight": "bold"},
        {"color": "mediumpurple", "fontweight": "bold"},
        {"color": "palegreen", "fontweight": "bold"},
        {"color": "lightsalmon", "fontweight": "bold"},
        {"color": "#6a6a6a", "fontweight": "bold"},
    ],
)
