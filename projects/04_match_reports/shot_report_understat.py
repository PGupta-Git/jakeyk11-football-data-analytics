...
selected_match = match_df[
    (match_df["h"].apply(lambda x: x["title"]) == home_team)
    & (match_df["a"].apply(lambda x: x["title"]) == away_team)
]
...
# Format shot data
shots_df = (
    pd.concat([pd.DataFrame(shot_data["h"]), pd.DataFrame(shot_data["a"])])
    .set_index("id")
    .astype(
        {
            "minute": "float",
...
for idx, shot in shots_df.iterrows():
    ...
    if len(shot["player"].split(" ")) == 1:
        ...
        shots_df.loc[idx, "player_initials"] = (
            shot["player"].split(" ")[0][0].upper()
            + shot["player"].split(" ")[1][0].upper()
        )
...
# Home team stats
home_shots = len(
    (shots_df[(shots_df["h_a"] == "h") & (shots_df["result"] != "OwnGoal")])
)
home_xg_shot = round(
    protected_divide(
        float(selected_match["xG"].apply(lambda x: x["h"])[0]),
        len(shots_df[shots_df["h_a"] == "h"]),
    ),
    2,
)
home_goal_shot = round(
    protected_divide(
        int(selected_match["goals"].apply(lambda x: x["h"])[0]),
        len(shots_df[shots_df["h_a"] == "h"]),
    ),
    2,
)
...
if len(shots_df[(shots_df["h_a"] == "h") & (shots_df["result"] == "Goal")]) == 0:
    ...
    home_low_xg_goal = round(
        min(shots_df[(shots_df["h_a"] == "h") & (shots_df["result"] == "Goal")]["xG"]),
        2,
    )
if len(shots_df[(shots_df["h_a"] == "h") & (shots_df["result"] != "Goal")]) == 0:
    home_high_xg_miss = "-"
else:
    home_high_xg_miss = round(
        max(shots_df[(shots_df["h_a"] == "h") & (shots_df["result"] != "Goal")]["xG"]),
        2,
    )

# Away team stats
away_shots = len(
    (shots_df[(shots_df["h_a"] == "a") & (shots_df["result"] != "OwnGoal")])
)
away_xg_shot = round(
    protected_divide(
        float(selected_match["xG"].apply(lambda x: x["a"])[0]),
        len(shots_df[shots_df["h_a"] == "a"]),
    ),
    2,
)
away_goal_shot = round(
    protected_divide(
        int(selected_match["goals"].apply(lambda x: x["a"])[0]),
        len(shots_df[shots_df["h_a"] == "a"]),
    ),
    2,
)
...
if len(shots_df[(shots_df["h_a"] == "a") & (shots_df["result"] == "Goal")]) == 0:
    ...
    away_low_xg_goal = round(
        min(shots_df[(shots_df["h_a"] == "a") & (shots_df["result"] == "Goal")]["xG"]),
        2,
    )
if len(shots_df[(shots_df["h_a"] == "a") & (shots_df["result"] != "Goal")]) == 0:
    away_high_xg_miss = "-"
else:
    away_high_xg_miss = round(
        max(shots_df[(shots_df["h_a"] == "a") & (shots_df["result"] != "Goal")]["xG"]),
        2,
    )
...
fig, ax = pitch.grid(
    nrows=1, ncols=2, title_height=0.03, grid_height=0.51, endnote_height=0, axis=False
)
...
for _, shot in shots_df.iterrows():
    ...
    p1 = ax["pitch"][pitch_num].scatter(
        PITCH_WIDTH_Y * (1 - shot["Y"]),
        PITCH_LENGTH_X * (shot["X"]),
        marker=marker,
        s=s,
        alpha=alpha,
        c=shot["xG"],
    ...
    ax["pitch"][pitch_num].text(
        PITCH_WIDTH_Y * (1 - shot["Y"]),
        PITCH_LENGTH_X * (shot["X"]) - 0.1,
        shot["player_initials"],
        color=textcolor,
        fontsize=7,
        ha="center",
        va="center",
...
fig.text(
    0.285, 0.24, f"{h_sign}{home_xg_perf}", fontweight="regular", fontsize=10, color="w"
)
...
fig.text(
    0.769, 0.24, f"{a_sign}{away_xg_perf}", fontweight="regular", fontsize=10, color="w"
)
...
legend_ax.scatter(
    1.3, 0.8, marker="h", s=250, c="indigo", edgecolors="w", lw=1.5, zorder=2
)
legend_ax.scatter(1.3, 0.8, marker="h", s=400, c="#313332", edgecolors="w", zorder=1)
legend_ax.scatter(
    1.3, 0.5, marker="h", s=300, c="indigo", edgecolors="darkgrey", lw=1.5, alpha=1
)
legend_ax.scatter(
    1.3, 0.2, marker="h", s=300, c="indigo", edgecolors="darkgrey", lw=0.5, alpha=1
)
...
legend_ax.scatter(
    3, 0.8, marker="h", s=300, c="darkslategrey", edgecolors="w", lw=1.5, alpha=1
)
legend_ax.scatter(
    3, 0.5, marker="h", s=250, c="indigo", edgecolors="w", lw=1.5, zorder=2
)
legend_ax.scatter(3, 0.5, marker="h", s=400, c="#313332", edgecolors="lime", zorder=1)
legend_ax.scatter(
    3, 0.2, marker="h", s=300, c="y", edgecolors="crimson", lw=1.5, alpha=1
)
...
fig.text(
    0.5, 0.835, subtitle_text, ha="center", fontweight="bold", fontsize=18, color="w"
)
fig.text(
    0.5,
    0.79,
...
fig.text(
    0.5,
    0.02,
    "Created by Jake Kolliari (@_JKDS_). Data provided by Understat.com",
    fontstyle="italic",
    ha="center",
    fontsize=9,
    color="white",
...
# Save image
fig.savefig(
    f"shot_reports/{league}-{selected_match['datetime'][0][0:10]}-{home_team}-{away_team}",
    dpi=300,
)
