...
events_df = bz2.BZ2File(
    f"../../data_directory/whoscored_data/{year}_{str(int(year.replace('20', '')) + 1)}/{league}/match-eventdata-{match_id}-{home_team}-{away_team}.pbz2",
    "rb",
)
events_df = pickle.load(events_df)
players_df = bz2.BZ2File(
    f"../../data_directory/whoscored_data/{year}_{str(int(year.replace('20', '')) + 1)}/{league}/match-playerdata-{match_id}-{home_team}-{away_team}.pbz2",
    "rb",
)
...
if "isOwnGoal" in events_df.columns:
    home_score = len(
        events_df[
            (events_df["teamId"] == players_df["teamId"].unique()[0])
            & (events_df["eventType"] == "Goal")
            & (events_df["isOwnGoal"] != events_df["isOwnGoal"])
        ]
    )
    home_score += len(
        events_df[
            (events_df["teamId"] == players_df["teamId"].unique()[1])
            & (events_df["eventType"] == "Goal")
            & (events_df["isOwnGoal"] == events_df["isOwnGoal"])
        ]
    )
...
# Successful passes
suc_passes = events_df[
    (events_df["eventType"] == "Pass") & (events_df["outcomeType"] == "Successful")
]
playerinfo_df = wde.group_player_events(
...
# Progressive passes
events_df["progressive_pass"] = events_df.apply(
    wce.progressive_pass, successful_only=False, axis=1
)
prog_passes = events_df[events_df["progressive_pass"] == True]
playerinfo_df = wde.group_player_events(
    prog_passes, playerinfo_df, primary_event_name="prog_passes"
)
suc_prog_passes = events_df[
    (events_df["progressive_pass"] == True) & (events_df["outcomeType"] == "Successful")
]
playerinfo_df = wde.group_player_events(
...
# Forward passes
forward_passes = events_df[
    (events_df["eventType"] == "Pass") & (events_df["endX"] > events_df["x"])
]
playerinfo_df = wde.group_player_events(
    forward_passes, playerinfo_df, primary_event_name="fwd_passes"
)
suc_forward_passes = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["outcomeType"] == "Successful")
    & (events_df["endX"] > events_df["x"])
]
playerinfo_df = wde.group_player_events(
...
crosses = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 125 in x or 126 in x))
]
playerinfo_df = wde.group_player_events(
    crosses, playerinfo_df, primary_event_name="crosses"
)
suc_crosses = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 125 in x))
]
playerinfo_df = wde.group_player_events(
...
# Through balls
through_balls = events_df[
    (events_df["eventType"] == "Pass")
    & (
        events_df["satisfiedEventsTypes"].apply(
            lambda x: 129 in x or 130 in x or 131 in x
        )
    )
]
playerinfo_df = wde.group_player_events(
...
suc_through_balls = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 129 in x))
]
playerinfo_df = wde.group_player_events(
...
# Long balls
long_balls = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 127 in x or 128 in x))
]
playerinfo_df = wde.group_player_events(
    long_balls, playerinfo_df, primary_event_name="long_balls"
)
suc_long_balls = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 127 in x))
]
playerinfo_df = wde.group_player_events(
...
# Pass zones
events_df[["start_zone", "start_zone_center", "end_zone", "end_zone_center"]] = (
    events_df.apply(
        pz.identify_zone,
        axis=1,
        get_centers=True,
...
key_zones = pz.get_key_zones(
    zone_type=zone_type, halfspace=True, zone_14=True, cross_areas=True
)

# Zone 14 passes
z14_passes = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["start_zone"].isin(key_zones["zone_14"]))
]
...
z14_touch_assists = events_df[
    (events_df["start_zone"].isin(key_zones["zone_14"]))
    & (events_df["eventType"] != "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 92 in x))
]
z14_pre_assists = z14_passes[z14_passes["pre_assist"] == True]
z14_touch_pre_assists = events_df[
    (events_df["start_zone"].isin(key_zones["zone_14"]))
    & (events_df["eventType"] != "Pass")
    & (events_df["pre_assist"] == True)
]

# Half space passes
hs_passes = events_df[
    (events_df["eventType"] == "Pass")
    & (events_df["start_zone"].isin(key_zones["halfspace"]))
]
...
hs_touch_assists = events_df[
    (events_df["start_zone"].isin(key_zones["halfspace"]))
    & (events_df["eventType"] != "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 92 in x))
]
hs_pre_assists = hs_passes[hs_passes["pre_assist"] == True]
hs_touch_pre_assists = events_df[
    (events_df["start_zone"].isin(key_zones["halfspace"]))
    & (events_df["eventType"] != "Pass")
    & (events_df["pre_assist"] == True)
]
...
for team in players_df["teamId"].unique():
    ...
    for zone_center in events_df[
        (events_df["eventType"] == "Pass") & (events_df["teamId"] == team)
    ]["start_zone_center"].unique():
        zone_passes = events_df[
            (events_df["eventType"] == "Pass")
            & (events_df["start_zone_center"] == zone_center)
            & (events_df["teamId"] == team)
        ]
        zz_pass_popularity_team[zone_center] = Counter(
            zone_passes["end_zone_center"].values
        )
...
offensive_actions_df = events_df[
    (events_df["eventType"] == "Pass")
    & (
        events_df["satisfiedEventsTypes"].apply(
            lambda x: not (31 in x or 34 in x or 212 in x)
        )
    )
]
...
for player_id in players_df[players_df["longest_xi"] == True].index:
    player_def_hull = wce.create_convex_hull(
        defensive_actions_df[defensive_actions_df["playerId"] == player_id],
        name=players_df.loc[player_id, "name"],
        min_events=5,
        include_events=central_pct,
        pitch_area=10000,
    )
    player_off_hull = wce.create_convex_hull(
        offensive_actions_df[offensive_actions_df["playerId"] == player_id],
        name=players_df.loc[player_id, "name"],
        min_events=5,
        include_events=central_pct,
        pitch_area=10000,
...
title_text = (
    f"{leagues[league]} - {year}/{int(year) + 1}"
    if not league in ["World_Cup"]
    else f"{leagues[league]} - {year}"
)
...
fig.text(
    0.5, 0.882, subtitle_text, ha="center", fontweight="bold", fontsize=18, color="w"
)
fig.text(
    0.5,
    0.84,
...
for idx, pass_count in enumerate(np.arange(1, 16, 2)):
    ...
    legend_ax_2.text(
        xpos, ypos, pass_count, color=text_color, fontsize=10, ha="center", va="center"
    )
    legend_ax_2.text(
        4, -0.2, "Pass Count", color=text_color, fontsize=10, ha="center", va="center"
    )
...
# Save image
fig.savefig(
    f"pass_reports/{league}-{match_id}-{home_team}-{away_team}-passflows", dpi=300
)
...
for hull_idx, hull_row in offensive_hull_df.iterrows():
    ...
    if (
        players_df[players_df["name"] == hull_idx]["teamId"].values[0]
        == players_df["teamId"].unique()[0]
    ):
        idx = 0
    elif (
        players_df[players_df["name"] == hull_idx]["teamId"].values[0]
        == players_df["teamId"].unique()[1]
    ):
        idx = 1
    ...
    # Plot
    ax["pitch"][idx].scatter(
        hull_row["hull_reduced_y"],
        hull_row["hull_reduced_x"],
        color=hull_colour,
        s=20,
        alpha=0.3,
        zorder=2,
    ...
    ax["pitch"][idx].scatter(
        hull_row["hull_centre"][1],
        hull_row["hull_centre"][0],
        marker="H",
        color=hull_colour,
        alpha=0.6,
        s=400,
...
for idx in np.arange(0, 3):
    ...
    ax.text(
        0.24,
        0.71 - 0.22 * idx,
        f"{round(home_top_area.iloc[idx : idx + 1]['hull_area_%'].values[0], 1)}%",
        color="w",
    )
    ...
    ax.text(
        0.91,
        0.71 - 0.22 * idx,
        f"{round(away_top_area.iloc[idx : idx + 1]['hull_area_%'].values[0], 1)}%",
        color="w",
    )

# Label based on include parameter
hull_include = (
    central_pct.replace("std", "") + " Std. Dev"
    if "std" in str(central_pct)
    else str(central_pct) + "%"
)
hull_include_s = (
    central_pct.replace("std", "") + " SD"
    if "std" in str(central_pct)
    else str(central_pct) + "%"
)
...
# Save image
fig.savefig(
    f"pass_reports/{league}-{match_id}-{home_team}-{away_team}-passhulls", dpi=300
)
...
ax11.set_title(
    "Pass Flow - Most frequent\ninter-zone passes",
    color="w",
    fontsize=10,
    fontweight="bold",
)
ax12.set_title(
    f"Variation in start position of player passes\nCentral {hull_include} passes shown per player",
    color="w",
    fontsize=10,
    fontweight="bold",
)
...
for idx in np.arange(0, 5):
    ...
    if len(home_player["name"].split(" ")) == 1:
        ...
        home_short_name = (
            f"{home_player['name'][0]}. {home_player['name'].split(' ')[-1]}"
        )
    if len(away_player["name"].split(" ")) == 1:
        away_short_name = f"{away_player['name']}"
    else:
        away_short_name = (
            f"{away_player['name'][0]}. {away_player['name'].split(' ')[-1]}"
        )
...
for idx, pass_count in enumerate(np.arange(1, 16, 2)):
    ...
    color_1 = cmaps[0](int(255 * min(1, pass_count / 15)))
    legend_ax_1.scatter(
        xpos, ypos, marker="H", s=350, color=color_1, edgecolor="w", lw=0.5
    )
    legend_ax_1.text(
        xpos, ypos, pass_count, color=text_color, ha="center", va="center", fontsize=9
    )
    legend_ax_1.text(
        4, -0.2, "Pass Count", color=text_color, ha="center", va="center", fontsize=9
    )
    color_2 = cmaps[1](int(255 * min(1, pass_count / 15)))
    legend_ax_2.scatter(
        xpos, ypos, marker="H", s=350, color=color_2, edgecolor="w", lw=0.5
    )
    legend_ax_2.text(
        xpos, ypos, pass_count, color=text_color, ha="center", va="center", fontsize=9
    )
...
for hull_idx, hull_row in offensive_hull_df.iterrows():
    ...
    if idx == 0:
        ...
        plot_hull = pitch.convexhull(
            hull_row["hull_reduced_x"], hull_row["hull_reduced_y"]
        )
        pitch.polygon(
            plot_hull,
            ax=ax12,
...
for idx, team in enumerate(players_df["teamId"].unique()):
    ...
    plot_z14_touch_pre_assists = z14_touch_pre_assists[
        z14_touch_pre_assists["teamId"] == team
    ]
    ...
    plot_hs_touch_pre_assists = hs_touch_pre_assists[
        hs_touch_pre_assists["teamId"] == team
    ]

    pitch.lines(
        plot_z14_passes["x"],
        plot_z14_passes["y"],
        plot_z14_passes["endX"],
        plot_z14_passes["endY"],
        lw=3,
        comet=True,
        capstyle="round",
    ...
    pitch.lines(
        plot_z14_suc_passes["x"],
        plot_z14_suc_passes["y"],
        plot_z14_suc_passes["endX"],
        plot_z14_suc_passes["endY"],
        lw=3,
        comet=True,
        capstyle="round",
    ...
    pitch.lines(
        plot_z14_prog_passes["x"],
        plot_z14_prog_passes["y"],
        plot_z14_prog_passes["endX"],
        plot_z14_prog_passes["endY"],
        lw=3,
        comet=True,
        capstyle="round",
    ...
    pitch.lines(
        plot_z14_assists["x"],
        plot_z14_assists["y"],
        plot_z14_assists["endX"],
        plot_z14_assists["endY"],
        lw=3,
        comet=True,
        capstyle="round",
        label="Assist",
    ...
    pitch.lines(
        plot_z14_pre_assists["x"],
        plot_z14_pre_assists["y"],
        plot_z14_pre_assists["endX"],
        plot_z14_pre_assists["endY"],
        lw=3,
        comet=True,
        capstyle="round",
        label="Pre-assist",
    ...
    pitch.scatter(
        plot_z14_touch_assists["x"],
        plot_z14_touch_assists["y"],
        color="magenta",
        alpha=0.8,
        s=15,
        zorder=5,
    ...
    pitch.scatter(
        plot_z14_touch_pre_assists["x"],
        plot_z14_touch_pre_assists["y"],
        color="lime",
        alpha=0.8,
        s=15,
        zorder=5,
    ...
    pitch.lines(
        plot_hs_suc_passes["x"],
        plot_hs_suc_passes["y"],
        plot_hs_suc_passes["endX"],
        plot_hs_suc_passes["endY"],
        lw=3,
        comet=True,
        capstyle="round",
        label="Successful Pass",
    ...
    pitch.lines(
        plot_hs_prog_passes["x"],
        plot_hs_prog_passes["y"],
        plot_hs_prog_passes["endX"],
        plot_hs_prog_passes["endY"],
        lw=3,
        comet=True,
        capstyle="round",
        label="Progressive Pass",
    ...
    pitch.lines(
        plot_hs_assists["x"],
        plot_hs_assists["y"],
        plot_hs_assists["endX"],
        plot_hs_assists["endY"],
        lw=3,
        comet=True,
        capstyle="round",
        label="Assist",
    ...
    pitch.lines(
        plot_hs_pre_assists["x"],
        plot_hs_pre_assists["y"],
        plot_hs_pre_assists["endX"],
        plot_hs_pre_assists["endY"],
        lw=3,
        comet=True,
        capstyle="round",
        label="Pre-assist",
    ...
    pitch.scatter(
        plot_hs_touch_assists["x"],
        plot_hs_touch_assists["y"],
        color="magenta",
        alpha=0.8,
        s=15,
        zorder=5,
    ...
    pitch.scatter(
        plot_hs_touch_pre_assists["x"],
        plot_hs_touch_pre_assists["y"],
        color="lime",
        alpha=0.8,
        s=15,
        zorder=5,
...
for idx in np.arange(0, 5):
    ...
    ax1.text(
        0.9,
        0.81 - 0.16 * idx,
        f"{int(home_player['prog_passes'] if home_player['prog_passes'] == home_player['prog_passes'] else 0)}",
        color="w",
    )
    ...
    ax2.text(
        0.9,
        0.81 - 0.16 * idx,
        f"{int(away_player['prog_passes'] if away_player['prog_passes'] == away_player['prog_passes'] else 0)}",
        color="w",
    )
...
h_pass_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["suc_passes"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["passes"],
    ),
    1,
)
h_fwd_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[home_team, :]["suc_fwd_passes"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["fwd_passes"],
    ),
    1,
)
h_prog_pct = round(
    ...
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[home_team, :]["suc_prog_passes"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["prog_passes"],
    ),
    1,
)
h_cross_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["suc_crosses"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["crosses"],
    ),
    1,
)
h_long_ball_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[home_team, :]["suc_long_balls"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["long_balls"],
    ),
    1,
)
h_through_ball_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[home_team, :]["suc_through_balls"],
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[home_team, :]["through_balls"],
    ),
    1,
)
a_pass_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["suc_passes"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["passes"],
    ),
    1,
)
a_fwd_pct = round(
    ...
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[away_team, :]["suc_fwd_passes"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["fwd_passes"],
    ),
    1,
)
a_prog_pct = round(
    ...
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[away_team, :]["suc_prog_passes"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["prog_passes"],
    ),
    1,
)
a_cross_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["suc_crosses"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["crosses"],
    ),
    1,
)
a_long_ball_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[away_team, :]["suc_long_balls"],
        playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["long_balls"],
    ),
    1,
)
a_through_ball_pct = round(
    100
    * protected_divide(
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[away_team, :]["suc_through_balls"],
        playerinfo_df.groupby(by="team", axis=0)
        .sum()
        .loc[away_team, :]["through_balls"],
    ),
    1,
)
...
ax1.text(
    0.6,
    0.65,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["passes"]),
    fontsize=10,
    color="white",
    ha="center",
)
ax1.text(
    0.775,
    0.65,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["suc_passes"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax1.text(
    0.6,
    0.52,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["fwd_passes"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax1.text(
    0.6,
    0.26,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["crosses"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax1.text(
    0.6,
    0.13,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[home_team, :]["long_balls"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax1.text(
    0.95, 0.13, str(h_long_ball_pct) + "%", fontsize=10, color="white", ha="center"
)
...
ax1.text(
    0.95, 0, str(h_through_ball_pct) + "%", fontsize=10, color="white", ha="center"
)
...
ax2.text(
    0.6,
    0.65,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["passes"]),
    fontsize=10,
    color="white",
    ha="center",
)
ax2.text(
    0.775,
    0.65,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["suc_passes"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax2.text(
    0.6,
    0.52,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["fwd_passes"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax2.text(
    0.6,
    0.26,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["crosses"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax2.text(
    0.6,
    0.13,
    int(playerinfo_df.groupby(by="team", axis=0).sum().loc[away_team, :]["long_balls"]),
    fontsize=10,
    color="white",
    ha="center",
)
...
ax2.text(
    0.95, 0.13, str(a_long_ball_pct) + "%", fontsize=10, color="white", ha="center"
)
...
ax2.text(
    0.95, 0, str(a_through_ball_pct) + "%", fontsize=10, color="white", ha="center"
)
...
# Save images
fig1.savefig(
    f"pass_reports/{league}-{match_id}-{home_team}-{away_team}-passreport_{home_team}",
    dpi=300,
)
fig2.savefig(
    f"pass_reports/{league}-{match_id}-{home_team}-{away_team}-passreport_{away_team}",
    dpi=300,
)
