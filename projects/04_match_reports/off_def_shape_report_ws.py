...
for player_id in players_df[players_df["longest_xi"] == True].index:
    player_def_hull = wce.create_convex_hull(
        ...
        min_events=5,
        include_events=central_pct_def,
        pitch_area=10000,
    )
    player_off_hull = wce.create_convex_hull(
        ...
        name=players_df.loc[player_id, "name"],
        min_events=5,
        include_events=central_pct_off,
        pitch_area=10000,
    )
...
for hull_idx, hull_row in offensive_hull_df.iterrows():
    ...
    if players_df[players_df["name"] == hull_idx]["team"].values[0] == plot_team:
        ...
        if len(hull_idx.split(" ")) == 1:
            ...
            initials = (
                hull_idx.split(" ")[0][0].upper() + hull_idx.split(" ")[1][0].upper()
            )
...
# Ax titles
ax["pitch"][0].set_title(
    f"{plot_team} Offensive Shape", pad=0, color="w", fontweight="bold"
)
ax["pitch"][1].set_title(
    f"{plot_team} Defensive Shape", pad=0, color="w", fontweight="bold"
)

# Label based on include parameter
hull_include = (
    central_pct_off.replace("std", "") + " Std. Dev"
    if "std" in str(central_pct_off)
    else str(central_pct_off) + "%"
)
hull_include_s = (
    central_pct_off.replace("std", "") + " SD"
    if "std" in str(central_pct_off)
    else str(central_pct_off) + "%"
)
