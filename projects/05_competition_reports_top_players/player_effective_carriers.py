...
playerinfo_df = wde.group_player_events(
    all_carries, playerinfo_df, primary_event_name="carries"
)
playerinfo_df = wde.group_player_events(
    all_carries, playerinfo_df, group_type="sum", event_types=["xThreat", "xThreat_gen"]
)

# Progressive passes
prog_carries = all_carries[all_carries["progressive_action"] == True]
playerinfo_df = wde.group_player_events(
    prog_carries, playerinfo_df, primary_event_name="prog_carries"
)

# Successful progressive carries in opposition half
opphalf_prog_carries = prog_carries[prog_carries["x"] > 50]
playerinfo_df = wde.group_player_events(
    opphalf_prog_carries, playerinfo_df, primary_event_name="opphalf_prog_carries"
)

# Carriers into opposition box
box_carries = all_carries[all_carries["box_entry"] == True]
playerinfo_df = wde.group_player_events(
    box_carries, playerinfo_df, primary_event_name="box_carries"
)
...
# Progressive Passes
playerinfo_df["prog_carries_pct"] = round(
    100 * playerinfo_df["prog_carries"] / playerinfo_df["carries"], 1
)
playerinfo_df["prog_carries_90"] = round(
    90 * playerinfo_df["prog_carries"] / playerinfo_df["mins_played"], 2
)

# Successful progressive passes in opposition half
playerinfo_df["opphalf_prog_carries_pct"] = round(
    100 * playerinfo_df["opphalf_prog_carries"] / playerinfo_df["carries"], 1
)
playerinfo_df["opphalf_prog_carries_90"] = round(
    90 * playerinfo_df["opphalf_prog_carries"] / playerinfo_df["mins_played"], 2
)

# Passes into opposition box
playerinfo_df["box_carries_pct"] = round(
    100 * playerinfo_df["box_carries"] / playerinfo_df["carries"], 1
)
playerinfo_df["box_carries_90"] = round(
    90 * playerinfo_df["box_carries"] / playerinfo_df["mins_played"], 2
)
...
if norm_mode == None:
    pp_sorted_df = playerinfo_df.sort_values(
        ["prog_carries", "prog_carries_90"], ascending=[False, False]
    )
    ...
    pp_sorted_df = playerinfo_df.sort_values(
        ["prog_carries_90", "prog_carries"], ascending=[False, False]
    )
    xt_sorted_df = playerinfo_df.sort_values(
...
for player_id, name in xt_sorted_df.head(12).iterrows():
    player_carries = all_carries[all_carries["playerId"] == player_id].sort_values(
...
# Save image
fig.savefig(
    f"player_effective_carriers/{league}-{year}-top-threat-generators-{run_date.replace('/', '_')}{file_pos_str.replace(' & ', '-').replace(' ', '-')}-{title_addition.replace(' ', '-')}",
    dpi=300,
)
