...
interception = events_df[
    (events_df["eventType"] == "Interception")
    & (events_df["outcomeType"] == "Successful")
]
playerinfo_df = wde.group_player_events(
    interception, playerinfo_df, primary_event_name="interception"
)
playerinfo_df["interception"].replace(np.nan, 0, inplace=True)
playerinfo_df["interception_90"] = round(
    90 * playerinfo_df["interception"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["interception_100opp_pass"] = round(
    100 * playerinfo_df["interception"] / playerinfo_df["opp_pass"], 2
)
...
playerinfo_df = wde.group_player_events(
    block, playerinfo_df, primary_event_name="block_pass"
)
playerinfo_df["block_pass"].replace(np.nan, 0, inplace=True)
playerinfo_df["block_pass_90"] = round(
    90 * playerinfo_df["block_pass"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["block_pass_100opp_pass"] = round(
    100 * playerinfo_df["block_pass"] / playerinfo_df["opp_pass"], 2
)
...
playerinfo_df = wde.group_player_events(
    tackle, playerinfo_df, primary_event_name="tackle"
)
playerinfo_df["tackle"].replace(np.nan, 0, inplace=True)
playerinfo_df["tackle_90"] = round(
    90 * playerinfo_df["tackle"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["tackle_100opp_pass"] = round(
    100 * playerinfo_df["tackle"] / playerinfo_df["opp_pass"], 2
)
...
playerinfo_df = wde.group_player_events(
    recovery, playerinfo_df, primary_event_name="recovery"
)
playerinfo_df["recovery"].replace(np.nan, 0, inplace=True)
playerinfo_df["recovery_90"] = round(
    90 * playerinfo_df["recovery"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["recovery_100opp_pass"] = round(
    100 * playerinfo_df["recovery"] / playerinfo_df["opp_pass"], 2
)

# %% Aerial

duel = events_df[
    (events_df["eventType"] == "Aerial") & (events_df["outcomeType"] == "Successful")
]
playerinfo_df = wde.group_player_events(
    duel, playerinfo_df, primary_event_name="duel_won"
)
playerinfo_df["duel_won"].replace(np.nan, 0, inplace=True)
playerinfo_df["duel_won_90"] = round(
    90 * playerinfo_df["duel_won"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["duel_won_100opp_pass"] = round(
    100 * playerinfo_df["duel_won"] / playerinfo_df["opp_pass"], 2
)
...
playerinfo_df["tackle_duel_90"] = (
    playerinfo_df["duel_won_90"] + playerinfo_df["tackle_90"]
)
playerinfo_df["tackle_duel_100opp_pass"] = (
    playerinfo_df["duel_won_100opp_pass"] + playerinfo_df["tackle_100opp_pass"]
)
playerinfo_df["interception_block_90"] = (
    playerinfo_df["interception_90"] + playerinfo_df["block_pass_90"]
)
playerinfo_df["interception_block_100opp_pass"] = (
    playerinfo_df["interception_100opp_pass"] + playerinfo_df["block_pass_100opp_pass"]
)
playerinfo_df["balls_won_from_opp_100opp_pas"] = (
    playerinfo_df["interception_block_100opp_pass"]
    + playerinfo_df["tackle_duel_100opp_pass"]
)
...
]  # | ((left_ax_norm_plot<plot_quantile_left[0]) | (right_ax_norm_plot<plot_quantile_right[0]))]
...
# Label axes
ax.axis["left"].set_label(
    "Balls Won Directly from opposition per 100 opposition touches"
)
...
# Save image
fig.savefig(
    f"top_defensive_contributions/{league}-{year}-defensive-contributions-player-variant",
    dpi=300,
)
...
for team in set(playerinfo_df["team"].values):
    ...
    team_players["lnorm"] = (
        0.99
        * team_players["balls_won_from_opp_100opp_pas"]
        / max(team_players["balls_won_from_opp_100opp_pas"])
    )
    team_players["rnorm"] = (
        0.99
        * team_players["recovery_100opp_pass"]
        / max(team_players["recovery_100opp_pass"])
    )
    team_players["norm"] = team_players["lnorm"] + team_players["rnorm"]
    team_players["lnorm_plot"] = (
        0.99
        * team_players["balls_won_from_opp_100opp_pas"]
        / max(playerinfo_df["balls_won_from_opp_100opp_pas"])
    )
    team_players["rnorm_plot"] = (
        0.99
        * team_players["recovery_100opp_pass"]
        / max(playerinfo_df["recovery_100opp_pass"])
    )
    ...
    if (
        len(
            set(
                team_players[team_players["norm"] == team_players["norm"].quantile(0.5)]
            )
        )
        == 2
    ):
        top_mid_low_player = team_players[
            (team_players["norm"] == team_players["norm"].quantile(0.5))
            | (team_players["norm"] == team_players["norm"].quantile(0))
            | (team_players["norm"] == team_players["norm"].quantile(1))
        ]
    else:
        top_mid_low_player = team_players[
            (team_players["norm"] == team_players["norm"].quantile(0))
        ...
        top_mid_low_player = pd.concat(
            [
                top_mid_low_player,
                pd.DataFrame(team_players.iloc[int(len(team_players) / 2), :]).T,
            ]
        )

    plot_player = pd.concat([plot_player, top_mid_low_player])

left_points = left_ax_norm_plot[
    (left_ax_norm_plot > left_ax_quantile[2])
    | (right_ax_norm_plot > right_ax_quantile[2])
    | (
        (left_ax_norm_plot < left_ax_quantile[0])
        | (right_ax_norm_plot < right_ax_quantile[0])
    )
]
right_points = right_ax_norm_plot[
    (left_ax_norm_plot > left_ax_quantile[2])
    | (right_ax_norm_plot > right_ax_quantile[2])
    | (
        (left_ax_norm_plot < left_ax_quantile[0])
...
# Save image
fig.savefig(
    f"top_defensive_contributions/{league}-{year}-defensive-contributions-team-variant",
    dpi=300,
)
