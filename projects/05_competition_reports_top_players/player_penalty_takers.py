...
event_types = bz2.BZ2File(
    f"../../data_directory/whoscored_data/{year}_{str(int(year.replace('20', '', 1)) + 1)}/{league}/event-types.pbz2",
    "rb",
)
event_types = pickle.load(event_types)
...
playerinfo_df_formatted = playerinfo_df_formatted[
    ~playerinfo_df_formatted.index.duplicated(keep="first")
]
...
penalty_events = events_df[
    (
        events_df["satisfiedEventsTypes"].apply(
            lambda x: 22 in x or 135 in x or 207 in x or 208 in x or 209 in x
        )
    )
]

playerinfo_df_formatted = wde.group_player_events(
    penalty_events, playerinfo_df_formatted, primary_event_name="penalties_taken"
)
playerinfo_df_formatted = wde.group_player_events(
    penalty_events[penalty_events["eventType"] == "Goal"],
    playerinfo_df_formatted,
    primary_event_name="penalties_scored",
)
playerinfo_df_formatted["penalty_conversion"] = (
    100
    * playerinfo_df_formatted["penalties_scored"]
    / playerinfo_df_formatted["penalties_taken"]
)
...
penalty_events["goal_marker"] = penalty_events["eventType"].apply(
    lambda x: 1 if x == "Goal" else 0
)
penalty_events.loc[:, "y_dist_from_gk"] = abs(
    (penalty_events["goalMouthY"] - 50) * goal_width_factor - gk_y_pos_yards
)
penalty_events.loc[:, "z_dist_from_gk"] = abs(
    penalty_events["goalMouthZ"] * goal_height_factor - gk_z_pos_yards
)
penalty_events.loc[:, "dist_from_gk"] = np.sqrt(
    penalty_events.loc[:, "y_dist_from_gk"] ** 2
    + penalty_events.loc[:, "z_dist_from_gk"] ** 2
)  # * penalty_events.loc[:,'goal_marker']

# Reset distance for off target shots
penalty_events.loc[
    penalty_events["eventType"].isin(["MissedShots", "ShotOnPost"]), "dist_from_gk"
] = 0
...
playerinfo_df_formatted = wde.group_player_events(
    ...
    playerinfo_df_formatted,
    group_type="mean",
    event_types="dist_from_gk",
)

# %% Filter out penalty takers

pentaker_df = playerinfo_df_formatted[
    playerinfo_df_formatted["penalties_taken"] >= min_pens
]
pentaker_df = pentaker_df.sort_values(
    ["dist_from_gk", "penalty_conversion"], ascending=[False, False]
)
...
for goal_ax in goal:
    ...
    # Add pitch colouring
    pitch_alpha = 0.5
    goal_ax.fill_between(
        (width_lower_lim, width_upper_lim),
        (0, 0),
        (-4.367, -4.367),
        color=(0.38612245, 0.77142857, 0.3744898, pitch_alpha),
        lw=0,
    )
    goal_ax.fill_between(
        (width_lower_lim, width_upper_lim),
        (-4.367, -4.367),
        (-10.175, -10.175),
        color=(0.25, 0.44, 0.12, pitch_alpha),
        lw=0,
    )
    goal_ax.fill_between(
        (width_lower_lim, width_upper_lim),
...
for player_id, player in pentaker_df.head(12).iterrows():
    ...
    goal[idx].set_title(
        f"  {idx + 1}: {player['name']}", loc="left", pad=12, color="w", fontsize=10
    )
    ...
    goal[idx].text(
        52,
        46.5,
        f"{round(player['dist_from_gk'], 2)}yds",
        ha="left",
        fontsize=7,
        color="w",
        zorder=4,
    ...
    goal[idx].text(
        45.4,
        46.5,
        f"{int(player['penalties_scored'])} / {int(player['penalties_taken'])}",
        ha="left",
        fontsize=7,
        color="w",
        zorder=4,
    ...
    for team in player["team"]:
        ...
        logo_ax = fig.add_axes(
            [ax_pos.x1 - 0.03 - logo_delta, ax_pos.y1 + 0.004, 0.035, 0.035]
        )
        logo_ax.axis("off")
...
title_text = (
    f"{title_str} {year_str}/{int(year_str) + 1} - Top 12 Players by Penalty Placement"
)
subtitle_text = (
    "Players ranked by mean distance of on-target penalties from Goalkeeper midriff"
)
...
for point in np.arange(0, 7):
    ...
    marker_colour = marker_cmap[int(255 * min(dist_norm, 1))]
    legend_ax.scatter(
        0.05 + point_delta, 0.5, color=marker_colour, edgecolor="#313332", s=700, lw=2
    )
    legend_ax.text(
        0.05 + point_delta,
        0.49,
...
fig.savefig(
    f"player_penalty_takers/{title_str.replace(' ', '_')}-{year_str}-top-penalty-takers",
    dpi=300,
)
