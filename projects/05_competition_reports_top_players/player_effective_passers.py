...
players_df = wde.events_while_playing(
    events_df, players_df, event_name="Pass", event_team="own"
)

# Add progressive pass and box entry information to event dataframe
events_df["progressive_action"] = events_df.apply(
    wce.progressive_action, axis=1, inplay=True, successful_only=False
)
events_df["into_box"] = events_df.apply(
    wce.box_entry, axis=1, inplay=True, successful_only=False
)
...
all_passes = events_df[
    (events_df["eventType"] == "Pass")
    & (
        ~events_df["satisfiedEventsTypes"].apply(
            lambda x: 31 in x or 32 in x or 33 in x or 34 in x or 212 in x
        )
    )
]
playerinfo_df = wde.group_player_events(
    all_passes, playerinfo_df, primary_event_name="passes"
)
playerinfo_df = wde.group_player_events(
    all_passes, playerinfo_df, group_type="sum", event_types=["xThreat", "xThreat_gen"]
)

# Successful passes
suc_passes = all_passes[all_passes["outcomeType"] == "Successful"]
playerinfo_df = wde.group_player_events(
    suc_passes, playerinfo_df, primary_event_name="suc_passes"
)

# Progressive passes
prog_passes = all_passes[all_passes["progressive_action"] == True]
playerinfo_df = wde.group_player_events(
    prog_passes, playerinfo_df, primary_event_name="prog_passes"
)
suc_prog_passes = prog_passes[prog_passes["outcomeType"] == "Successful"]
playerinfo_df = wde.group_player_events(
    suc_prog_passes, playerinfo_df, primary_event_name="suc_prog_passes"
)

# Successful progressive passes in opposition half
opphalf_prog_pass = suc_prog_passes[suc_prog_passes["x"] > 50]
playerinfo_df = wde.group_player_events(
    opphalf_prog_pass, playerinfo_df, primary_event_name="opphalf_prog_passes"
)

# Forward passes
forward_passes = all_passes[all_passes["endX"] > all_passes["x"]]
playerinfo_df = wde.group_player_events(
    forward_passes, playerinfo_df, primary_event_name="fwd_passes"
)
suc_forward_passes = forward_passes[forward_passes["outcomeType"] == "Successful"]
playerinfo_df = wde.group_player_events(
    suc_forward_passes, playerinfo_df, primary_event_name="suc_fwd_passes"
)

# Crosses
crosses = all_passes[
    all_passes["satisfiedEventsTypes"].apply(lambda x: 125 in x or 126 in x)
]
playerinfo_df = wde.group_player_events(
    crosses, playerinfo_df, primary_event_name="crosses"
)
suc_crosses = all_passes[all_passes["satisfiedEventsTypes"].apply(lambda x: 125 in x)]
playerinfo_df = wde.group_player_events(
    suc_crosses, playerinfo_df, primary_event_name="suc_crosses"
)

# Through balls
through_balls = all_passes[
    all_passes["satisfiedEventsTypes"].apply(lambda x: 129 in x or 130 in x or 131 in x)
]
playerinfo_df = wde.group_player_events(
    through_balls, playerinfo_df, primary_event_name="through_balls"
)
suc_through_balls = all_passes[
    all_passes["satisfiedEventsTypes"].apply(lambda x: 129 in x)
]
playerinfo_df = wde.group_player_events(
    suc_through_balls, playerinfo_df, primary_event_name="suc_through_balls"
)

# Long balls
long_balls = all_passes[
    all_passes["satisfiedEventsTypes"].apply(lambda x: 127 in x or 128 in x)
]
playerinfo_df = wde.group_player_events(
    long_balls, playerinfo_df, primary_event_name="long_balls"
)
suc_long_balls = all_passes[
    all_passes["satisfiedEventsTypes"].apply(lambda x: 127 in x)
]
playerinfo_df = wde.group_player_events(
    suc_long_balls, playerinfo_df, primary_event_name="suc_long_balls"
)

# Key passes
key_passes = all_passes[all_passes["satisfiedEventsTypes"].apply(lambda x: 123 in x)]
playerinfo_df = wde.group_player_events(
    key_passes, playerinfo_df, primary_event_name="key_passes"
)

# Key progressive passes
key_prog_passes = suc_prog_passes[
    suc_prog_passes["satisfiedEventsTypes"].apply(lambda x: 123 in x)
]
playerinfo_df = wde.group_player_events(
    key_prog_passes, playerinfo_df, primary_event_name="key_prog_passes"
)

# Assists
assists = all_passes[all_passes["satisfiedEventsTypes"].apply(lambda x: 92 in x)]
touch_assists = events_df[
    (events_df["eventType"] != "Pass")
    & (events_df["satisfiedEventsTypes"].apply(lambda x: 92 in x))
]
playerinfo_df = wde.group_player_events(
    assists, playerinfo_df, primary_event_name="assists"
)

# Pre assists
pre_assists = all_passes[all_passes["pre_assist"] == True]
playerinfo_df = wde.group_player_events(
    pre_assists, playerinfo_df, primary_event_name="pre_assists"
)

# Passes into opposition box
box_passes = all_passes[all_passes["into_box"] == True]
playerinfo_df = wde.group_player_events(
    box_passes, playerinfo_df, primary_event_name="box_passes"
)
suc_box_passes = box_passes[box_passes["outcomeType"] == "Successful"]
playerinfo_df = wde.group_player_events(
    suc_box_passes, playerinfo_df, primary_event_name="suc_box_passes"
)

# %% Calculate statistics as percentages and per 90 mins

# Passes and xT generated
playerinfo_df["passes_90"] = round(
    90 * playerinfo_df["passes"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["passes_100teampass"] = round(
    100 * playerinfo_df["passes"] / playerinfo_df["team_pass"], 1
)
playerinfo_df["xThreat_100pass"] = round(
    100 * playerinfo_df["xThreat"] / playerinfo_df["passes"], 2
)
playerinfo_df["xThreat_90"] = round(
    90 * playerinfo_df["xThreat"] / playerinfo_df["mins_played"], 3
)
playerinfo_df["xThreat_100teampass"] = round(
    100 * playerinfo_df["xThreat"] / playerinfo_df["team_pass"], 3
)
playerinfo_df["xThreat_gen_100pass"] = round(
    100 * playerinfo_df["xThreat_gen"] / playerinfo_df["passes"], 2
)
playerinfo_df["xThreat_gen_90"] = round(
    90 * playerinfo_df["xThreat_gen"] / playerinfo_df["mins_played"], 3
)
playerinfo_df["xThreat_gen_100teampass"] = round(
    100 * playerinfo_df["xThreat_gen"] / playerinfo_df["team_pass"], 3
)

# Successful Passes
playerinfo_df["suc_passes_pct"] = round(
    100 * playerinfo_df["suc_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["suc_passes_90"] = round(
    90 * playerinfo_df["suc_passes"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["suc_passes_100teampass"] = round(
    100 * playerinfo_df["suc_passes"] / playerinfo_df["team_pass"], 1
)

# Progressive Passes
playerinfo_df["prog_passes_pct"] = round(
    100 * playerinfo_df["prog_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["prog_passes_90"] = round(
    90 * playerinfo_df["prog_passes"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["prog_passes_100teampass"] = round(
    100 * playerinfo_df["prog_passes"] / playerinfo_df["team_pass"], 1
)
playerinfo_df["suc_prog_passes_pct"] = round(
    100 * playerinfo_df["suc_prog_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["suc_prog_passes_pctpp"] = round(
    100 * playerinfo_df["suc_prog_passes"] / playerinfo_df["prog_passes"], 1
)
playerinfo_df["suc_prog_passes_90"] = round(
    90 * playerinfo_df["suc_prog_passes"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["suc_prog_passes_100teampass"] = round(
    100 * playerinfo_df["suc_prog_passes"] / playerinfo_df["team_pass"], 1
)

# Successful progressive passes in opposition half
playerinfo_df["opphalf_prog_passes_pct"] = round(
    100 * playerinfo_df["opphalf_prog_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["opphalf_prog_passes_90"] = round(
    90 * playerinfo_df["opphalf_prog_passes"] / playerinfo_df["mins_played"], 2
)

# Forward passes
playerinfo_df["fwd_passes_pct"] = round(
    100 * playerinfo_df["fwd_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["fwd_passes_90"] = round(
    90 * playerinfo_df["fwd_passes"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["suc_fwd_passes_pct"] = round(
    100 * playerinfo_df["suc_fwd_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["suc_fwd_passes_pctfwd"] = round(
    100 * playerinfo_df["suc_fwd_passes"] / playerinfo_df["fwd_passes"], 1
)
playerinfo_df["suc_fwd_passes_90"] = round(
    90 * playerinfo_df["suc_fwd_passes"] / playerinfo_df["mins_played"], 2
)

# Crosses
playerinfo_df["crosses_pct"] = round(
    100 * playerinfo_df["crosses"] / playerinfo_df["passes"], 1
)
playerinfo_df["crosses_90"] = round(
    90 * playerinfo_df["crosses"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["suc_crosses_pct"] = round(
    100 * playerinfo_df["suc_crosses"] / playerinfo_df["passes"], 1
)
playerinfo_df["suc_crosses_pctcross"] = round(
    100 * playerinfo_df["suc_crosses"] / playerinfo_df["crosses"], 1
)
playerinfo_df["suc_crosses_90"] = round(
    90 * playerinfo_df["suc_crosses"] / playerinfo_df["mins_played"], 2
)

# Through balls
playerinfo_df["through_balls_pct"] = round(
    100 * playerinfo_df["through_balls"] / playerinfo_df["passes"], 1
)
playerinfo_df["through_balls_90"] = round(
    90 * playerinfo_df["through_balls"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["suc_through_balls_pct"] = round(
    100 * playerinfo_df["suc_through_balls"] / playerinfo_df["passes"], 1
)
playerinfo_df["suc_through_balls_pcttb"] = round(
    100 * playerinfo_df["suc_through_balls"] / playerinfo_df["through_balls"], 1
)
playerinfo_df["suc_through_balls_90"] = round(
    90 * playerinfo_df["suc_through_balls"] / playerinfo_df["mins_played"], 2
)

# Long balls
playerinfo_df["long_balls_pct"] = round(
    100 * playerinfo_df["long_balls"] / playerinfo_df["passes"], 1
)
playerinfo_df["long_balls_90"] = round(
    90 * playerinfo_df["long_balls"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["suc_long_balls_pct"] = round(
    100 * playerinfo_df["suc_long_balls"] / playerinfo_df["passes"], 1
)
playerinfo_df["suc_long_balls_pctlb"] = round(
    100 * playerinfo_df["suc_long_balls"] / playerinfo_df["long_balls"], 1
)
playerinfo_df["suc_long_balls_90"] = round(
    90 * playerinfo_df["suc_long_balls"] / playerinfo_df["mins_played"], 2
)

# Key Passes
playerinfo_df["key_passes_pct"] = round(
    100 * playerinfo_df["key_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["key_passes_90"] = round(
    90 * playerinfo_df["key_passes"] / playerinfo_df["mins_played"], 2
)

# Key Progressive Passes
playerinfo_df["key_prog_passes_pct"] = round(
    100 * playerinfo_df["key_prog_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["key_prog_passes_90"] = round(
    90 * playerinfo_df["key_prog_passes"] / playerinfo_df["mins_played"], 2
)

# Assists
playerinfo_df["assists_90"] = round(
    90 * playerinfo_df["assists"] / playerinfo_df["mins_played"], 2
)

# Pre-assists
playerinfo_df["pre_assists_90"] = round(
    90 * playerinfo_df["pre_assists"] / playerinfo_df["mins_played"], 2
)

# Passes into opposition box
playerinfo_df["box_passes_pct"] = round(
    100 * playerinfo_df["box_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["box_passes_90"] = round(
    90 * playerinfo_df["box_passes"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["suc_box_passes_pct"] = round(
    100 * playerinfo_df["suc_box_passes"] / playerinfo_df["passes"], 1
)
playerinfo_df["suc_box_passes_pctbp"] = round(
    100 * playerinfo_df["suc_box_passes"] / playerinfo_df["box_passes"], 1
)
playerinfo_df["suc_box_passes_90"] = round(
    90 * playerinfo_df["suc_box_passes"] / playerinfo_df["mins_played"], 2
)

# %% Filter playerinfo

playerinfo_df = playerinfo_df[
    (playerinfo_df["mins_played"] >= min_mins)
    & (~playerinfo_df["pos_type"].isin(pos_exclude))
]
...
if norm_mode == None:
    pp_sorted_df = playerinfo_df.sort_values(
        ["suc_prog_passes", "suc_prog_passes_90"], ascending=[False, False]
    )
    xt_sorted_df = playerinfo_df.sort_values(
        ["xThreat_gen", "xThreat_gen_90"], ascending=[False, False]
    )
elif norm_mode == "_90":
    pp_sorted_df = playerinfo_df.sort_values(
        ["suc_prog_passes_90", "suc_prog_passes"], ascending=[False, False]
    )
    xt_sorted_df = playerinfo_df.sort_values(
        ["xThreat_gen_90", "xThreat_gen"], ascending=[False, False]
    )
elif norm_mode == "_100pass":
    pp_sorted_df = playerinfo_df.sort_values(
        ["suc_prog_passes_pct", "suc_prog_passes"], ascending=[False, False]
    )
    xt_sorted_df = playerinfo_df.sort_values(
        ["xThreat_gen_100pass", "xThreat_gen"], ascending=[False, False]
    )
elif norm_mode == "_100teampass":
    pp_sorted_df = playerinfo_df.sort_values(
        ["suc_prog_passes_100teampass", "suc_prog_passes"], ascending=[False, False]
    )
    xt_sorted_df = playerinfo_df.sort_values(
        ["xThreat_gen_100teampass", "xThreat_gen"], ascending=[False, False]
    )
...
plot_player = playerinfo_df[
    (left_ax_norm_plot > plot_quantile_left[2])
    | (right_ax_norm_plot > plot_quantile_right[2])
]
...
ax.axis["bottom"].set_label(
    "Successful in-play progressive passes within opp. half per 90mins"
)
...
text = list()
path_eff = [
    path_effects.Stroke(linewidth=1.5, foreground="#313332"),
    path_effects.Normal(),
]
for i, player in plot_player.iterrows():
    format_name = (
        ...
        + " "
        + player["name"].split(" ")[len(player["name"].split(" ")) - 1]
        if len(player["name"].split(" ")) > 1
        else player["name"]
    )
    text.append(
        aux_ax.text(
            right_ax_norm_plot[i] + 0.01,
            left_ax_norm_plot[i],
            format_name,
            color="w",
            fontsize=7,
...
aux_ax.plot(
    [right_ax_quantile[0], right_ax_quantile[0]],
    [0, left_ax_quantile[0]],
    color="w",
    lw=1,
    alpha=0.3,
    ls="dashed",
...
aux_ax.plot(
    [right_ax_quantile[2], right_ax_quantile[2]],
    [0, left_ax_quantile[0]],
    color="w",
    lw=1,
    alpha=0.3,
...
aux_ax.fill(
    [0, right_ax_quantile[0], right_ax_quantile[0], 0],
    [
        left_ax_quantile[0],
        left_ax_quantile[0],
        left_ax_quantile[2],
...
aux_ax.fill(
    [right_ax_quantile[2], 100, 100, right_ax_quantile[2]],
    [
        left_ax_quantile[0],
        left_ax_quantile[0],
        left_ax_quantile[2],
...
aux_ax.plot(
    [0, right_ax_quantile[0]],
    [left_ax_quantile[0], left_ax_quantile[0]],
    color="w",
    lw=1,
    alpha=0.3,
    ls="dashed",
...
aux_ax.plot(
    [0, right_ax_quantile[0]],
    [left_ax_quantile[2], left_ax_quantile[2]],
    color="w",
    lw=1,
    alpha=0.3,
    ls="dashed",
...
text_ax_left.text(
    0.23,
    0.78,
    "Players towards this side\nof the grid frequently\npass the ball into the\nopposition box successfully",
    ha="center",
    fontsize=8,
    alpha=0.8,
)
...
# Save image
fig.savefig(
    f"player_effective_passers/{league}-{year}-diamond-{run_date.replace('/', '_')}-{left_metric}-vs-{right_metric}-player-variant",
    dpi=300,
)
...
for player_id, name in pp_sorted_df.head(12).iterrows():
    ...
    ax["pitch"][idx].set_title(
        f"  {idx + 1}: {name['name']}", loc="left", color="w", fontsize=10
    )
    ...
    pitch.lines(
        player_key_passes["x"],
        player_key_passes["y"],
        player_key_passes["endX"],
        player_key_passes["endY"],
        lw=2,
        comet=True,
        capstyle="round",
        label="Assists",
    ...
    pitch.scatter(
        player_touch_assists["x"],
        player_touch_assists["y"],
        color="magenta",
        alpha=0.8,
        s=12,
        zorder=3,
    ...
    ax["pitch"][idx].text(
        2, 4, "Total:", fontsize=8, fontweight="bold", color="w", zorder=3
    )
    ax["pitch"][idx].text(
        15, 4, f"{int(name['suc_prog_passes'])}", fontsize=8, color="w", zorder=3
    )

    if norm_mode == "_90":
        ax["pitch"][idx].text(
            2, 12, "/90 mins:", fontsize=8, fontweight="bold", color="w", zorder=3
        )
        ax["pitch"][idx].text(
            24, 12, f"{name['suc_prog_passes_90']}", fontsize=8, color="w", zorder=3
        )
    elif norm_mode == "_100pass":
        ax["pitch"][idx].text(
            2, 12, "/100 pass:", fontsize=8, fontweight="bold", color="w", zorder=3
        )
        ax["pitch"][idx].text(
            26,
            12,
            f"{round(name['suc_prog_passes_pct'], 1)}",
            fontsize=8,
            color="w",
            zorder=3,
        )
        ...
        ax["pitch"][idx].text(
            2, 12, "/100 team pass:", fontsize=8, fontweight="bold", color="w", zorder=3
        )
        ax["pitch"][idx].text(
            38,
            12,
            f"{round(name['suc_prog_passes_100teampass'], 1)}",
            fontsize=8,
            color="w",
            zorder=3,
        )
...
subtitle_text = (
    f"<Successful Progressive Passes>, <Key Progressive Passes> and <Assists>"
)
...
# Save image
fig.savefig(
    f"player_effective_passers/{league}-{year}-top-progressive-passers-{run_date.replace('/', '_')}{file_pos_str.replace(' & ', '-').replace(' ', '-')}-{title_addition.replace(' ', '-')}",
    dpi=300,
)
...
for player_id, name in xt_sorted_df.head(12).iterrows():
    player_passes = suc_passes[suc_passes["playerId"] == player_id].sort_values(
        "xThreat_gen", ascending=True
    )
    ...
    ax["pitch"][idx].text(
        15, 4, f"{round(name['xThreat_gen'], 2)}", fontsize=8, color="w", zorder=3
    )
    if norm_mode == "_90":
        ax["pitch"][idx].text(
            2, 12, "/90 mins:", fontsize=8, fontweight="bold", color="w", zorder=3
        ...
        ax["pitch"][idx].text(
            24, 12, f"{name['xThreat_gen_90']}", fontsize=8, color="w", zorder=3
        )
    if norm_mode == "_100pass":
        ...
        ax["pitch"][idx].text(
            26,
            12,
            f"{round(name['xThreat_gen_100pass'], 3)}",
            fontsize=8,
            color="w",
            zorder=3,
        )
        ...
        ax["pitch"][idx].text(
            38,
            12,
            f"{round(name['xThreat_gen_100teampass'], 3)}",
            fontsize=8,
            color="w",
            zorder=3,
        )
...
# Save image
fig.savefig(
    f"player_effective_passers/{league}-{year}-top-threat-generators-{run_date.replace('/', '_')}{file_pos_str.replace(' & ', '-').replace(' ', '-')}-{title_addition.replace(' ', '-')}",
    dpi=300,
)
