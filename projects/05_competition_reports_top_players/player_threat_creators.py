...
events_df["progressive"] = events_df.apply(
    wce.progressive_action, axis=1, inplay=True, successful_only=True
)
events_df["box_entry"] = events_df.apply(
    wce.box_entry, axis=1, inplay=True, successful_only=True
)
...
playerinfo_df = wde.group_player_events(
    ...
    playerinfo_df,
    group_type="sum",
    agg_columns=["xThreat", "xThreat_gen"],
    col_names=["xThreat_carry", "xThreat_gen_carry"],
)
...
playerinfo_df = wde.group_player_events(
    ...
    group_type="sum",
    agg_columns=["xThreat", "xThreat_gen"],
    col_names=["xThreat_pass", "xThreat_gen_pass"],
)

# Box entries and progresive actions
playerinfo_df = wde.group_player_events(
    all_passes[all_passes["box_entry"] == True],
    playerinfo_df,
    col_names="pass_into_box",
)
playerinfo_df = wde.group_player_events(
    all_carries[all_carries["box_entry"] == True],
    playerinfo_df,
    col_names="carry_into_box",
)
playerinfo_df = wde.group_player_events(
    all_passes[all_passes["progressive"] == True],
    playerinfo_df,
    col_names="progressive_pass",
)
playerinfo_df = wde.group_player_events(
    all_carries[all_carries["progressive"] == True],
    playerinfo_df,
    col_names="progressive_carry",
)

# Aggregate carries
playerinfo_df["carries_90"] = round(
    90 * playerinfo_df["carries"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["xThreat_carry_90"] = round(
    90 * playerinfo_df["xThreat_carry"] / playerinfo_df["mins_played"], 3
)
playerinfo_df["xThreat_gen_carry_90"] = round(
    90 * playerinfo_df["xThreat_gen_carry"] / playerinfo_df["mins_played"], 3
)
playerinfo_df["box_carries_90"] = round(
    90 * playerinfo_df["carry_into_box"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["prog_carries_90"] = round(
    90 * playerinfo_df["progressive_carry"] / playerinfo_df["mins_played"], 2
)
...
playerinfo_df["xThreat_pass_90"] = round(
    90 * playerinfo_df["xThreat_pass"] / playerinfo_df["mins_played"], 3
)
playerinfo_df["xThreat_gen_pass_90"] = round(
    90 * playerinfo_df["xThreat_gen_pass"] / playerinfo_df["mins_played"], 3
)
playerinfo_df["box_passes_90"] = round(
    90 * playerinfo_df["pass_into_box"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["prog_passes_90"] = round(
    90 * playerinfo_df["progressive_pass"] / playerinfo_df["mins_played"], 2
)

# Aggregate carries + passes
playerinfo_df["box_actions"] = (
    playerinfo_df["pass_into_box"] + playerinfo_df["carry_into_box"]
)
playerinfo_df["box_actions_90"] = (
    playerinfo_df["box_passes_90"] + playerinfo_df["box_carries_90"]
)
playerinfo_df["prog_actions_90"] = (
    playerinfo_df["prog_passes_90"] + playerinfo_df["prog_carries_90"]
)
...
for player_id, player_details in playerinfo_df.iterrows():
    ...
    bin_statistic = pitch.bin_statistic(
        ...
        player_events["y"],
        statistic="sum",
        bins=(12, 10) if grid_density == "dense" else (6, 5),
        normalize=False,
        values=player_events["xThreat_gen"],
    )
    if player_details["mins_played"] != 0:
        bin_statistic["statistic"] = (
            90 * bin_statistic["statistic"] / player_details["mins_played"]
        )
    else:
        bin_statistic["statistic"] = np.zeros(bin_statistic["statistic"].shape)
    for idx, zone_threat in enumerate(
        bin_statistic["statistic"].reshape(-1, order="F")
    ):
...
text_ax_left.text(
    0.08,
    0.73,
    "Players towards this\nside of the grid\ncreate significant threat\nthrough playing passes",
    ha="center",
    fontsize=8,
    alpha=0.8,
)
...
# Save image
fig.savefig(
    f"player_threat_creators/{league}-{year}-diamond-{run_date.replace('/', '_')}-{left_metric}-vs-{right_metric}-player-variant",
    dpi=300,
)
...
for idx in np.arange(0, len(bin_statistic["statistic"].reshape(-1))):
    ...
    if pitch_mode == "3" and grid_density != "dense":
        ...
        for p_id, p_info in playerinfo_df.head(3).iterrows():
            ...
            format_name = (
                player_name.split(" ")[0][0]
                + ". "
                + player_name.split(" ")[len(player_name.split(" ")) - 1]
                if len(player_name.split(" ")) > 1
                else player_name
            )
            format_name = (
                format_name if len(format_name) <= 13 else format_name[0:11] + "..."
            )
            format_text = (
            ...
            ab = AnnotationBbox(
                ...
                (ys[idx] + 6.75, xs[idx] + 5 - cnt),
                frameon=False,
            )
            ax["pitch"].add_artist(ab)
            pitch.annotate(
                text=format_text,
                xy=(xs[idx] + 5 - cnt, ys[idx] + 3.5),
                ha="left",
                va="center",
                ax=ax["pitch"],
                fontsize=6,
        ...
        format_name = (
            format_name
            if len(format_name) <= 14
            else format_name[0:14] + "\n" + format_name[14:]
        )
        format_text = (
            format_name
            + "\n"
            + "xT: "
            + str(round(playerinfo_df.head(1)[f"zone_{idx}_xT"].values[0], 3))
        )
        team_logo, _ = lab.get_team_badge_and_colour(
            playerinfo_df.head(1)["team"].values[0]
        )
        title_plural = ""
        if grid_density == "dense":
            ...
            ab = AnnotationBbox(
                OffsetImage(team_logo, zoom=0.1, resample=True),
                (ys[idx], xs[idx] + 2.5),
                frameon=False,
            )
            pitch.annotate(
                text=format_text,
                xy=(xs[idx] - 2.5, ys[idx]),
                ha="center",
                va="top",
                ax=ax["pitch"],
                fontsize=8,
...
# Save image
fig.savefig(
    f"player_threat_creators/{league}-{year}-top_threat_creator_by_zone-{file_ext}-{run_date.replace('/', '_')}",
    dpi=300,
)
...
for player_id, name in be_sorted_df.head(20).iterrows():
    # Get box entries
    player_box_entries = events_df[
        (events_df["box_entry"] == True) & (events_df["playerId"] == player_id)
    ]
    ...
    format_name = (
        name["name"].split(" ")[0][0]
        + " "
        + name["name"].split(" ")[len(name["name"].split(" ")) - 1]
        if len(name["name"].split(" ")) > 1
        else name["name"]
    )
    ax["pitch"][idx].set_title(
        f"  {idx + 1}: {format_name}", loc="left", color="w", fontsize=10, pad=-2
    )

    # Plot density of start positions of box entries
    kde_plot = pitch.kdeplot(
        player_box_entries["x"],
        player_box_entries["y"],
        ax=ax["pitch"][idx],
        fill=True,
        levels=100,
        shade_lowest=True,
    ...
    ax["pitch"][idx].fill(
        [21.1, 78.9, 78.9, 21.1], [83, 83, 100, 100], "#313332", alpha=1, zorder=0
    )

    # Add text
    ax["pitch"][idx].text(
        96, 58, "Total:", fontsize=8, fontweight="bold", color="w", zorder=3
    )
    ax["pitch"][idx].text(
        96, 53, f"{int(name['box_actions'])}", fontsize=8, color="w", zorder=3
    )
    if norm_mode == "_90":
        ax["pitch"][idx].text(
            4,
...
title_text = (
    f"{leagues[league]} {year}/{int(year) + 1} − Players Ranked by Box Entries per 90"
)
subtitle_text = (
    "Heatmaps showing <Distribution> in starting position of player box entries"
)
