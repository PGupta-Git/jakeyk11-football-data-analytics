...
playerinfo_df = wde.create_player_list(
    players_df, additional_cols=["opp_pass", "oppthird_opp_pass"]
)
...
playerinfo_df = wde.group_player_events(
    all_def_actions, playerinfo_df, primary_event_name="def_actions"
)

# Aggregate defensive actions in opp half
oppthird_def_actions = all_def_actions[all_def_actions["x"] > 67]
playerinfo_df = wde.group_player_events(
    oppthird_def_actions, playerinfo_df, primary_event_name="oppthird_def_actions"
)

# Defensive actions per 100 opp passes and per 90 mins
playerinfo_df["def_actions_100opp_pass"] = round(
    100 * playerinfo_df["def_actions"] / playerinfo_df["opp_pass"], 2
)
playerinfo_df["def_actions_90"] = round(
    90 * playerinfo_df["def_actions"] / playerinfo_df["mins_played"], 2
)
playerinfo_df["oppthird_def_actions_100opp_pass"] = round(
    100 * playerinfo_df["oppthird_def_actions"] / playerinfo_df["oppthird_opp_pass"], 2
)
playerinfo_df["oppthird_def_actions_90"] = round(
    90 * playerinfo_df["oppthird_def_actions"] / playerinfo_df["mins_played"], 2
)

# %% Player removal

playerinfo_reduced_df = playerinfo_df[
    (playerinfo_df["position"].isin(pos_exclude) == False)
...
if norm_mode == None:
    sorted_df = playerinfo_reduced_df.sort_values(
        ["oppthird_def_actions", "oppthird_def_actions_100opp_pass"],
        ascending=[False, False],
    )
elif norm_mode == "_90":
    sorted_df = playerinfo_reduced_df.sort_values(
        ["oppthird_def_actions_90", "oppthird_def_actions_100opp_pass"],
        ascending=[False, False],
    )
elif norm_mode == "_100opp_pass":
    sorted_df = playerinfo_reduced_df.sort_values(
        ["oppthird_def_actions_100opp_pass", "oppthird_def_actions_90"],
        ascending=[False, False],
    )
...
for player_id, name in sorted_df.head(12).iterrows():
    ...
    player_opp_third_def_actions = oppthird_def_actions[
        oppthird_def_actions["playerId"] == player_id
    ]
    ...
    pitch.kdeplot(
        player_def_actions["x"],
        player_def_actions["y"],
        ax=ax["pitch"][idx],
        fill=True,
        levels=80,
        shade_lowest=True,
    ...
    pitch.scatter(
        player_opp_third_def_actions["x"],
        player_opp_third_def_actions["y"],
        color="w",
        alpha=0.6,
        s=12,
        zorder=1,
    ...
    ax["pitch"][idx].text(
        0, -8, "Opp. 3rd Actions:", fontsize=8, fontweight="bold", color="w", zorder=1
    )
    ax["pitch"][idx].text(
        39, -8, f"{int(name['oppthird_def_actions'])}", fontsize=8, color="w", zorder=1
    )
    ...
    if norm_mode == "_100opp_pass":
        ...
        ax["pitch"][idx].text(
            95,
            -8,
            f"{round(name['oppthird_def_actions_100opp_pass'], 1)}",
            fontsize=8,
            color="w",
            zorder=1,
        )
    ...
    if norm_mode == "_90":
        ax["pitch"][idx].text(
            50, -8, "Per 90 Mins:", fontsize=8, fontweight="bold", color="w", zorder=1
        )
        ax["pitch"][idx].text(
            85,
            -8,
            f"{round(name['oppthird_def_actions_90'], 1)}",
            fontsize=8,
            color="w",
            zorder=1,
        )
...
# Save image
fig.savefig(
    f"top_defensive_actions/{league}-{year}-top-defensive-actions{file_pos_str.replace(' & ', '-').replace(' ', '-')}-{title_addition.replace(' ', '-')}",
    dpi=300,
)
