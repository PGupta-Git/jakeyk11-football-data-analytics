...
for team_id in playerinfo_df["teamId"].unique():
    team_players_dict[team_id] = (
        playerinfo_df[playerinfo_df["teamId"] == team_id].index.unique().tolist()
    )
...
for match in players_df["match_id"].unique():
    for team_id in players_df[players_df["match_id"] == match]["teamId"].unique():
        team_players_in_squad = players_df[
            (players_df["match_id"] == match) & (players_df["teamId"] == team_id)
        ]
            ...
            if player_id in team_players_in_squad.index.tolist():
                ...
                playerinfo_list.append(
                    [
                        ...
                        match,
                        player["time_on"].values[0]
                        if player["time_on"].values[0] == player["time_on"].values[0]
                        else 0,
                        player["time_off"].values[0]
                        if player["time_off"].values[0] == player["time_off"].values[0]
                        else 0,
                        player["mins_played"].values[0]
                        if player["mins_played"].values[0]
                        == player["mins_played"].values[0]
                        else 0,
                        match_mins - player["mins_played"].values[0]
                        if player["mins_played"].values[0]
                        == player["mins_played"].values[0]
                        else match_mins,
                    ]
                ...
                playerinfo_list.append(
                    [
                        players_df[players_df.index == player_id]["name"].values[0],
                        "Out",
                        np.nan,
                        False,
                        team_id,
...
full_players_df = pd.DataFrame(
    data=playerinfo_list,
    columns=[
        "name",
        "position",
        "isFirstEleven",
...
for idx, lineup_player in full_players_df.iterrows():
    ...
    ip_events_when_on = match_events[
        (match_events["cumulative_mins"] >= lineup_player["time_on"])
        & (match_events["cumulative_mins"] <= lineup_player["time_off"])
        & (
            match_events["satisfiedEventsTypes"].apply(
                lambda x: not any(
                    item in x for item in [48, 50, 51, 42, 44, 45, 31, 34, 212]
                )
            )
        )
    ]
    ip_events_when_off = match_events[
        (
            (match_events["cumulative_mins"] < lineup_player["time_on"])
            | (match_events["cumulative_mins"] > lineup_player["time_off"])
        )
    ...
    ip_team_events_when_on = ip_events_when_on[
        ip_events_when_on["teamId"] == lineup_player["teamId"]
    ]
    ip_opp_events_when_on = ip_events_when_on[
        ip_events_when_on["teamId"] != lineup_player["teamId"]
    ]
    ip_team_events_when_off = ip_events_when_off[
        ip_events_when_off["teamId"] == lineup_player["teamId"]
    ]
    ip_opp_events_when_off = ip_events_when_off[
        ip_events_when_off["teamId"] != lineup_player["teamId"]
    ]

    xT_gen_list_on.append(ip_team_events_when_on["xThreat_gen"].sum())
    xT_conc_list_on.append(ip_opp_events_when_on["xThreat_gen"].sum())
    shots_for_list_on.append(
        ip_team_events_when_on[
            ip_team_events_when_on["eventType"].isin(
                ["MissedShots", "SavedShot", "Goal"]
            )
        ]["id"].count()
    )
    shots_against_list_on.append(
        ip_opp_events_when_on[
            ip_opp_events_when_on["eventType"].isin(
                ["MissedShots", "SavedShot", "Goal"]
            )
        ]["id"].count()
    )
    ...
    shots_for_list_off.append(
        ip_team_events_when_off[
            ip_team_events_when_off["eventType"].isin(
                ["MissedShots", "SavedShot", "Goal"]
            )
        ]["id"].count()
    )
    shots_against_list_off.append(
        ip_opp_events_when_off[
            ip_opp_events_when_off["eventType"].isin(
                ["MissedShots", "SavedShot", "Goal"]
            )
        ]["id"].count()
    )
...
playerinfo_df = playerinfo_df[
    (~playerinfo_df["position"].isin(pos_exclude))
    & (playerinfo_df["mins_not_played"] >= min_mnp)
]

# %% Normalise per 90 and split into valid offensive and defensive positions

playerinfo_df["increase_team_xT_90_on"] = (
    90 * playerinfo_df["team_xT_on"] / playerinfo_df["mins_played"]
) - (90 * playerinfo_df["team_xT_off"] / playerinfo_df["mins_not_played"])
playerinfo_df["increase_team_shots_90_on"] = (
    90 * playerinfo_df["team_shots_on"] / playerinfo_df["mins_played"]
) - (90 * playerinfo_df["team_shots_off"] / playerinfo_df["mins_not_played"])
playerinfo_df["decrease_opp_xT_90_on"] = (
    90 * playerinfo_df["opp_xT_off"] / playerinfo_df["mins_not_played"]
) - (90 * playerinfo_df["opp_xT_on"] / playerinfo_df["mins_played"])
playerinfo_df["decrease_opp_shots_90_on"] = (
    90 * playerinfo_df["opp_shots_off"] / playerinfo_df["mins_not_played"]
) - (90 * playerinfo_df["opp_shots_on"] / playerinfo_df["mins_played"])
playerinfo_df["increase_delta_xT_90_on"] = (
    90
    * (playerinfo_df["team_xT_on"] - playerinfo_df["opp_xT_on"])
    / playerinfo_df["mins_played"]
) - (
    90
    * (playerinfo_df["team_xT_off"] - playerinfo_df["opp_xT_off"])
    / playerinfo_df["mins_not_played"]
)

att_playerinfo_df = playerinfo_df[
    playerinfo_df["position"].isin(
        [
            "FW",
            "FWL",
            "FWR",
...
def_playerinfo_df = playerinfo_df[
    playerinfo_df["position"].isin(["DMC", "DMR", "DML", "MC", "DC", "DR", "DL"])
]
...
att_playerinfo_df = att_playerinfo_df.sort_values(
    "increase_team_xT_90_on", ascending=False
)
...
plot_df = pd.concat(
    [
        att_playerinfo_df.head(n_players),
        empty_df,
        att_playerinfo_df.tail(n_players),
        empty_df,
    ]
)
...
color_scale = (
    255
    * (plot_df["increase_team_xT_90_on"] - plot_df["increase_team_xT_90_on"].min())
    / (
        plot_df["increase_team_xT_90_on"].max()
        - plot_df["increase_team_xT_90_on"].min()
    )
)
...
ylim = ax.get_ylim()
xlim = ax.set_xlim(
    [-max([abs(xlim[0]), abs(xlim[1])]), max([abs(xlim[0]), abs(xlim[1])])]
)
...
subtitle_text = (
    "A Comparison of Team Threat Creation when Player is On vs. Off the Pitch"
)
...
# Define axes
ax.set_xlabel(
    "Improvement in total xT created by team when on pitch (ΔxT For/90)",
    labelpad=10,
    fontweight="bold",
    fontsize=12,
    color="w",
...
for idx, plot_pos in enumerate(np.arange(0.2, len(plot_df))):
    if idx not in [n_players, 2 * n_players + 1]:
        ...
        if plot_df.loc[idx, "increase_team_xT_90_on"] > 0:
            ax.text(
                plot_df.loc[idx, "increase_team_xT_90_on"],
                plot_pos,
                "+" + str(round(plot_df.loc[idx, "increase_team_xT_90_on"], 2)) + " ",
                fontsize=8,
                color="w",
                va="bottom",
                ha="right",
            ...
            ab = AnnotationBbox(
                OffsetImage(team_logo, zoom=0.05, resample=True),
                (-xdist * 0.025, plot_pos - ydist * 0.007),
                frameon=False,
            )
            ax.add_artist(ab)
            ax.text(
...
# Save image to file
fig.savefig(
    f"player_impact_on_team/{league}-{year}-player_impact_on_threat_creation", dpi=300
)
...
#  Get top 10 and bottom 10 players and pad with empty rows
def_playerinfo_df = def_playerinfo_df.sort_values(
    "decrease_opp_xT_90_on", ascending=False
)
empty_df = pd.DataFrame(index=[0], columns=att_playerinfo_df.columns)
...
plot_df = pd.concat(
    [
        def_playerinfo_df.head(n_players),
        empty_df,
        def_playerinfo_df.tail(n_players),
        empty_df,
    ]
)
plot_df.reset_index(drop=True, inplace=True)
...
color_scale = (
    255
    * (plot_df["decrease_opp_xT_90_on"] - plot_df["decrease_opp_xT_90_on"].min())
    / (plot_df["decrease_opp_xT_90_on"].max() - plot_df["decrease_opp_xT_90_on"].min())
)
color_scale = [int(color) for color in color_scale.values.tolist()]
...
title_text = f"{leagues[league]} {year}/{str(int(year) + 1).replace('20', '', 1)} − Player Impact on Team"
subtitle_text = (
    "A Comparison of Team Threat Concession when Player is On vs. Off the Pitch"
)
...
# Define axes
ax.set_xlabel(
    "Reduction in total xT conceded by team when on pitch (ΔxT Against/90)",
    labelpad=10,
    fontweight="bold",
    fontsize=12,
    color="w",
...
for idx, plot_pos in enumerate(np.arange(0.2, len(plot_df))):
    if idx not in [n_players, 2 * n_players + 1]:
        ...
        if plot_df.loc[idx, "decrease_opp_xT_90_on"] > 0:
            ax.text(
                plot_df.loc[idx, "decrease_opp_xT_90_on"],
                plot_pos,
                "+" + str(round(plot_df.loc[idx, "decrease_opp_xT_90_on"], 2)) + " ",
                fontsize=8,
                color="w",
                va="bottom",
                ha="right",
...
# Save image to file
fig.savefig(
    f"player_impact_on_team/{league}-{year}-player_impact_on_threat_concession", dpi=300
)
...
empty_df["name"] = ""
plot_df = pd.concat(
    [playerinfo_df.head(n_players), empty_df, playerinfo_df.tail(n_players), empty_df]
)
plot_df.reset_index(drop=True, inplace=True)
...
color_scale = (
    255
    * (plot_df["increase_delta_xT_90_on"] - plot_df["increase_delta_xT_90_on"].min())
    / (
        plot_df["increase_delta_xT_90_on"].max()
        - plot_df["increase_delta_xT_90_on"].min()
    )
)
color_scale = [int(color) for color in color_scale.values.tolist()]
...
title_text = f"{leagues[league]} {year}/{str(int(year) + 1).replace('20', '', 1)} − Player Impact on Team"
subtitle_text = (
    "A Comparison of Team Threat Difference when Player is On vs. Off the Pitch"
)
...
# Define axes
ax.set_xlabel(
    "Improvement in team 'xT For' − 'xT Against' when on pitch (ΔxT Diff/90)",
    labelpad=10,
    fontweight="bold",
    fontsize=12,
    color="w",
...
for idx, plot_pos in enumerate(np.arange(0.2, len(plot_df))):
    if idx not in [n_players, 2 * n_players + 1]:
        ...
        if plot_df.loc[idx, "increase_delta_xT_90_on"] > 0:
            ax.text(
                plot_df.loc[idx, "increase_delta_xT_90_on"],
                plot_pos,
                "+" + str(round(plot_df.loc[idx, "increase_delta_xT_90_on"], 2)) + " ",
                fontsize=8,
                color="w",
                va="bottom",
                ha="right",
...
# Save image to file
fig.savefig(
    f"player_impact_on_team/{league}-{year}-player_impact_on_threat_difference", dpi=300
)
