...
# Statsbomb data
events = bz2.BZ2File(
    "../../data_directory/statsbomb_data/2017_18/FIFA World Cup/fifa-world-cup-2018-eventdata.pbz2",
    "rb",
)
events = pickle.load(events)
lineups = bz2.BZ2File(
    "../../data_directory/statsbomb_data/2017_18/FIFA World Cup/fifa-world-cup-2018-lineupdata.pbz2",
    "rb",
)
lineups = pickle.load(lineups)

# Misc data
dist_covered = pd.read_excel(
...
lineups = sde.events_while_playing(
    events, lineups, event_name="Pass", event_team="opposition"
)
...
playerinfo_df = playerinfo_df[
    (playerinfo_df["mins_played"] >= minimum_mins)
    & (
...
midfielder_events = events[
    (events["player"].isin(playerinfo_df["player_name"]))
    & (events["position"].isin(midfielder_pos))
]

# %% Add custom event types that do not require time-based event information

# Passes into the box
midfielder_events.loc[:, "pass_into_box"] = midfielder_events.apply(
    sce.pass_into_box, axis=1
)

# Progressive passes
midfielder_events.loc[:, "progressive_pass"] = midfielder_events.apply(
    sce.progressive_pass, axis=1
)
...
playerinfo_df = sde.group_player_events(
    passes,
    playerinfo_df,
    event_types=[
        "pre_assist",
    ...
    primary_event_name="passes",
)
playerinfo_df = sde.group_player_events(
    passes, playerinfo_df, group_type="sum", event_types=["xg_assisted"]
)
playerinfo_df = sde.group_player_events(
    successful_passes,
    playerinfo_df,
    event_types=["under_pressure"],
    primary_event_name="successful_passes",
)

# Shots
shots = midfielder_events[
    (midfielder_events["type"] == "Shot")
    & (midfielder_events["shot_type"] != "Penalty")
]
playerinfo_df = sde.group_player_events(
    shots, playerinfo_df, group_type="sum", event_types=["shot_statsbomb_xg"]
)

# Pressures
pressures = midfielder_events[midfielder_events["type"] == "Pressure"]
playerinfo_df = sde.group_player_events(
    pressures, playerinfo_df, primary_event_name="pressures"
)

# Tackles
tackles = midfielder_events[
    (midfielder_events["type"] == "Duel") & (midfielder_events["duel_type"] != "Tackle")
]
playerinfo_df = sde.group_player_events(
    tackles, playerinfo_df, primary_event_name="tackles"
)

# Interceptions
interceptions = midfielder_events[
    (midfielder_events["type"] == "Interception")
    & (midfielder_events["interception_outcome"] != "Lost")
]
playerinfo_df = sde.group_player_events(
    interceptions, playerinfo_df, primary_event_name="interceptions"
)

# Recoveries
recoveries = midfielder_events[
    (midfielder_events["type"] == "Ball Recovery")
    & (midfielder_events["ball_recovery_recovery_failure"] != True)
]
playerinfo_df = sde.group_player_events(
    recoveries, playerinfo_df, primary_event_name="recoveries"
)

# Distance covered / 90
playerinfo_df = playerinfo_df.merge(
    dist_covered, left_on="player_name", right_on="Midfielders", how="outer"
).drop("Midfielders", axis=1)

# %% Normalise information and calculate additional metrics

# Per 90 minutes
for normalised_col in [
    "progressive_pass",
    "pass_into_box",
    "pass_goal_assist",
    "pre_assist",
    ...
    playerinfo_df[normalised_col + "_90"] = round(
        90 * playerinfo_df[normalised_col] / playerinfo_df["mins_played"], 2
    )

# Per 100 opposition passes
for normalised_col in ["pressures", "tackles", "interceptions", "recoveries"]:
    playerinfo_df[normalised_col + "_100pass"] = round(
        100 * playerinfo_df[normalised_col] / playerinfo_df["opp_pass"], 2
    )

# Percentage-based info
playerinfo_df["successful_passes_%"] = round(
    100 * (playerinfo_df["successful_passes"] / playerinfo_df["passes"]), 0
)
playerinfo_df["pass_under_pressure_%"] = round(
    100 * (playerinfo_df["under_pressure_x"] / playerinfo_df["passes"]), 0
)
playerinfo_df["pressure_pass_success_%"] = round(
    100 * (playerinfo_df["under_pressure_y"] / playerinfo_df["under_pressure_x"]), 0
)
playerinfo_df["xGC_90"] = (
    playerinfo_df["shot_statsbomb_xg_90"] + playerinfo_df["xg_assisted_90"]
)
...
for player_choice in players_considered:
    player_def_hull = sce.create_convex_hull(
        defensive_actions_df[defensive_actions_df["player_nickname"] == player_choice],
        name=player_choice,
        include_events="1std",
    )
    player_off_hull = sce.create_convex_hull(
        offensive_actions_df[offensive_actions_df["player_nickname"] == player_choice],
        name=player_choice,
        include_events="1std",
    )
...
hull_df = defensive_hull_df.merge(
    offensive_hull_df, left_index=True, right_index=True, suffixes=("_def", "_off")
)
...
playerinfo_df = playerinfo_df.merge(
    hull_df[["hull_area_%_def", "hull_area_%_off", "hull_area_tot"]],
    left_on="player_name",
    right_index=True,
    how="left",
)
...
playerscore_df[["player_name", "player_nickname"]] = playerinfo_df[
...
playerscore_df["creativity"] = (
    playerscore_df[["progressive_pass_90", "xGC_90", "pressure_pass_success_%"]].sum(
        axis=1
    )
    / 3
)
...
playerscore_df[["Distance", "hull_area_%_off", "hull_area_%_def"]] = (
    scaler.fit_transform(
        playerinfo_df[["Distance", "hull_area_%_off", "hull_area_%_def"]]
    )
)
playerscore_df["mobility"] = (
    playerscore_df[["Distance", "hull_area_%_off", "hull_area_%_def"]].sum(axis=1) / 3
)

# %% Score overall

playerscore_df["total"] = (
    (4 / 9) * playerscore_df["creativity"]
    + (2 / 9) * playerscore_df["ball_winning"]
    + (3 / 9) * playerscore_df["mobility"]
)
...
passes_to_plot = midfielder_events[
    (
        (midfielder_events["progressive_pass"] == True)
...
for _, name in sorted_df.head(12).iterrows():
    ...
    ax["pitch"][idx].text(
        3, 71, "Total:", fontsize=8, fontweight="bold", color="w", zorder=3
    )
    ax["pitch"][idx].text(
        25, 71, f"{name['progressive_pass']}", fontsize=8, color="w", zorder=3
    )
    ax["pitch"][idx].text(
        3, 77, "Per 90:", fontsize=8, fontweight="bold", color="w", zorder=3
    )
    ax["pitch"][idx].text(
        25,
        77,
        f"{round(name['progressive_pass_90'], 1)}",
        fontsize=8,
        color="w",
        zorder=3,
    )
...
subtitle_text = (
    f"{competition} {year} - <Progressive passes>, <Assists> and <Pre-Assists>"
)
...
fig.text(
    0.5,
    0.04,
    "Created by Jake Kolliari. Data provided by Statsbomb",
    fontstyle="italic",
    ha="center",
    fontsize=9,
    color="white",
...
ax = fig.add_axes([0.0, 0.85, 0.15, 0.15])
ax.axis("off")
...
ax.scatter(
    playerinfo_df["pass_under_pressure_%"],
    playerinfo_df["pressure_pass_success_%"],
    s=100,
    c=playerinfo_df["pressure_pass_success_%"],
    cmap="coolwarm",
    edgecolors="w",
    alpha=0.75,
    lw=0.5,
...
for i, player in playerinfo_df.iterrows():
    if (
        player["pressure_pass_success_%"] >= y80
        or player["pass_under_pressure_%"] >= x80
    ):
        text.append(
            ax.text(
                player["pass_under_pressure_%"] + 0.2,
                player["pressure_pass_success_%"] + 0.2,
...
# Define axes
ax.set_xlabel(
    "% of Passes Played Under Pressure", fontweight="bold", fontsize=14, color="w"
)
...
for _, name in sorted_df.head(12).iterrows():
    ...
    hist = ax["pitch"][idx].imshow(
        100 * hist_data[0] / name["pressures"],
        extent=[0, 120, 0, 80],
        cmap=plt.cm.viridis,
        vmin=0,
        vmax=6,
    ...
    ax["pitch"][idx].text(
        3, 77, "Per 100 passes:", fontsize=8, fontweight="bold", color="w", zorder=3
    )
    ax["pitch"][idx].text(
        49,
        77,
        f"{round(name['pressures_100pass'], 1)}",
        fontsize=8,
        fontweight="bold",
        color="w",
        zorder=3,
...
# Create title and subtitles, using highlighting as figure legend
title_text = (
    "Top 12 Midfielders, ranked by number of pressures per 100 opposition passes"
)
subtitle_text = (
    f"{competition} {year} - Distribution of pressures applied during competition"
)
subsubtitle_text = (
    "Players with total game\ntime below 180 minutes\nare not considered."
)
fig.text(0.11, 0.93, title_text, fontweight="bold", fontsize=16, color="w")
fig.text(0.11, 0.895, subtitle_text, fontweight="regular", fontsize=14, color="w")
...
ax.grid(ls="dotted", lw="0.5", color="lightgrey", zorder=1)
ax.scatter(
    playerinfo_df["recoveries_100pass"],
    y,
    s=100,
    c=y + playerinfo_df["recoveries_100pass"],
    cmap="coolwarm",
    edgecolors="w",
    alpha=0.75,
    lw=0.5,
...
for i, player in playerinfo_df.iterrows():
    if (
        player["interceptions_100pass"] + player["tackles_100pass"] >= y80
        or player["recoveries_100pass"] >= x80
    ):
        text.append(
            ax.text(
                player["recoveries_100pass"] + 0.01,
                player["interceptions_100pass"] + player["tackles_100pass"] + 0.01,
                player["player_nickname"],
                color="w",
                fontsize=8,
                ha="left",
...
# Define axes
ax.set_xlabel(
    "Ball Recoveries per 100 Opposition Passes",
    fontweight="bold",
    fontsize=14,
    color="w",
)
ax.set_ylabel(
    "Tackles + Interceptions per 100 Opposition Passes",
    fontweight="bold",
    fontsize=14,
    color="w",
)
...
for hull_idx, hull_row in hull_df.head(12).iterrows():
    ...
    def_hull = pitch.convexhull(
        hull_row["hull_reduced_x_def"], hull_row["hull_reduced_y_def"]
    )
    off_hull = pitch.convexhull(
        hull_row["hull_reduced_x_off"], hull_row["hull_reduced_y_off"]
    )
    pitch.polygon(
        def_hull,
        ax=ax["pitch"][idx],
    ...
    ax["pitch"][idx].text(
        42,
        -1.5,
        f"{round(hull_row['hull_area_%_def'])}%",
        fontsize=7,
        color="w",
        zorder=3,
    )
    ...
    ax["pitch"][idx].text(
        100,
        -1.5,
        f"{round(hull_row['hull_area_%_off'])}%",
        fontsize=7,
        color="w",
        zorder=3,
    )
...
# Create title and subtitles, using highlighting as figure legend
title_text = (
    "Top 12 Midfielders, ranked by distribution of defensive actions over competition"
)
...
# Create title and subtitles, using highlighting as figure legend
title_text = (
    "Top 12 Midfielders, ranked by distribution of offensive actions over competition"
)
...
radarNorm_df = pd.DataFrame(
    {
        "Player": playerscore_df["player_nickname"],
...
radar_df = pd.DataFrame(
    {
        "Player": playerinfo_df["player_nickname"],
        ...
        "Interceptions / 100 opp. passes": round(
            playerinfo_df["interceptions_100pass"], 2
        ),
        "Tackles / 100 opp. passes": round(playerinfo_df["tackles_100pass"], 2),
        "Def. Cvx Hull Area (% Pitch)": round(
            playerinfo_df["hull_area_%_def"].astype(float), 1
        ),
        "Off. Cvx Hull Area (% Pitch)": round(
            playerinfo_df["hull_area_%_off"].astype(float), 1
        ),
...
text = [
    "\n".join(wrap(r + f" = {radar[r].values[0]}", 12, break_long_words=False))
    for r in radar.columns.values
]
...
for i, label in enumerate(radar.columns.values):
    ...
    onethird_val = round(
        min(radar_df[label]) + (1 / 3) * (max_val - min(radar_df[label])), 2
    )
    twothird_val = round(
        min(radar_df[label]) + (2 / 3) * (max_val - min(radar_df[label])), 2
    )
    label_val = float(round(radar.loc[:, label], 2))

    if label in [
        "Pass Success (%)",
        "Pressure Pass Success (%)",
        "Def. Cvx Hull Area (% Pitch)",
    ...
    text_angles = [
        (180 / np.pi) * (radar_angles[i] - 0.13 + np.pi / len(radarNorm.columns)) - 90,
