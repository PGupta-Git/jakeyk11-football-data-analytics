...
for match_id in team_match_ids:
    ...
    for pos_id in team_events_df[
        (team_events_df["match_id"] == match_id)
        & (team_events_df["possession_team"] == team_id)
    ]["possession_id"].unique():
        # Get possession chain and count up passes
        possession_chain = team_events_df[
            (team_events_df["match_id"] == match_id)
            & (team_events_df["possession_id"] == pos_id)
        ].copy()
        ...
        if len(first_pass) == 1:
            possession_chain_passes = possession_chain[
                (possession_chain["eventId"] != first_pass["eventId"].values[0])
                & (possession_chain["eventType"] == "Pass")
                & (~possession_chain["satisfiedEventsTypes"].apply(lambda x: 31 in x))
            ]
        ...
        if len(possession_chain_passes) > 0:
            possession_chain.loc[
                possession_chain_passes.index.values, "pass_number"
            ] = np.arange(1, len(possession_chain_passes) + 1)
...
pos_chain_df.loc[
    (pos_chain_df["evt_number"] == 1) & (pos_chain_df["x"] < 100 / 3), "pos_start"
] = "Own 3"
pos_chain_df.loc[
    (pos_chain_df["evt_number"] == 1)
    & (pos_chain_df["x"] >= 100 / 3)
    & (pos_chain_df["x"] < 200 / 3),
    "pos_start",
] = "Mid 3"
pos_chain_df.loc[
    (pos_chain_df["evt_number"] == 1) & (pos_chain_df["x"] >= 200 / 3), "pos_start"
] = "Opp 3"
...
for idx in np.arange(3 * first_n_passes):
    ...
    pass_top_cluster_ids = (
        pass_plot.groupby("pass_cluster_id")
        .count()["id"]
        .sort_values(ascending=False)
        .head(clusters_shown)
        .index.values
    )
    cluster_rank_dict = dict(
        zip(pass_top_cluster_ids, np.arange(1, len(pass_top_cluster_ids) + 1))
    )
    cluster_color_dict = dict(
        zip(pass_top_cluster_ids, cluster_colours[0:clusters_shown])
    )
    pass_top_clusters = pass_plot[
        pass_plot["pass_cluster_id"].isin(pass_top_cluster_ids)
    ].copy()
    pass_top_clusters["cluster_rank"] = pass_top_clusters["pass_cluster_id"].apply(
        lambda x: cluster_rank_dict[x]
    )
    pass_top_clusters["cluster_c"] = pass_top_clusters["pass_cluster_id"].apply(
        lambda x: cluster_color_dict[x]
    )
    ...
    ax["pitch"][idx].set_title(
        f"{pass_num}{suffix} Pass", pad=-1, color="w", fontsize=9
    )
...
subtitle_text = (
    f"Where {team} directed passes during possessions starting in each third"
)
...
for loc in [[0.38, 0.84], [0.42, 0.57], [0.381, 0.3]]:
    sep_ax = fig.add_axes([0.02, loc[1], 0.96, 0.02])
    sep_ax.axis("off")
...
fig.text(
    0.075,
    0.845,
    "Possessions Initiated in Final Third",
    fontweight="bold",
    fontsize=11,
    color="w",
)
fig.text(
    0.075,
    0.575,
    "Possessions Initiated in Centre of Pitch",
    fontweight="bold",
    fontsize=11,
    color="w",
)
fig.text(
    0.075,
    0.305,
    "Possessions Initiated in Own Third",
    fontweight="bold",
    fontsize=11,
    color="w",
)
...
# Save
plt.savefig(
    f"team_pass_tendencies/{team.lower().replace(' ', '')}-{league.lower().replace(' ', '')}-{year}",
    dpi=300,
)
