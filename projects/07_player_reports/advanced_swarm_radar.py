...
players_df = wde.events_while_playing(
    events_df, players_df, event_name="Pass", event_team="opposition"
)
players_df = wde.events_while_playing(
    events_df, players_df, event_name="Touch", event_team="own"
)
players_df = wde.events_while_playing(
    events_df[events_df["x"] <= 100 / 3],
    players_df,
    event_name="Touch",
    event_team="opposition",
).rename(columns={"opp_touch": "opp_touch_own_3rd"})
players_df = wde.events_while_playing(
    events_df, players_df, event_name="Touch", event_team="opposition"
)
...
playerinfo_df = wde.create_player_list(
    players_df,
    pass_extra=["competition", "year"],
    additional_cols=[
        "team_touch",
        "opp_touch",
        "opp_touch_own_3rd",
...
playerinfo_df = playerinfo_df[
    (playerinfo_df["name"].isin([player_1["name"], player_2["name"]]))
    | (
...
playerinfo_df = wde.group_player_events(
    touches, playerinfo_df, group_type="count", col_names="touches"
)
...
playerinfo_df = wde.group_player_events(
    box_touches, playerinfo_df, group_type="count", col_names="box_touches"
)
playerinfo_df = wde.group_player_events(
    suc_box_entries, playerinfo_df, group_type="count", col_names="suc_box_entries"
)
...
pass_carry_f3rd = pass_carry[
    (pass_carry["x"] < 100 / 3) & (pass_carry["endX"] >= 200 / 3)
]
assists = passes[
    passes["satisfiedEventsTypes"].apply(lambda x: True if 92 in x else False)
]
playerinfo_df = wde.group_player_events(
    passes, playerinfo_df, group_type="count", col_names="passes"
)
playerinfo_df = wde.group_player_events(
    suc_passes, playerinfo_df, group_type="count", col_names="suc_passes"
)
playerinfo_df = wde.group_player_events(
    carries, playerinfo_df, group_type="count", col_names="carries"
)
playerinfo_df = wde.group_player_events(
    passes, playerinfo_df, group_type="sum", agg_columns="xThreat", col_names="pass_xt"
)
playerinfo_df = wde.group_player_events(
    carries,
    playerinfo_df,
...
playerinfo_df = wde.group_player_events(
    pass_carry_f3rd, playerinfo_df, group_type="count", col_names="final_3rd_entries"
)
playerinfo_df = wde.group_player_events(
    prog_pass_carry, playerinfo_df, group_type="count", col_names="prog_actions"
)

# Shots and big chances
shots = events_df[
    (events_df["eventType"].isin(["MissedShots", "SavedShot", "ShotOnPost", "Goal"]))
    & (
...
shots["shot_distance"] = shots[["x", "y"]].apply(
    lambda a: np.sqrt(
...
big_chances = events_df[
    events_df["satisfiedEventsTypes"].apply(
        lambda x: True if 203 in x and not (31 in x or 34 in x or 212 in x) else False
    )
]
playerinfo_df = wde.group_player_events(
    shots, playerinfo_df, group_type="count", col_names="shots"
)
playerinfo_df = wde.group_player_events(
    shots,
    playerinfo_df,
    group_type="mean",
    agg_columns="shot_distance",
    col_names="mean_shot_dist",
)
playerinfo_df = wde.group_player_events(
    goals, playerinfo_df, group_type="count", col_names="goals"
)
playerinfo_df = wde.group_player_events(
    gc, playerinfo_df, group_type="count", col_names="goal_contributions"
)
playerinfo_df = wde.group_player_events(
    box_shots, playerinfo_df, group_type="count", col_names="box_shots"
)
playerinfo_df = wde.group_player_events(
    box_goals, playerinfo_df, group_type="count", col_names="box_goals"
)
playerinfo_df = wde.group_player_events(
    big_chances, playerinfo_df, group_type="count", col_names="big_chances"
)
...
playerinfo_df = wde.group_player_events(
    aerials, playerinfo_df, group_type="count", col_names="aerials"
)
playerinfo_df = wde.group_player_events(
    aerials_won, playerinfo_df, group_type="count", col_names="aerials_won"
)
...
playerinfo_df = wde.group_player_events(
    take_on, playerinfo_df, group_type="count", col_names="take_on"
)
playerinfo_df = wde.group_player_events(
    suc_take_on, playerinfo_df, group_type="count", col_names="suc_take_on"
)

# Crosses
crosses = passes[
    passes["satisfiedEventsTypes"].apply(
        lambda x: True
        if (125 in x or 126 in x or 59 in x) and not (31 in x or 34 in x or 212 in x)
        else False
    )
]
...
playerinfo_df = wde.group_player_events(
    crosses, playerinfo_df, group_type="count", col_names="cross"
)
playerinfo_df = wde.group_player_events(
    cross_to_chance, playerinfo_df, group_type="count", col_names="cross_to_chance"
)

# High ball wins
recoveries = events_df[
    (events_df["eventType"] == "BallRecovery")
    & (events_df["outcomeType"] == "Successful")
]
interceptions = events_df[
    (events_df["eventType"] == "Interception")
    & (events_df["outcomeType"] == "Successful")
]
bad_tackles = tackles = events_df[
    (events_df["eventType"] == "Tackle") & (events_df["outcomeType"] == "Unsuccessful")
]
tackles = events_df[
    (events_df["eventType"] == "Tackle") & (events_df["outcomeType"] == "Successful")
]
pass_blocks = events_df[
    (events_df["eventType"] == "BlockedPass")
    & (events_df["outcomeType"] == "Successful")
]
ball_wins = pd.concat([recoveries, interceptions, tackles, pass_blocks], axis=0)
ball_wins_high_33 = ball_wins[ball_wins["x"] >= 200 / 3]
playerinfo_df = wde.group_player_events(
    tackles, playerinfo_df, group_type="count", col_names="tackles"
)
playerinfo_df = wde.group_player_events(
    bad_tackles, playerinfo_df, group_type="count", col_names="bad_tackles"
)
playerinfo_df = wde.group_player_events(
    ball_wins, playerinfo_df, group_type="count", col_names="ball_wins"
)
playerinfo_df = wde.group_player_events(
    ball_wins_high_33, playerinfo_df, group_type="count", col_names="high_ball_wins"
)

# Ball losses
bad_touches = events_df[
    (events_df["eventType"] == "BallTouch")
    & (events_df["outcomeType"] == "Unsuccessful")
]
bad_pass = events_df[
    (events_df["eventType"] == "Pass") & (events_df["outcomeType"] == "Unsuccessful")
]
bad_takeon = events_df[
    (events_df["eventType"] == "TakeOn") & (events_df["outcomeType"] == "Unsuccessful")
]
...
playerinfo_df = wde.group_player_events(
    ball_losses, playerinfo_df, group_type="count", col_names="ball_losses"
)

# %% Normalise metrics

playerinfo_df["pass_xt_100_team_touch"] = (
    100 * playerinfo_df["pass_xt"] / playerinfo_df["team_touch"]
)
playerinfo_df["carry_xt_100_team_touch"] = (
    100 * playerinfo_df["carry_xt"] / playerinfo_df["team_touch"]
)
playerinfo_df["final_3rd_entries_100_team_touch"] = (
    100 * playerinfo_df["final_3rd_entries"] / playerinfo_df["team_touch"]
)
playerinfo_df["prog_actions_100_team_touch"] = (
    100 * playerinfo_df["prog_actions"] / playerinfo_df["team_touch"]
)
playerinfo_df["pass_success_pct"] = (
    100 * playerinfo_df["suc_passes"] / playerinfo_df["passes"]
)
playerinfo_df["goal_contributions_100_team_pass"] = (
    100 * playerinfo_df["goal_contributions"] / playerinfo_df["team_pass"]
)
playerinfo_df["goal_contributions_90"] = (
    90 * playerinfo_df["goal_contributions"] / playerinfo_df["mins_played"]
)
playerinfo_df["box_touches_100_team_pass"] = (
    100 * playerinfo_df["box_touches"] / playerinfo_df["team_pass"]
)
playerinfo_df["box_entries_pct"] = (
    100
    * playerinfo_df["suc_box_entries"]
    / (playerinfo_df["passes"] + playerinfo_df["carries"])
)
playerinfo_df["xt_gen_100_team_pass"] = (
    100
    * (playerinfo_df["pass_xt"] + playerinfo_df["carry_xt"])
    / playerinfo_df["team_pass"]
)
playerinfo_df["xt_gen_100_team_touch"] = (
    100
    * (playerinfo_df["pass_xt"] + playerinfo_df["carry_xt"])
    / playerinfo_df["team_touch"]
)
playerinfo_df["box_shot_conversion"] = (
    100 * playerinfo_df["box_goals"] / playerinfo_df["box_shots"]
)
playerinfo_df["big_chances_100_team_pass"] = (
    100 * playerinfo_df["big_chances"] / playerinfo_df["team_pass"]
)
playerinfo_df["aerial_win_pct"] = (
    100 * playerinfo_df["aerials_won"] / playerinfo_df["aerials"]
)
playerinfo_df["tackles_success_pct"] = (
    100
    * playerinfo_df["tackles"]
    / (playerinfo_df["tackles"] + playerinfo_df["bad_tackles"])
)
playerinfo_df["takeon_pct"] = (
    100 * playerinfo_df["suc_take_on"] / playerinfo_df["take_on"]
)
playerinfo_df["cross_chance_pct"] = (
    100 * playerinfo_df["cross_to_chance"] / playerinfo_df["cross"]
).fillna(0)
playerinfo_df["ball_wins_100_opp_touch"] = (
    100 * playerinfo_df["ball_wins"] / playerinfo_df["opp_touch"]
)
playerinfo_df["high_ball_wins_100_opp_pass_own_3rd"] = (
    100 * playerinfo_df["high_ball_wins"] / playerinfo_df["opp_touch_own_3rd"]
)
playerinfo_df["ball_losses_100_touch"] = (
    100 * playerinfo_df["ball_losses"] / playerinfo_df["touches"]
)
...
template = "Forward"
swarm_radar_metrics = [
    "goal_contributions_100_team_passes",
...
path_eff = [
    path_effects.Stroke(linewidth=2, foreground="#313332"),
    path_effects.Normal(),
]
...
if player_1["name"] != "":
    player_1_id = playerinfo_df[
        (playerinfo_df["name"] == player_1["name"])
        & (playerinfo_df["competition"] == player_1["comp"])
        & (playerinfo_df["year"] == player_1["year"])
    ].index.values[0]
    playerinfo_df.loc[player_1_id, "Primary Player"] = "Primary 1"
if player_2["name"] != "":
    player_2_id = playerinfo_df[
        (playerinfo_df["name"] == player_2["name"])
        & (playerinfo_df["competition"] == player_2["comp"])
        & (playerinfo_df["year"] == player_2["year"])
    ].index.values[0]
    playerinfo_df.loc[player_2_id, "Primary Player"] = "Primary 2"

playerinfo_df["Plot Size"] = playerinfo_df["Primary Player"].apply(
    lambda x: 7 if x == "Untagged" else 7
)
...
x_base, y_base = (
    0.325 + r_base * np.cos(theta_mid),
    0.3 + 0.89 * r_base * np.sin(theta_mid),
)
...
for idx, metric in enumerate(swarm_radar_metrics):
    ...
    # Plot on axis
    sns.swarmplot(
        x=playerinfo_df[metric],
        y=[""] * len(playerinfo_df),
        color="grey",
        edgecolor="w",
        s=playerinfo_df["Plot Size"],
        zorder=1,
    )
    ...
    ax_save.tick_params(
        axis="both", which="major", labelsize=8, zorder=10, pad=0, colors="w"
    )
    ...
    ax_div = Divider(
        fig,
        [x_base[idx], y_base[idx], 0.35, 0.35],
        horiz_scale,
        vert_scale,
        aspect=True,
    )
...
if player_1["name"] != "":
    team_logo_1, _ = lab.get_team_badge_and_colour(
        playerinfo_df.loc[player_1_id, "team"]
    )
    ...
    title_text_3 = (
        playerinfo_df.loc[player_1_id, "competition"]
        + " "
        + playerinfo_df.loc[player_1_id, "year"]
        + "/"
        + str(int(playerinfo_df.loc[player_1_id, "year"]) + 1).replace("20", "", 1)
    )
    fig.text(
        0.11, 0.953, title_text_1, fontweight="bold", fontsize=14, color=player_1_colour
    )
...
if player_2["name"] != "":
    team_logo_2, _ = lab.get_team_badge_and_colour(
        playerinfo_df.loc[player_2_id, "team"]
    )
    ...
    title_text_3 = (
        playerinfo_df.loc[player_2_id, "competition"]
        + " "
        + playerinfo_df.loc[player_2_id, "year"]
        + "/"
        + str(int(playerinfo_df.loc[player_2_id, "year"]) + 1).replace("20", "", 1)
    )
    fig.text(
        0.48, 0.953, title_text_1, fontweight="bold", fontsize=14, color=player_2_colour
    )
...
radar_values_p1 = (
    playerinfo_df[playerinfo_df["Primary Player"] == "Primary 1"][pizza_metrics]
    .values[0]
    .tolist()
)
radar_values_p2 = (
    playerinfo_df[playerinfo_df["Primary Player"] == "Primary 2"][pizza_metrics]
    .values[0]
    .tolist()
)

radar_object = PyPizza(
    params=pizza_metrics,
    background_color="w",
...
radar_object.make_pizza(
    ...
    kwargs_slices=dict(
        facecolor=player_1_colour, alpha=0.3, edgecolor="#313332", linewidth=1, zorder=1
    ),
    kwargs_compare=dict(
        facecolor=player_2_colour, alpha=0.3, edgecolor="#313332", linewidth=1, zorder=3
    ),
    ax=pizza_ax,
)
...
description_text = "\n ".join(
    tw.wrap(description, 30, break_long_words=False, drop_whitespace=True)
)

fig.text(
    0.975, 0.953, title_text, fontweight="bold", fontsize=12, color="w", ha="right"
)
fig.text(
    0.975,
    0.942,
...
# Save fig
plt.savefig(
    f"advanced_radars/{comparison_competition['comp'].lower()}-{comparison_competition['year']}-{player_1['name'].replace(' ', '-').lower()}-{player_2['name'].replace(' ', '-').lower()}",
    dpi=300,
)
