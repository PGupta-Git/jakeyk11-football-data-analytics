...
playerinfo_df = bz2.BZ2File(
    f"../../data_directory/transfermarkt_data/{league_year}_{int(league_year.replace('20', '', 1)) + 1}/{filename}",
    "rb",
)
...
playerinfo_df["GC_per_mGBP"] = round(
    playerinfo_df["Goal_Contributions"] / playerinfo_df["Market_Value"], 2
)
playerinfo_df["mGBP_per_GC"] = 1 / playerinfo_df["GC_per_mGBP"]

# Player goal contributions per £1m, and Cost per Goal Contribution (inverse)
playerinfo_df["Delta_Value"] = (
    playerinfo_df["Market_Value"] - playerinfo_df["Prev_Market_Value"]
)
...
teaminfo_mean = (
    playerinfo_df.groupby(["Team"])
    .mean()
    .reset_index()[["Team", "Mean_Market_Val", "Tot_Market_Val"]]
)
...
teaminfo_df["Value_Rank"] = teaminfo_df["Tot_Market_Val"].rank(
    method="min", ascending=False
)
teaminfo_df["Value_Pos_Delta"] = (
    teaminfo_df["Value_Rank"] - teaminfo_df["League_Position"]
)
...
playerinfo_df["Percent_GC"] = (
    100 * playerinfo_df["Goal_Contributions"] / playerinfo_df["Team_GC"]
)
...
forwardinfo_df.loc[forwardinfo_df["Position"].str.contains("Winger"), "Position"] = (
    "Winger"
)
forwardinfo_df.loc[forwardinfo_df["Position"].str.contains("Striker"), "Position"] = (
    "Centre-Forward"
)
...
successforward_df = forwardinfo_df[
    forwardinfo_df["Goal_Contributions"] >= min_contributions
]
...
low_contract = successforward_df[
    successforward_df["Contract_End_YY"] <= int(league_year) + 1
]
...
ax.scatter(
    successforward_df["Age"],
    successforward_df["GC_per_mGBP"],
    s=80,
    color=filler,
...
if player_tags:
    for player in successforward_df["Name"].values:
        ...
        if ypos.values[0] / xpos.values[0] >= age_gc_rat:
            if (player in rel_fwds["Name"].values) or (
                player in low_contract["Name"].values
            ):
                colour = text_color
            else:
                colour = filler
...
ax.set_ylabel(
    "Goal Contributions per £m player value",
    fontweight="bold",
    fontsize=14,
    color=text_color,
)
x_min = (
    successforward_df["Age"].min() - 2
    if (successforward_df["Age"].min() % 2) == 0
    else successforward_df["Age"].min() - 1
)
...
htext.fig_text(
    ...
    fontsize=12,
    color=text_color,
    highlight_textprops=[{"color": "r", "fontweight": "bold"}],
)
...
fig.text(
    0.28,
    0.02,
    "Created by Jake Kolliari. Data provided by Transfermarkt.co.uk",
    fontstyle="italic",
    fontsize=9,
    color=text_color,
)  # Credit text
...
fig.savefig(
    f"forward_value_analysis/{league_code}-{league_year}-cost-per-goal-contribution",
    dpi=300,
)
...
ax.grid(ls="dotted", lw="0.5", color="lightgrey", zorder=1)
ax.scatter(
    X_W, y_W, s=80, color="y", edgecolors=background, alpha=0.4, lw=0.5, zorder=2
)
ax.scatter(
    X_CF, y_CF, s=80, color="r", edgecolors=background, alpha=0.4, lw=0.5, zorder=3
)
ax.scatter(
    X_CAM, y_CAM, s=80, color="m", edgecolors=background, alpha=0.4, lw=0.5, zorder=4
)
...
if player_tags:
    for player in successforward_df["Name"].values:
        ...
        if xpos.values[0] >= np.percentile(
            forwardinfo_df["Goal_Contributions"], 80
        ) or ypos.values[0] >= np.percentile(forwardinfo_df["Market_Value"], 80):
            tag = player
            text.append(
                ax.text(
                    xpos.values[0] + 0.02,
...
fig.savefig(
    f"forward_value_analysis/{league_code}-{league_year}-goal-contribution-vs-value",
    dpi=300,
)
...
teaminfo_df["Team_Pos_Str"] = (
    teaminfo_df["Team"] + ": " + teaminfo_df["League_Position"].astype(str)
)
...
color_scale = (
    teaminfo_df["Value_Pos_Delta"] + teaminfo_df["League_Position"].max()
) / (2 * teaminfo_df["League_Position"].max())

# Plot data
ax.barh(
    teaminfo_df["Team_Pos_Str"],
    teaminfo_df["Tot_Market_Val"],
...
for idx, plot_pos in enumerate(np.arange(0.2, teaminfo_df["League_Position"].max())):
    ...
    ax.text(
        text_team["Tot_Market_Val"] + 1,
        plot_pos,
        value_str + str(int(text_team["Value_Rank"].values[0])),
        color="w",
    )
...
fig.savefig(
    f"market_value_league_table/{league_code}-{league_year}-market-value-league-table",
    dpi=300,
)
